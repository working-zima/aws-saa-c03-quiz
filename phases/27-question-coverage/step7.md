# Step 7: questions-dynamodb-cache — 문항 27개 이상
...
|---|---|---|---|---|
| `dynamodb` | DynamoDB | 3 | 18 | **16** |
| `elasticache-purpose-built-db` | ElastiCache·DAX·Neptune·DocumentDB·QLDB·Timestream | 3 | 14 | **11** |

**27개 개념에 문항이 없다. 이 step이 전부 덮는다.**

담당 개념 목록(괄호는 덤프 근거가 있는 파일):

- `dynamodb.dynamodb-single-digit-latency` — DynamoDB의 응답 시간  (Q051-Q100.md)
- `dynamodb.dynamodb-streams` — DynamoDB Streams  (Q151-Q200.md)
- `dynamodb.dynamodb-global-tables` — DynamoDB 글로벌 테이블  (Q001-Q050.md)
- `dynamodb.dynamodb-ttl` — DynamoDB TTL  (Q051-Q100.md)
- `dynamodb.dynamodb-global-secondary-index` — 전역 보조 인덱스(GSI)  (Q501-Q550.md)
- `dynamodb.dynamodb-capacity-modes` — 프로비저닝된 용량과 온디맨드 용량  (Q301-Q350.md)
- `dynamodb.dynamodb-auto-scaling-target-utilization` — DynamoDB 오토 스케일링과 목표 활용률  (Q501-Q550.md)
- `dynamodb.dynamodb-read-consistency` — 최종 일관성 읽기와 강력한 일관성 읽기  (Q301-Q350.md)
- `dynamodb.dynamodb-s3-export-vs-streams` — 분석용 적재는 기본 내보내기가 먼저다  (Q501-Q550.md)
- `dynamodb.dynamodb-incremental-export` — S3 내보내기에는 증분 형태가 있다  (Q601-Q650.md)
- `dynamodb.dynamodb-export-no-read-capacity` — S3 내보내기는 테이블의 읽기 용량을 쓰지 않는다  (Q551-Q600.md)
- `dynamodb.dynamodb-export-requires-pitr` — S3로 내보내려면 PITR이 켜져 있어야 한다  (Q501-Q550.md)
- `dynamodb.dynamodb-item-size-limit` — DynamoDB 항목 크기 제한  (Q301-Q350.md)
- `dynamodb.dynamodb-ttl-deletion-delay` — TTL 삭제는 만료 시각에 딱 맞춰 일어나지 않는다  (Q551-Q600.md)
- `dynamodb.dynamodb-streams-retention-24h` — DynamoDB Streams는 24시간만 보존한다  (Q651-Q700.md)
- `dynamodb.dynamodb-streams-batch-size` — DynamoDB Streams 소비의 배치 크기를 조정한다  (Q651-Q700.md)
- `elasticache-purpose-built-db.neptune` — Amazon Neptune  (Q151-Q200.md)
- `elasticache-purpose-built-db.neptune-streams` — Neptune Streams  (Q151-Q200.md)
- `elasticache-purpose-built-db.qldb` — Amazon Quantum Ledger Database(QLDB)  (Q401-Q450.md)
- `elasticache-purpose-built-db.timestream` — Amazon Timestream  (Q401-Q450.md)
- `elasticache-purpose-built-db.elasticache-redis-vs-memcached` — Redis와 Memcached의 갈림길  (Q001-Q050.md)
- `elasticache-purpose-built-db.elasticache-multi-az-failover` — ElastiCache의 다중 AZ 자동 장애 조치  (Q251-Q300.md)
- `elasticache-purpose-built-db.elasticache-global-datastore` — ElastiCache 글로벌 데이터스토어  (Q101-Q150.md)
- `elasticache-purpose-built-db.documentdb-global-cluster` — DocumentDB 글로벌 클러스터  (Q151-Q200.md)
- `elasticache-purpose-built-db.cache-requires-application-change` — 캐시를 넣으면 애플리케이션을 고쳐야 한다  (Q601-Q650.md)
- `elasticache-purpose-built-db.elasticache-not-a-durable-store` — 캐시는 처리 결과를 두는 자리가 아니다  (Q851-Q880.md)
- `elasticache-purpose-built-db.dax-encryption-at-rest` — DAX의 저장 중 암호화는 만들 때만 켠다  (Q201-Q250.md)

목록을 다시 뽑으려면:

```bash
node scripts/coverage.mjs dynamodb elasticache-purpose-built-db
```
...
node scripts/coverage.mjs dynamodb elasticache-purpose-built-db  # exit 0 — 담당 주제 커버리지 100%
```
