# Step 19: questions-org-cost — 문항 21개 이상
...
|---|---|---|---|---|
| `organizations-cloudtrail-config` | Organizations·SCP·CloudTrail·Config·Audit Manager | 3 | 16 | **13** |
| `cost-management` | 절약 플랜·Budgets·Cost Explorer·Trusted Advisor | 2 | 17 | **8** |

**21개 개념에 문항이 없다. 이 step이 전부 덮는다.**

담당 개념 목록(괄호는 덤프 근거가 있는 파일):

- `organizations-cloudtrail-config.cloudtrail-lake` — AWS CloudTrail Lake  (Q101-Q150.md)
- `organizations-cloudtrail-config.audit-manager` — AWS Audit Manager  (Q151-Q200.md)
- `organizations-cloudtrail-config.organizational-unit` — 조직 단위(OU)  (Q351-Q400.md)
- `organizations-cloudtrail-config.organizations-tag-policy` — 태그 정책과 SCP는 하는 일이 다르다  (Q401-Q450.md)
- `organizations-cloudtrail-config.organizations-consolidated-billing` — Organizations의 통합 청구  (Q101-Q150.md)
- `organizations-cloudtrail-config.cloudtrail-data-events` — 관리 이벤트와 데이터 이벤트  (Q151-Q200.md)
- `organizations-cloudtrail-config.config-configuration-recorder` — 구성 레코더  (Q151-Q200.md)
- `organizations-cloudtrail-config.scp-attachment-targets` — SCP를 붙일 수 있는 자리  (Q451-Q500.md)
- `organizations-cloudtrail-config.scp-condition-exception` — SCP에서 예외를 두는 방법  (Q001-Q050.md)
- `organizations-cloudtrail-config.cloudtrail-log-file-validation` — CloudTrail 로그 파일 유효성 검사  (Q501-Q550.md)
- `organizations-cloudtrail-config.config-conformance-pack` — AWS Config 준수 팩  (Q601-Q650.md)
- `organizations-cloudtrail-config.config-custom-rule` — 관리형 규칙에 없는 값은 사용자 지정 규칙으로 본다  (Q801-Q850.md)
- `organizations-cloudtrail-config.config-rule-remediation` — AWS Config 규칙과 자동 수정  (Q001-Q050.md)
- `cost-management.cost-and-usage-report` — AWS 비용 및 사용량 보고서(CUR)  (Q301-Q350.md)
- `cost-management.savings-plan-baseline-vs-spike` — 약정은 기준 용량까지만 건다  (Q151-Q200.md)
- `cost-management.rds-reserved-instance` — 예약 인스턴스는 RDS에도 있다  (Q651-Q700.md)
- `cost-management.on-demand-capacity-reservation` — 온디맨드 용량 예약  (Q051-Q100.md)
- `cost-management.cost-allocation-tag-activation-in-management-account` — 통합 청구에서는 관리 계정에서 활성화한다  (Q601-Q650.md)
- `cost-management.budget-actions` — AWS Budgets의 예산 조치  (Q101-Q150.md)
- `cost-management.budget-forecasted-alert` — 예산 알림은 예측값에도 걸 수 있다  (Q251-Q300.md)
- `cost-management.compute-optimizer-ebs-recommendations` — 권장 대상에는 EBS 볼륨도 있다  (Q551-Q600.md)

목록을 다시 뽑으려면:

```bash
node scripts/coverage.mjs organizations-cloudtrail-config cost-management
```
...
node scripts/coverage.mjs organizations-cloudtrail-config cost-management  # exit 0 — 담당 주제 커버리지 100%
```
