# Step 12: questions-apigw-stepfunctions — 문항 16개 이상
...
|---|---|---|---|---|
| `api-gateway-step-functions` | API Gateway·Step Functions | 3 | 20 | **16** |

**16개 개념에 문항이 없다. 이 step이 전부 덮는다.**

담당 개념 목록(괄호는 덤프 근거가 있는 파일):

- `api-gateway-step-functions.amplify` — AWS Amplify  (Q301-Q350.md)
- `api-gateway-step-functions.api-gateway-rest-vs-http-timeout` — REST API가 더 긴 통합 타임아웃을 견딘다  (Q501-Q550.md)
- `api-gateway-step-functions.api-gateway-rest-only-features` — API 키·요청 검증·스로틀링은 REST API 쪽이다  (Q701-Q750.md)
- `api-gateway-step-functions.api-gateway-websocket-api` — WebSocket API  (Q551-Q600.md)
- `api-gateway-step-functions.api-gateway-api-key-not-auth` — API 키는 인증 수단이 아니다  (Q301-Q350.md)
- `api-gateway-step-functions.api-gateway-resource-policy` — API Gateway 리소스 정책  (Q451-Q500.md)
- `api-gateway-step-functions.api-gateway-endpoint-types` — 엣지 최적화 엔드포인트와 리전 엔드포인트  (Q701-Q750.md)
- `api-gateway-step-functions.api-gateway-behind-cloudfront` — API도 CloudFront 뒤에 둘 수 있다  (Q251-Q300.md)
- `api-gateway-step-functions.api-gateway-lambda-proxy-integration` — REST API의 Lambda 프록시 통합  (Q151-Q200.md)
- `api-gateway-step-functions.api-gateway-aws-service-integration` — API Gateway에서 AWS 서비스를 직접 호출한다  (Q451-Q500.md)
- `api-gateway-step-functions.step-functions-long-running-workflow` — Step Functions는 하루를 넘기는 작업도 맡는다  (Q301-Q350.md)
- `api-gateway-step-functions.step-functions-express-workflow` — Express 워크플로  (Q751-Q800.md)
- `api-gateway-step-functions.step-functions-map-state` — Map 상태로 목록의 항목마다 한 번씩 실행한다  (Q651-Q700.md)
- `api-gateway-step-functions.api-gateway-custom-domain-name` — API Gateway 사용자 지정 도메인 이름  (Q501-Q550.md)
- `api-gateway-step-functions.api-gateway-mapping-template-limits` — 매핑 템플릿으로 되는 변환과 안 되는 변환  (Q551-Q600.md)
- `api-gateway-step-functions.api-gateway-ip-restriction-by-resource-policy` — 호출자를 IP로 거르는 자리도 리소스 정책이다  (Q851-Q880.md)

목록을 다시 뽑으려면:

```bash
node scripts/coverage.mjs api-gateway-step-functions
```
...
node scripts/coverage.mjs api-gateway-step-functions  # exit 0 — 담당 주제 커버리지 100%
```
