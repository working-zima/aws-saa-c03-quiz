# Step 18: questions-identity — 문항 20개 이상
...
|---|---|---|---|---|
| `iam-permissions` | IAM 사용자·그룹·역할·정책·권한 경계·Access Analyzer | 3 | 18 | **14** |
| `identity-federation` | IAM Identity Center·STS·Cognito·Directory Service·SAML | 3 | 11 | **6** |

**20개 개념에 문항이 없다. 이 step이 전부 덮는다.**

담당 개념 목록(괄호는 덤프 근거가 있는 파일):

- `iam-permissions.iam-group-policy-attachment` — 권한은 사용자가 아니라 그룹에 붙인다  (Q551-Q600.md)
- `iam-permissions.iam-roles-anywhere` — IAM Roles Anywhere  (Q401-Q450.md)
- `iam-permissions.iam-access-analyzer` — IAM Access Analyzer  (Q001-Q050.md)
- `iam-permissions.network-access-analyzer` — Network Access Analyzer  (Q001-Q050.md)
- `iam-permissions.abac` — 속성 기반 액세스 제어(ABAC)  (Q001-Q050.md)
- `iam-permissions.permissions-boundary` — 권한 경계  (Q301-Q350.md)
- `iam-permissions.cross-account-iam-role` — 계정 간 IAM 역할과 신뢰 정책  (Q101-Q150.md)
- `iam-permissions.iam-user-is-account-scoped` — IAM 사용자는 계정 밖으로 나가지 않는다  (Q851-Q880.md)
- `iam-permissions.iam-explicit-deny-precedence` — 명시적 거부가 허용을 이긴다  (Q451-Q500.md)
- `iam-permissions.iam-notaction-deny` — NotAction을 쓴 Deny로 한 서비스만 남긴다  (Q501-Q550.md)
- `iam-permissions.iam-requested-region-condition` — 요청이 향한 리전을 조건으로 건다  (Q751-Q800.md)
- `iam-permissions.access-analyzer-delegated-administrator` — 조직 전체 결과는 위임 관리자 계정에 모인다  (Q601-Q650.md)
- `iam-permissions.root-user-multiple-mfa` — 루트 사용자에게 MFA 장치를 여러 개 붙인다  (Q351-Q400.md)
- `iam-permissions.root-user-cannot-be-disabled` — 루트 사용자는 없앨 수 없다  (Q601-Q650.md)
- `identity-federation.aws-directory-service` — AWS Directory Service와 AD Connector  (Q051-Q100.md)
- `identity-federation.identity-center-external-idp` — IAM Identity Center와 외부 IdP의 연결  (Q101-Q150.md)
- `identity-federation.cognito-social-idp-federation` — Cognito 사용자 풀과 소셜 로그인 연동  (Q301-Q350.md)
- `identity-federation.custom-identity-broker-for-non-saml` — SAML을 못 쓰는 디렉터리는 브로커로 잇는다  (Q701-Q750.md)
- `identity-federation.identity-center-permission-set` — 권한 세트  (Q101-Q150.md)
- `identity-federation.saml-federation-role-to-ad-group-mapping` — 사내 디렉터리를 SAML 2.0으로 붙이고 역할을 그룹에 맵핑한다  (Q801-Q850.md)

목록을 다시 뽑으려면:

```bash
node scripts/coverage.mjs iam-permissions identity-federation
```
...
node scripts/coverage.mjs iam-permissions identity-federation  # exit 0 — 담당 주제 커버리지 100%
```
