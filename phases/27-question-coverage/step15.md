# Step 15: questions-route53-analytics — 문항 23개 이상
...
|---|---|---|---|---|
| `route53` | Route 53 | 2 | 13 | **8** |
| `emr-glue-athena` | EMR·Spark·Glue·Athena·Lake Formation | 2 | 20 | **15** |

**23개 개념에 문항이 없다. 이 step이 전부 덮는다.**

담당 개념 목록(괄호는 덤프 근거가 있는 파일):

- `route53.route53-zone-file-import` — DNS 호스팅을 Route 53으로 옮기는 절차  (Q051-Q100.md)
- `route53.route53-failover-routing` — 페일오버 라우팅 정책  (Q451-Q500.md)
- `route53.multi-region-failover-for-region-outage` — 리전 장애는 가용 영역으로 막지 못한다  (Q451-Q500.md)
- `route53.latency-record-for-non-aws-endpoint` — 온프레미스 엔드포인트에도 지연 시간 레코드를 만든다  (Q251-Q300.md)
- `route53.route53-alias-record` — 별칭 레코드  (Q301-Q350.md)
- `route53.private-hosted-zone-vpc-only` — 프라이빗 호스팅 영역은 VPC에만 연결한다  (Q701-Q750.md)
- `route53.route53-resolver-forward-rule` — 전달 규칙과 VPC 연결  (Q051-Q100.md)
- `route53.route53-query-logging` — Route 53 쿼리 로깅  (Q101-Q150.md)
- `emr-glue-athena.emr-node-types` — EMR 클러스터의 세 가지 노드  (Q001-Q050.md)
- `emr-glue-athena.glue-databrew` — AWS Glue DataBrew  (Q551-Q600.md)
- `emr-glue-athena.lake-formation` — AWS Lake Formation  (Q001-Q050.md)
- `emr-glue-athena.emr-transient-cluster` — 일시적 클러스터와 장기 실행 클러스터  (Q001-Q050.md)
- `emr-glue-athena.emr-managed-scaling` — EMR 관리형 스케일링  (Q201-Q250.md)
- `emr-glue-athena.glue-etl-with-per-customer-kms-key` — 고객마다 키가 다른 변환 작업은 관리형 ETL로  (Q851-Q880.md)
- `emr-glue-athena.athena-encrypted-and-pay-per-query` — Athena의 과금 단위와 암호화된 데이터  (Q251-Q300.md)
- `emr-glue-athena.athena-federated-query` — Athena 페더레이션 쿼리  (Q551-Q600.md)
- `emr-glue-athena.log-storage-s3-athena` — 가끔 조회하는 로그는 S3에 쌓고 Athena로 읽는다  (Q251-Q300.md)
- `emr-glue-athena.lake-formation-blueprint-and-athena` — 블루프린트로 모으고 Athena로 권한을 적용한다  (Q251-Q300.md)
- `emr-glue-athena.lake-formation-lf-tags` — LF 태그로 테이블과 열 권한을 준다  (Q701-Q750.md)
- `emr-glue-athena.emr-node-instance-family-choice` — 노드 역할에 따라 인스턴스 제품군이 갈린다  (Q851-Q880.md)
- `emr-glue-athena.emr-runtime-role` — EMR 런타임 역할  (Q301-Q350.md)
- `emr-glue-athena.emr-security-configuration` — EMR 보안 구성  (Q751-Q800.md)
- `emr-glue-athena.parquet-columnar-format` — Apache Parquet  (Q001-Q050.md)

목록을 다시 뽑으려면:

```bash
node scripts/coverage.mjs route53 emr-glue-athena
```
...
node scripts/coverage.mjs route53 emr-glue-athena  # exit 0 — 담당 주제 커버리지 100%
```
