# Step 16: questions-streaming-monitoring — 문항 23개 이상
...
|---|---|---|---|---|
| `kinesis-streaming` | Kinesis Data Streams·Data Firehose·Flink·Video Streams·MSK | 2 | 16 | **14** |
| `cloudwatch-xray` | CloudWatch·X-Ray·Performance Insights·Managed Grafana | 2 | 11 | **9** |

**23개 개념에 문항이 없다. 이 step이 전부 덮는다.**

담당 개념 목록(괄호는 덤프 근거가 있는 파일):

- `kinesis-streaming.kinesis-data-streams` — Kinesis Data Streams  (원본 concepts-raw.md 계열 — dump-gaps에 항목이 없다)
- `kinesis-streaming.data-firehose` — Data Firehose  (원본 concepts-raw.md 계열 — dump-gaps에 항목이 없다)
- `kinesis-streaming.managed-service-apache-flink` — Managed Service for Apache Flink  (원본 concepts-raw.md 계열 — dump-gaps에 항목이 없다)
- `kinesis-streaming.kinesis-video-streams` — Amazon Kinesis Video Streams  (Q301-Q350.md)
- `kinesis-streaming.flink-kinesis-source-sink` — Flink는 스트림 사이에 끼워 넣는다  (Q101-Q150.md)
- `kinesis-streaming.firehose-lambda-transformation` — Firehose가 적재 전에 Lambda로 데이터를 고친다  (Q301-Q350.md)
- `kinesis-streaming.firehose-format-conversion` — Firehose가 적재하면서 형식을 바꾼다  (Q501-Q550.md)
- `kinesis-streaming.kinesis-retention-and-fanout` — 보존 기간과 향상된 팬아웃  (Q151-Q200.md)
- `kinesis-streaming.kinesis-client-library` — 샤드와 체크포인트를 직접 다루는 소비자  (Q701-Q750.md)
- `kinesis-streaming.msk-kafka-connect` — MSK가 수집과 변환까지 맡는 방식  (Q151-Q200.md)
- `kinesis-streaming.kinesis-record-size-limit` — 스트림에 넣을 수 있는 레코드 크기  (Q201-Q250.md)
- `kinesis-streaming.kinesis-partition-key-hot-shard` — 파티션 키가 쏠리면 핫 샤드가 생긴다  (Q051-Q100.md)
- `kinesis-streaming.kinesis-capacity-mode` — Kinesis 스트림의 프로비저닝 모드와 온디맨드 모드  (Q051-Q100.md)
- `kinesis-streaming.firehose-buffering` — Firehose는 모아서 보낸다  (Q151-Q200.md)
- `cloudwatch-xray.x-ray` — X-Ray  (원본 concepts-raw.md 계열 — dump-gaps에 항목이 없다)
- `cloudwatch-xray.performance-insight` — Performance Insight  (원본 concepts-raw.md 계열 — dump-gaps에 항목이 없다)
- `cloudwatch-xray.amazon-managed-grafana` — Amazon Managed Grafana  (Q401-Q450.md)
- `cloudwatch-xray.cloudwatch-network-monitor` — CloudWatch Network Monitor  (Q101-Q150.md)
- `cloudwatch-xray.cloudwatch-container-insights` — CloudWatch Container Insights  (Q151-Q200.md)
- `cloudwatch-xray.performance-insights-rightsizing` — Performance Insights와 적정 규모 조정  (Q151-Q200.md)
- `cloudwatch-xray.cloudwatch-agent-memory-metric` — 메모리 사용률은 기본 지표가 아니다  (Q101-Q150.md)
- `cloudwatch-xray.ec2-detailed-monitoring` — 상세 모니터링은 1분 간격이다  (Q251-Q300.md)
- `cloudwatch-xray.cloudwatch-alarm-state-change-event` — 알람 상태 변경은 이벤트로 흘러간다  (Q551-Q600.md)

목록을 다시 뽑으려면:

```bash
node scripts/coverage.mjs kinesis-streaming cloudwatch-xray
```
...
node scripts/coverage.mjs kinesis-streaming cloudwatch-xray  # exit 0 — 담당 주제 커버리지 100%
```
