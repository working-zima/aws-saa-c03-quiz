# Step 11: questions-containers — 문항 19개 이상
...
|---|---|---|---|---|
| `ecs-eks-fargate` | ECS·EKS·Fargate·Batch·ECR·Elastic Beanstalk | 3 | 23 | **19** |

**19개 개념에 문항이 없다. 이 step이 전부 덮는다.**

담당 개념 목록(괄호는 덤프 근거가 있는 파일):

- `ecs-eks-fargate.ecr-image-scan-on-push` — Amazon ECR과 푸시 시 스캔  (Q651-Q700.md)
- `ecs-eks-fargate.elastic-beanstalk` — AWS Elastic Beanstalk  (Q351-Q400.md)
- `ecs-eks-fargate.app2container` — AWS App2Container  (Q651-Q700.md)
- `ecs-eks-fargate.eks-compute-options` — EKS의 컴퓨팅은 세 갈래이고 관리 책임이 다르다  (Q651-Q700.md)
- `ecs-eks-fargate.fargate-spot` — Fargate Spot  (Q301-Q350.md)
- `ecs-eks-fargate.batch-fargate-compute-environment` — AWS Batch의 컴퓨팅 환경  (Q051-Q100.md)
- `ecs-eks-fargate.eks-fargate-pod-isolation` — Fargate는 파드마다 환경을 격리한다  (Q501-Q550.md)
- `ecs-eks-fargate.eks-cluster-autoscaler` — 파드를 늘리는 것과 노드를 늘리는 것은 다르다  (Q301-Q350.md)
- `ecs-eks-fargate.eks-aws-load-balancer-controller` — AWS Load Balancer Controller  (Q451-Q500.md)
- `ecs-eks-fargate.eks-connector` — Amazon EKS Connector  (Q151-Q200.md)
- `ecs-eks-fargate.eks-anywhere` — Amazon EKS Anywhere  (Q151-Q200.md)
- `ecs-eks-fargate.ecs-task-role` — 태스크 역할과 인스턴스 역할  (Q151-Q200.md)
- `ecs-eks-fargate.ecs-task-role-vs-task-execution-role` — 태스크 역할과 태스크 실행 역할은 다른 자리다  (Q701-Q750.md)
- `ecs-eks-fargate.eks-irsa` — 서비스 계정용 IAM 역할(IRSA)  (Q401-Q450.md)
- `ecs-eks-fargate.ecs-awsvpc-mode` — ECS의 awsvpc 네트워크 모드  (Q051-Q100.md)
- `ecs-eks-fargate.ecs-task-placement-strategy` — ECS 태스크 배치 전략  (Q351-Q400.md)
- `ecs-eks-fargate.fargate-per-second-billing` — Fargate의 최소 과금 단위  (Q101-Q150.md)
- `ecs-eks-fargate.fargate-efs-mount` — Fargate 태스크에 EFS를 마운트한다  (Q051-Q100.md)
- `ecs-eks-fargate.eks-secrets-kms-encryption` — EKS 시크릿의 KMS 암호화  (Q351-Q400.md)

목록을 다시 뽑으려면:

```bash
node scripts/coverage.mjs ecs-eks-fargate
```
...
node scripts/coverage.mjs ecs-eks-fargate  # exit 0 — 담당 주제 커버리지 100%
```
