...
## 먼저 읽어라 — 다른 기기에서 이어받는 법 (2026-09-08 세션 중단)

**브랜치는 `feat-29-content-quality-pilot`이고 origin에 push돼 있다.** `develop`은 이 작업을
담고 있지 않고 `main`도 그대로다(배포된 사이트는 아직 이 변경이 없다).
...
### 다음에 할 것 — phase 30, 명세가 이미 다 쓰여 있다

사용자 요청은 **"AWS 퀴즈 문장을 전체적으로 다시 검토해달라 — 의미만 전달되는 번역체가
아니라 한국어로 자연스럽게 읽히는 문제 문장으로"**다. 기준 여섯과 다시 쓴 예시 둘을 직접
주었고, 그 내용이 `phases/30-prompt-korean-polish/`의 **step 문서 29개 전부에 같은 형태로**
들어 있다(각 step은 독립 세션에서 실행되므로 자기완결로 썼다).

```bash
python scripts/execute.py 30-prompt-korean-polish --max-steps 2
```

**`--max-steps N`은 총 step 수가 아니라 이번 실행에서 돌릴 step 수다.** 이 세션에서 한 번
헷갈려 phase 하나가 끝까지 돌았다.

**처음 2개 배치만 돌리고 멈춰 사용자에게 문체를 확인받는 것을 권한다.** 합격 기준이
"한국어로 자연스럽게 읽히는가"라 사람만 판정할 수 있고, 기준이 틀렸을 때 되돌릴 범위가 작다.

배치는 주제를 쪼개지 않고 문항 34개를 넘지 않게 끊어 29개다. 착수 시점 실측은 이렇다.

| 지표 | 값 |
|---|---|
| 프롬프트가 체언으로 끊긴 것(`…서비스는?`·`…구성은?`·`…것은?`) | 616 / 732 (84%) |
| 명시적 물음(`무엇인가?`·`해야 하는가?`)으로 닫은 것 | 116 (16%) |
| 한 문장짜리 — 상황 설명 없이 조건을 몰아넣음 | 225 |
| 사용자가 제안한 `~하는 상황이다` 형태가 이미 쓰인 곳 | 0 |
| 끝맺음 형태 종류 | 163종 — 문체가 통일돼 있지 않다 |

phase 30의 가장 큰 실패 방식은 **문장을 자연스럽게 만들려다 정답을 가르는 조건을 흘리는
것**이다. step 문서마다 「절대 바꾸지 말 것」으로 박아 두었고, 검증 절차에
`git diff`로 전후 대조하라고 적었다.
