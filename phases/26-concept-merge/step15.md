# Step 15: merge-analytics — 분석·스트리밍
...
이 step은 `analytics-monitoring` 주제 중 **분석 · 스트리밍 계열**(EMR·Redshift·Athena·Glue·Kinesis 등) 를 맡는다.

| 대상 | 기존 개념 | 신규 개념 |
|---|---|---|
| `analytics-monitoring 중 분석·스트리밍` | 9 | 28 |
...
```bash
python3 scripts/collect-gaps.py analytics-monitoring
```

## 이 step의 특수 사정

`analytics-monitoring`을 step 16과 나눠 맡는다. **이 step은 분석·스트리밍 계열만 새 주제로 빼내고 CloudWatch·X-Ray 계열은 남긴다.** 주제를 지우지 마라 — step 16이 마저 비우고 없앤다.

## 헷갈리는 짝 — 이 step에서 특히 지킬 것

**Kinesis 4종(Data Streams·Data Firehose·Data Analytics·Video Streams)은 한 주제에 둔다.** Athena ↔ Redshift Spectrum, EMR ↔ Glue도 갈림길이다.
...
3. 결과에 따라 `phases/26-concept-merge/index.json`의 step 15을 업데이트한다:
