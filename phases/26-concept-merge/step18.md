# Step 18: merge-security — 암호화·위협 보호
...
이 step은 `secrets-encryption` · `threat-protection` 두 주제 를 맡는다.

| 대상 | 기존 개념 | 신규 개념 |
|---|---|---|
| `secrets-encryption` | 9 | 11 |
| `threat-protection` | 11 | 15 |
...
```bash
python3 scripts/collect-gaps.py secrets-encryption
python3 scripts/collect-gaps.py threat-protection
```

## 헷갈리는 짝 — 이 step에서 특히 지킬 것

**KMS ↔ Secrets Manager ↔ Parameter Store는 반드시 한 주제에 둔다** — 무엇을 어디에 두는가가 문항이다. **WAF ↔ Shield ↔ Shield Advanced**도 한 벌이고, **GuardDuty ↔ Macie ↔ Inspector ↔ Detective**도 서로 갈림길이다.
...
3. 결과에 따라 `phases/26-concept-merge/index.json`의 step 18을 업데이트한다:
