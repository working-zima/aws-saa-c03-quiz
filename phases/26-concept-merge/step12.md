# Step 12: merge-messaging — SQS·SNS·EventBridge
...
이 step은 `messaging-backup` 주제 중 **SQS · SNS · EventBridge 계열** 를 맡는다.

| 대상 | 기존 개념 | 신규 개념 |
|---|---|---|
| `messaging-backup 중 메시징` | 6 | 20 |
...
`messaging-backup`을 step 13과 나눠 맡는다. **이 step은 메시징 계열만 새 주제로 빼내고 AWS Backup 계열은 남긴다.** 주제를 지우지 마라 — step 13이 마저 비우고 없앤다.

## 헷갈리는 짝 — 이 step에서 특히 지킬 것

**SQS ↔ SNS ↔ EventBridge는 반드시 한 주제 안에 둔다.** 메시징 세 갈래의 선택이 문항의 핵심이다. 표준 큐 ↔ FIFO 큐도 갈림길이다.
...
3. 결과에 따라 `phases/26-concept-merge/index.json`의 step 12을 업데이트한다:
