# Step 3: merge-s3-access — S3 암호화·접근 제어·운영
...
이 step은 `s3-encryption-batch` 주제와, 주제 미정 41개 중 **S3 접근 제어·운영 계열** 를 맡는다.

| 대상 | 기존 개념 | 신규 개념 |
|---|---|---|
| `s3-encryption-batch` | 5 | 8 |
| `(주제 미정) 중 S3 계열` | 0 | 13 |
...
```bash
python3 scripts/collect-gaps.py s3-encryption-batch
python3 scripts/collect-gaps.py '(주제 미정)'
```

## 이 step의 특수 사정

`(주제 미정)`에서 **S3 계열만** 가져간다. 거버넌스·SSM·AI·DR 계열은 step 13·19·20의 몫이다. 네가 가져간 slug를 `summary`에 **전부 나열해라** — 뒤 step이 중복을 피하는 유일한 수단이다.

## 헷갈리는 짝 — 이 step에서 특히 지킬 것

SSE-S3 ↔ SSE-KMS ↔ SSE-C 는 한 주제에 둔다. 버킷 정책·액세스 포인트·퍼블릭 액세스 차단은 서로 갈림길이므로 흩지 마라.
...
3. 결과에 따라 `phases/26-concept-merge/index.json`의 step 3을 업데이트한다:
