# Step 13: merge-backup-dr — 백업·재해 복구
...
이 step은 `messaging-backup` 주제의 나머지(**AWS Backup 계열**)와, 주제 미정 41개 중 **재해 복구 계열** 를 맡는다.

| 대상 | 기존 개념 | 신규 개념 |
|---|---|---|
| `messaging-backup 중 백업` | 5 | 13 |
| `(주제 미정) 중 DR 계열` | 0 | 4 |
...
```bash
python3 scripts/collect-gaps.py messaging-backup
python3 scripts/collect-gaps.py '(주제 미정)'
```

## 이 step의 특수 사정

step 12가 메시징을 빼내고 남긴 `messaging-backup`을 마저 처리한다. **이 step이 끝나면 `messaging-backup` 주제는 없어진다.** `(주제 미정)`에서는 **재해 복구 계열만** 가져간다. 네가 가져간 slug를 `summary`에 전부 나열해라.

## 헷갈리는 짝 — 이 step에서 특히 지킬 것

DR 전략 네 가지(백업 및 복원 ↔ 파일럿 라이트 ↔ 웜 스탠바이 ↔ 다중 사이트)는 RTO·RPO로 갈리는 한 벌이다. **반드시 한 주제에 둔다.** 흩으면 "짧은 RTO는 무엇을 고르는가"를 배울 수 없다.
...
3. 결과에 따라 `phases/26-concept-merge/index.json`의 step 13을 업데이트한다:
