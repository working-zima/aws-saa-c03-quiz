# Step 17: merge-identity — IAM·자격 증명
...
이 step은 `identity-access` 주제 를 맡는다.

| 대상 | 기존 개념 | 신규 개념 |
|---|---|---|
| `identity-access` | 12 | 32 |
...
```bash
python3 scripts/collect-gaps.py identity-access
```

## 헷갈리는 짝 — 이 step에서 특히 지킬 것

**IAM 역할 ↔ 사용자 ↔ 정책 ↔ 권한 경계는 한 주제에 둔다.** Identity Center ↔ SAML 페더레이션 ↔ Cognito도 갈림길이다. CloudTrail은 감사 쪽이라 쪼개도 되지만, 쪼갠다면 배열에서 인접시켜라.
...
3. 결과에 따라 `phases/26-concept-merge/index.json`의 step 17을 업데이트한다:
