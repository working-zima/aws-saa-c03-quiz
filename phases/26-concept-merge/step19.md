# Step 19: merge-cost-governance — 비용 관리·거버넌스·운영
...
이 step은 `cost-management` 주제와, 주제 미정 41개 중 **거버넌스·IaC 계열**(CloudFormation·Service Catalog·Control Tower·Config·RAM)과 **Systems Manager 운영 계열** 를 맡는다.

| 대상 | 기존 개념 | 신규 개념 |
|---|---|---|
| `cost-management` | 9 | 8 |
| `(주제 미정) 중 거버넌스·IaC` | 0 | 7 |
| `(주제 미정) 중 SSM 운영` | 0 | 6 |
...
```bash
python3 scripts/collect-gaps.py cost-management
python3 scripts/collect-gaps.py '(주제 미정)'
```

## 이 step의 특수 사정

`(주제 미정)`에서 **거버넌스·IaC 계열과 Systems Manager 계열만** 가져간다. S3 계열은 step 3이, DR 계열은 step 13이 이미 가져갔다. AI·ML 계열은 step 20의 몫이다. 네가 가져간 slug를 `summary`에 전부 나열해라.

## 헷갈리는 짝 — 이 step에서 특히 지킬 것

**Cost Explorer ↔ Budgets ↔ Cost Anomaly Detection**은 한 벌이다. **CloudFormation ↔ Service Catalog ↔ Control Tower**도 거버넌스 갈림길이므로 함께 둔다. Systems Manager 기능들(Session Manager·Patch Manager·Run Command·Inventory)도 한 주제다.
...
3. 결과에 따라 `phases/26-concept-merge/index.json`의 step 19을 업데이트한다:
