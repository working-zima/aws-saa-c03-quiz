# Step 7: merge-aurora-dynamodb — Aurora·DynamoDB·캐시
...
이 step은 `aurora-dynamodb-cache` 주제 를 맡는다.

| 대상 | 기존 개념 | 신규 개념 |
|---|---|---|
| `aurora-dynamodb-cache` | 8 | 42 |
...
```bash
python3 scripts/collect-gaps.py aurora-dynamodb-cache
```

## 헷갈리는 짝 — 이 step에서 특히 지킬 것

**Aurora ↔ DynamoDB ↔ ElastiCache는 성격이 달라 쪼개도 된다.** 다만 ElastiCache(Redis·Memcached) ↔ DAX는 캐시 갈림길이므로 함께 둔다. step 6의 RDS 주제와 배열에서 인접해야 한다.
...
3. 결과에 따라 `phases/26-concept-merge/index.json`의 step 7을 업데이트한다:
