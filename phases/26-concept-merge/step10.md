...
이 step은 `serverless-containers` 주제 중 **Lambda 계열** 를 맡는다.

| 대상 | 기존 개념 | 신규 개념 |
|---|---|---|
| `serverless-containers 중 Lambda` | 2 | 22 |
...
`serverless-containers`를 step 11과 나눠 맡는다. **이 step은 Lambda 계열만 새 주제로 빼내고 나머지는 `serverless-containers`에 남긴다.** 주제를 지우지 마라 — step 11이 마저 비우고 없앤다.

## 헷갈리는 짝 — 이 step에서 특히 지킬 것

동시성 세 갈래(예약 동시성 ↔ 프로비저닝된 동시성 ↔ 계정 한도)는 서로 갈림길이므로 함께 둔다. SnapStart·레이어·메모리 한계 같은 것은 3단(한계)이다.
...
3. 결과에 따라 `phases/26-concept-merge/index.json`의 step 10을 업데이트한다:
