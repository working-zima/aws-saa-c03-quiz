# Step 4: merge-block-file — 블록·파일 스토리지
...
이 step은 `block-file-storage` 주제 를 맡는다.

| 대상 | 기존 개념 | 신규 개념 |
|---|---|---|
| `block-file-storage` | 8 | 32 |
...
**EFS ↔ FSx(Lustre·Windows·ONTAP)는 반드시 한 주제 안에 둔다.** 공유 파일 스토리지 선택이 문항의 핵심이다. EBS 볼륨 유형(gp2·gp3·io1·io2)도 서로 갈림길이므로 함께 둔다.
...
3. 결과에 따라 `phases/26-concept-merge/index.json`의 step 4을 업데이트한다:
