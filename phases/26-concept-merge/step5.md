# Step 5: merge-data-transfer — 데이터 전송·마이그레이션
...
이 step은 `data-transfer-services` 주제 를 맡는다.

| 대상 | 기존 개념 | 신규 개념 |
|---|---|---|
| `data-transfer-services` | 4 | 22 |
...
```bash
python3 scripts/collect-gaps.py data-transfer-services
```

## 헷갈리는 짝 — 이 step에서 특히 지킬 것

DataSync ↔ Snowball ↔ Transfer Family ↔ Storage Gateway는 "언제 무엇을 쓰는가"가 그대로 문항이다. 한 주제에 두거나, 쪼갠다면 갈림길 개념이 양쪽에서 다 보이게 하라.
...
3. 결과에 따라 `phases/26-concept-merge/index.json`의 step 5을 업데이트한다:
