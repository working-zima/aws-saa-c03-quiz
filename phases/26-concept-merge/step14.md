# Step 14: merge-vpc-hybrid — VPC·보안 그룹·하이브리드 연결
...
이 step은 `vpc-networking` · `security-groups-nacl` · `hybrid-connectivity` 세 주제 를 맡는다.

| 대상 | 기존 개념 | 신규 개념 |
|---|---|---|
| `vpc-networking` | 12 | 8 |
| `security-groups-nacl` | 5 | 4 |
| `hybrid-connectivity` | 10 | 9 |
...
```bash
python3 scripts/collect-gaps.py vpc-networking
python3 scripts/collect-gaps.py security-groups-nacl
python3 scripts/collect-gaps.py hybrid-connectivity
```

## 헷갈리는 짝 — 이 step에서 특히 지킬 것

**보안 그룹 ↔ NACL은 반드시 함께 둔다** — 상태 저장 여부가 대표 문항이다. **Site-to-Site VPN ↔ Direct Connect ↔ Transit Gateway**도 한 벌이다. VPC 엔드포인트(게이트웨이 ↔ 인터페이스) ↔ PrivateLink도 갈림길이다.
...
3. 결과에 따라 `phases/26-concept-merge/index.json`의 step 14을 업데이트한다:
