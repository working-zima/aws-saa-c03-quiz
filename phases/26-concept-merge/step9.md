# Step 9: merge-cloudfront-edge — CloudFront·Global Accelerator
...
이 step은 `compute-delivery` 주제의 나머지 — **CloudFront · Global Accelerator · 엣지 계열** 를 맡는다.

| 대상 | 기존 개념 | 신규 개념 |
|---|---|---|
| `compute-delivery 중 CloudFront·GA` | 2 | 16 |
...
```bash
python3 scripts/collect-gaps.py compute-delivery
```

## 이 step의 특수 사정

step 8이 EC2·ASG·ELB를 빼내고 남긴 `compute-delivery`를 마저 처리한다. **이 step이 끝나면 `compute-delivery` 주제는 없어지고**, 새 주제가 step 8의 주제 바로 뒤에 놓인다. Lambda@Edge와 CloudFront Functions가 `serverless-containers` 쪽으로 분류된 것이 있으면 `topic-plan.md`가 정한 자리를 따르되, 엣지 실행이라는 갈림길이 흩어지지 않게 하라.

## 헷갈리는 짝 — 이 step에서 특히 지킬 것

**CloudFront ↔ Global Accelerator는 절대 갈라 놓지 마라.** SAA-C03의 대표 혼동 짝이고, `data.test.ts`에 「CloudFront와 Global Accelerator의 갈림길이 추상적인 대비 대신 구체적인 기준으로 쓰인다」 테스트가 이미 있다. 이 둘을 다른 주제에 두면 이 phase의 설계 규칙을 정면으로 어기는 것이다.
...
3. 결과에 따라 `phases/26-concept-merge/index.json`의 step 9을 업데이트한다:
