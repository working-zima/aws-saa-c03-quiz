# Step 20: merge-ai-ml — AI·ML과 잔여 개념
...
이 step은 주제 미정 41개 중 **AI·ML 계열**(SageMaker·Comprehend·Rekognition·Lex 등)과, step 3·13·19가 가져가지 않고 남은 **잔여 개념 전부** 를 맡는다.

| 대상 | 기존 개념 | 신규 개념 |
|---|---|---|
| `(주제 미정) 중 AI·ML과 잔여` | 0 | 15 |
...
**이 step이 `(주제 미정)` 41개를 남김없이 비운다.** step 3·13·19의 `summary`에 각자 가져간 slug가 나열돼 있다. 그것을 빼고 남은 전부가 이 step 몫이다. **하나도 남기지 마라** — 아래 AC가 436개 전부가 `topics.json`에 들어갔는지 센다. 어느 주제에도 어울리지 않는 개념이 있으면 억지로 끼우지 말고 성격이 가까운 주제 끝에 3단(한계)으로 넣어라.

## 헷갈리는 짝 — 이 step에서 특히 지킬 것

음성·이미지·번역·문서를 나눠 맡는 AI 서비스들은 "무엇을 하는 서비스인가"가 그대로 문항이므로 한 주제에 둔다.
...
3. 결과에 따라 `phases/26-concept-merge/index.json`의 step 20을 업데이트한다:
