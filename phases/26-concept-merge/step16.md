# Step 16: merge-monitoring-dns — 모니터링·Route 53
...
이 step은 `analytics-monitoring` 주제의 나머지(**CloudWatch · X-Ray 계열**)와 `route53` 주제 를 맡는다.

| 대상 | 기존 개념 | 신규 개념 |
|---|---|---|
| `analytics-monitoring 중 모니터링` | 5 | 14 |
| `route53` | 5 | 8 |
...
python3 scripts/collect-gaps.py route53
```

## 이 step의 특수 사정

step 15가 분석·스트리밍을 빼내고 남긴 `analytics-monitoring`을 마저 처리한다. **이 step이 끝나면 `analytics-monitoring` 주제는 없어진다.**

## 헷갈리는 짝 — 이 step에서 특히 지킬 것

**Route 53 라우팅 정책들(가중치·지연 시간·장애 조치·지리 위치 등)은 한 주제에 둔다.** CloudWatch 지표 ↔ 로그 ↔ 경보도 한 벌이다.
...
3. 결과에 따라 `phases/26-concept-merge/index.json`의 step 16을 업데이트한다:
