# Step 2: merge-core-s3 — 기초·S3 스토리지 클래스
...
이 step은 `aws-core-services` · `s3-storage-classes` · `s3-versioning-lifecycle` 세 주제 를 맡는다.

| 대상 | 기존 개념 | 신규 개념 |
|---|---|---|
| `aws-core-services` | 16 | 0 |
| `s3-storage-classes` | 9 | 8 |
| `s3-versioning-lifecycle` | 5 | 5 |
...
```bash
python3 scripts/collect-gaps.py aws-core-services
python3 scripts/collect-gaps.py s3-storage-classes
python3 scripts/collect-gaps.py s3-versioning-lifecycle
```

## 헷갈리는 짝 — 이 step에서 특히 지킬 것

S3 스토리지 클래스 7종(Standard·IA·One Zone-IA·Intelligent-Tiering·Glacier 3종)은 **한 주제에 함께 둔다.** 지금도 그렇고 `data.test.ts`에 「S3 스토리지 클래스 주제가 클래스별 개념 7개와 정리 개념 2개로 나뉜다」 테스트가 있다.
...
3. 결과에 따라 `phases/26-concept-merge/index.json`의 step 2을 업데이트한다:
