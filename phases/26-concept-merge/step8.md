# Step 8: merge-ec2-elb — EC2·Auto Scaling·로드 밸런서
...
이 step은 `compute-delivery` 주제 중 **EC2 · Auto Scaling · ELB 계열** 를 맡는다.

| 대상 | 기존 개념 | 신규 개념 |
|---|---|---|
| `compute-delivery 중 EC2·ASG·ELB` | 2 | 27 |
...
`compute-delivery`를 step 9와 나눠 맡는다. **이 step은 EC2·ASG·ELB만 새 주제로 빼내고, CloudFront·Global Accelerator 개념은 `compute-delivery`에 그대로 남긴다.** 주제를 지우지 마라 — step 9가 마저 비우고 없앤다. 빈 주제가 생기면 `모든 주제는 하나 이상의 개념을 가진다` 테스트가 깨진다.

## 헷갈리는 짝 — 이 step에서 특히 지킬 것

**ALB ↔ NLB ↔ Gateway Load Balancer는 반드시 한 주제 안에 둔다.** 세 로드 밸런서의 선택 기준이 문항의 핵심이다. 스팟 ↔ 온디맨드 ↔ 예약 인스턴스도 갈림길이므로 함께 둔다.
...
3. 결과에 따라 `phases/26-concept-merge/index.json`의 step 8을 업데이트한다:
