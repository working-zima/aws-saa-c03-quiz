# Step 11: merge-containers-apigw — 컨테이너·API Gateway·Step Functions
...
이 step은 `serverless-containers` 주제의 나머지 — **ECS · EKS · Fargate · API Gateway · Step Functions 계열** 를 맡는다.

| 대상 | 기존 개념 | 신규 개념 |
|---|---|---|
| `serverless-containers 중 나머지` | 9 | 29 |
...
```bash
python3 scripts/collect-gaps.py serverless-containers
```

## 이 step의 특수 사정

step 10이 Lambda를 빼내고 남긴 `serverless-containers`를 마저 처리한다. **이 step이 끝나면 `serverless-containers` 주제는 없어지고**, 새 주제가 step 10의 주제 바로 뒤에 놓인다.

## 헷갈리는 짝 — 이 step에서 특히 지킬 것

**ECS ↔ EKS ↔ Fargate는 한 주제에 둔다.** 컨테이너 실행 방식의 선택이 문항이다. API Gateway REST ↔ HTTP ↔ WebSocket도 갈림길이므로 함께 둔다. Step Functions 표준 ↔ Express도 마찬가지다.
...
3. 결과에 따라 `phases/26-concept-merge/index.json`의 step 11을 업데이트한다:
