...
무엇이고 왜 쓰는지가 잡히게 한다. 이 step은 그중 전송·서버리스 4문항을 맡는다.

`step0`·`step1`이 먼저 실행되었다면 `q171`~`q177`, `q179`~`q188` 일부의 프롬프트가 이미
길어져 있다. **그 문항들을 다시 건드리지 마라.** 이 step의 대상과 겹치지 않는다.
...
| 문항 | 상황 문장이 쓰는 사실 | 근거 |
|---|---|---|
| q192 | 실시간 게임 서버처럼 HTTP가 아닌 트래픽을 가속하고, 네트워크 상태에 따라 정상 리전으로 보낸다 | `topics.json` `compute-delivery.global-accelerator-protocols` 문단 0 — "실시간 게임 서버나 TCP 기반 영상 전송처럼 HTTP가 아닌 트래픽을 가속할 때 쓴다", "네트워크 상태에 따라 트래픽을 정상 리전으로 라우팅한다" |
| q197 | 관리자가 필요할 때 눌러 보고서를 만드는 정도의 단순한 호출이면 API Gateway가 과하다 | `exam-gaps.md` `lambda-function-url` — "관리자가 필요할 때 눌러서 보고서를 만드는 정도의 단순한 호출이면 API Gateway를 앞에 두는 것보다 구성이 간단하고 비용이 적게 든다" |
| q198 | 권한 부여 로직을 사용자와 가장 가까운 곳에서 처리하면 로그인 지연까지 준다 | `exam-gaps.md` `lambda-at-edge` — "인증·권한 부여 로직을 사용자와 가장 가까운 곳에서 처리하므로 로그인 과정의 지연까지 줄일 수 있다" |
| q200 | JWT로 인증하는 퍼블릭 API를 ALB와 직접 통합해야 하고, REST API보다 비용·지연이 낮다 | `exam-gaps.md` `api-gateway-jwt-authorizer` — "JWT로 사용자를 인증하는 퍼블릭 API를 ALB와 직접 통합해야 한다는 요구가 나오면 REST API가 아니라 HTTP API가 답이다. REST API로도 가능하지만 비용이 높고 설정이 복잡하다" |

**출처에 없어서 이 step이 쓰지 않는 것** — 넣으면 `blocked`다.

- `JWT`가 무엇의 약자인지, 무엇을 하는 형식인지 **풀이하지 마라.** ADR-010이 "`JWT`가
  무엇인지는 두 출처 어디에도 없다"고 명시하고 있고, 이 phase는 ADR-010의 13종 목록을
  늘리지 않는다. `q200`은 상황만 세우고 용어는 그대로 둔다.
- `q192`의 상황 문장에 **CloudFront와의 비교를 넣지 마라.** 정답 소거 근거다.
- `q198`의 상황 문장에 **"리전의 일반 Lambda보다 빠르다"를 넣지 마라.** 보기 `일반 Lambda`를
  지워주는 서술이다.
...
- 아래 문자열에는 따옴표·역슬래시가 없으므로 **JSON 이스케이프가 필요 없다.** 그대로 넣는다.
  `HTTP(S)`의 괄호도 이스케이프 대상이 아니다.
- 줄바꿈을 넣지 마라.

### 문항별 지금 / 바꿔

**q192** (`compute-delivery.global-accelerator-protocols`)

```
지금:
HTTP가 아닌 TCP·UDP 연결을 전 세계에서 가속하고 리전 장애 시 우회하는 서비스는?
```
```
바꿔:
실시간 게임 서버처럼 HTTP가 아닌 트래픽을 전 세계 사용자에게 빠르게 전달하고, 리전에 장애가 나면 정상 리전으로 넘겨야 한다. 적합한 서비스는?
```

**q197** (`serverless-containers.lambda-function-url`)

```
지금:
API Gateway 없이 Lambda 함수에 직접 HTTP(S) 엔드포인트를 제공하는 기능은?
```
```
바꿔:
관리자가 필요할 때 눌러 보고서를 만드는 정도의 단순한 호출이라 API Gateway를 앞에 두기가 과하다. Lambda 함수에 직접 HTTP(S) 주소를 붙이는 기능은?
```

**q198** (`serverless-containers.lambda-at-edge`)

```
지금:
CloudFront 엣지 로케이션에서 인증·권한 부여 코드를 실행하는 방식은?
```
```
바꿔:
로그인 과정의 지연까지 줄이려고 권한 부여 로직을 사용자와 가장 가까운 곳에서 처리하려 한다. CloudFront 엣지 로케이션에서 코드를 실행하는 방식은?
```

**q200** (`serverless-containers.api-gateway-jwt-authorizer`)

```
지금:
JWT 인증과 ALB 프라이빗 통합을 기본 지원하면서 비용과 지연이 낮은 API Gateway 유형은?
```
```
바꿔:
JWT로 사용자를 인증하는 퍼블릭 API를 ALB와 직접 통합해야 한다. 이 두 가지를 기본 지원하면서 REST API보다 비용과 지연이 낮은 API Gateway 유형은?
```
...
- **위에 적힌 4문항 외의 문항을 고치지 마라.** 나머지는 다른 step이 맡는다.
  특히 `q193`·`q195`·`q196`·`q199`는 이미 상황형이고, `q194`·`q201`은 용어의 뜻 자체를
  묻는 문항이라 대상이 아니다.
...
const TARGET=['q192','q197','q198','q200'];
...
- 검증 스크립트에서 **`prompt`가 바뀐 문항이 정확히 `q192` `q197` `q198` `q200`**이고,
  **`prompt` 외 필드 그대로 / id 순서 그대로 / 금지 표현 없음이 전부 `true`**다.
- 4문항 모두 길이 120자 이하이고 정답 문자열을 노출하지 않는다.
- `grep -c`가 246을 출력한다.
- 변경된 파일이 위 세 개뿐이다.
...
- **`JWT`를 풀이하지 마라.** 이유: 두 출처 어디에도 없고, ADR-010의 예외 목록을 늘리는
  것은 이 phase의 범위가 아니다.
