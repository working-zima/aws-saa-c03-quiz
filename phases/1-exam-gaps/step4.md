...
|---|---|---|
| `security-groups-nacl` | `security-group-referencing`, `nacl-rule-limit`, `web-acl-vs-nacl` | 3 |
| `secrets-encryption` | `acm-cloudfront-region`, `lambda-env-var-kms`, `cloudhsm`, `rotation-heuristic` | 4 |
| `threat-protection` | `waf-attach-targets`, `waf-bot-control`, `waf-rule-types`, `shield-advanced-drt`, `guardduty-db-login`, `security-service-lineup` | 6 |
| `identity-access` | `least-privilege`, `instance-profile`, `iam-group-users-only`, `sts-assume-role`, `cognito-pools`, `organizations-scp` | 6 |
| `cost-management` | `cost-allocation-tag-activation`, `savings-plan-details`, `cost-anomaly-detection`, `compute-optimizer` | 4 |

**합계 23개 concept.** slug는 `exam-gaps.md`의 `### `...`` 제목에 있는 값 그대로 쓴다.
...
- **문단 안의 줄바꿈은 이어 붙여 한 줄로 만들어라.** `exam-gaps.md`는 읽기 좋게 100자 근처에서
  줄을 접어 두었다. 그건 문서 서식일 뿐 문단 구분이 아니다. 앱은 `whitespace-pre-line`으로
  렌더링하므로 줄바꿈을 남기면 화면에서 문장 중간이 강제로 끊긴다.
  줄바꿈을 넣어도 되는 경우는 원본이 항목을 줄 단위로 나열할 때뿐이다.
...
- `topics.slice(15, 22).map((topic) => topic.concepts.length)`
  - 현재: `[5, 14, 2, 5, 5, 6, 5]`
  - 갱신 후: `[5, 14, 5, 9, 11, 12, 9]`
...
2. 데이터 체크리스트를 확인한다:
   - 주제가 22개 그대로인가?
   - 새 concept 23개가 각 주제의 `concepts` **끝에** 붙었는가?
