...
- `/phases/1-exam-gaps/step2.md` — 이 step이 문제로 만들 개념을 넣은 step이다.
...
| topicId | 문항 수 | id 범위 |
|---|---|---|
| `compute-delivery` | 7 | q188–q194 |
| `serverless-containers` | 7 | q195–q201 |
| `messaging-backup` | 7 | q202–q208 |

**합계 21문항 (q188–q208).** id는 `q` + 3자리 zero-padding이다.

### 개념 하나에 문항 하나

이 phase의 문항은 step 2에서 추가한 **새 concept과 1:1로 대응한다.**

- step 2에서 주제 `X`에 concept을 N개 넣었으면, 이 step은 주제 `X`에 문항을 N개 만든다.
- 각 문항의 `conceptId`는 **step 2에서 새로 추가한 concept 중 하나**이며,
...
1. 위 AC 커맨드를 실행한다.
2. 데이터 체크리스트를 확인한다:
   - 문항 21개가 배열 **끝에** 붙었는가? id가 q188부터 q208까지 빠짐없이 이어지는가?
   - 주제별 문항 수가 표와 일치하는가?
   - 모든 `conceptId`가 step 2에서 추가한 새 concept이고, 중복 없이 한 번씩 쓰였는가?
