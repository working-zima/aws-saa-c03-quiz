...
- `/phases/1-exam-gaps/step1.md` — 이 step이 문제로 만들 개념을 넣은 step이다.
...
| topicId | 문항 수 | id 범위 |
|---|---|---|
| `rds-storage-features` | 4 | q179–q182 |
| `aurora-dynamodb-cache` | 5 | q183–q187 |

**합계 9문항 (q179–q187).** id는 `q` + 3자리 zero-padding이다.

### 개념 하나에 문항 하나

이 phase의 문항은 step 1에서 추가한 **새 concept과 1:1로 대응한다.**

- step 1에서 주제 `X`에 concept을 N개 넣었으면, 이 step은 주제 `X`에 문항을 N개 만든다.
- 각 문항의 `conceptId`는 **step 1에서 새로 추가한 concept 중 하나**이며,
...
1. 위 AC 커맨드를 실행한다.
2. 데이터 체크리스트를 확인한다:
   - 문항 9개가 배열 **끝에** 붙었는가? id가 q179부터 q187까지 빠짐없이 이어지는가?
   - 주제별 문항 수가 표와 일치하는가?
   - 모든 `conceptId`가 step 1에서 추가한 새 concept이고, 중복 없이 한 번씩 쓰였는가?
