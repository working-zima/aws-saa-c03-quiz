# Step 5: questions-gap-foundation
...
- `/phases/1-exam-gaps/step0.md` — 이 step이 문제로 만들 개념을 넣은 step이다.
...
| topicId | 문항 수 | id 범위 |
|---|---|---|
| `aws-core-services` | 1 | q170–q170 |
| `s3-versioning-lifecycle` | 2 | q171–q172 |
| `s3-encryption-batch` | 2 | q173–q174 |
| `block-file-storage` | 4 | q175–q178 |

**합계 9문항 (q170–q178).** id는 `q` + 3자리 zero-padding이다.

### 개념 하나에 문항 하나

이 phase의 문항은 step 0에서 추가한 **새 concept과 1:1로 대응한다.**

- step 0에서 주제 `X`에 concept을 N개 넣었으면, 이 step은 주제 `X`에 문항을 N개 만든다.
- 각 문항의 `conceptId`는 **step 0에서 새로 추가한 concept 중 하나**이며,
...
## 기존 테스트 손보기 (이 step에서만)

`src/data/data.test.ts`에 **끝이 열린 슬라이스**를 쓰는 단언이 두 개 있다.
...
`slice(116)`은 116번 이후 **전부**를 가져오므로, 문항을 하나라도 더하는 순간 깨진다.
`questions.slice(116, 169)`로 **범위를 닫아라.** 두 군데 모두다.

이 단언의 의도는 "0-mvp의 보안·운영 문항 53개가 그대로 있다"를 지키는 것이다.
단언을 지우거나 길이 검사를 빼지 마라. 슬라이스 끝만 막아라.
...
1. 위 AC 커맨드를 실행한다.
2. 데이터 체크리스트를 확인한다:
   - 문항 9개가 배열 **끝에** 붙었는가? id가 q170부터 q178까지 빠짐없이 이어지는가?
   - 주제별 문항 수가 표와 일치하는가?
   - 모든 `conceptId`가 step 0에서 추가한 새 concept이고, 중복 없이 한 번씩 쓰였는가?
