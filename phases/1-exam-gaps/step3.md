...
|---|---|---|
| `vpc-networking` | `endpoint-pricing`, `egress-only-igw`, `nat-instance`, `vpc-peering-scaling-limit`, `s3-is-regional` | 5 |
| `hybrid-connectivity` | `direct-connect-caveats`, `direct-connect-gateway`, `client-vpn`, `access-terms`, `data-locality-cost`, `onprem-connectivity-heuristic` | 6 |
| `route53` | `private-hosted-zone`, `multivalue-answer-details` | 2 |
| `analytics-monitoring` | `glue-crawler`, `log-analysis-options` | 2 |

**합계 15개 concept.** slug는 `exam-gaps.md`의 `### `...`` 제목에 있는 값 그대로 쓴다.
...
2. 데이터 체크리스트를 확인한다:
   - 주제가 22개 그대로인가?
   - 새 concept 15개가 각 주제의 `concepts` **끝에** 붙었는가?
