# Step 3: rewrite-block-file-transfer — 해설 22개 재작성
...
|---|---|---|---|
| `ebs-instance-store` | EBS·인스턴스 스토어·스냅샷·배치 그룹 | 15 | 5 |
| `efs-fsx` | EFS·FSx(Windows·Lustre·ONTAP) | 25 | 8 |
| `data-transfer-services` | DataSync·Snowball Edge·Transfer Family·S3 전송 | 19 | 3 |
| `storage-gateway-migration` | Storage Gateway·DMS·Application Migration Service | 7 | 6 |

**담당 문항 22개. 현재 해설 평균 144자, 최소 67자다.**

| 문항 | 현재 해설 길이 | 근거 개념 | 근거 |
|---|---|---|---|
| `q040` | 203 | `ebs-instance-store.ebs` | 개념 본문 |
| `q041` | 185 | `ebs-instance-store.ebs` | 개념 본문 |
| `q042` | 178 | `efs-fsx.efs` | 개념 본문 |
| `q043` | 152 | `efs-fsx.efs` | 개념 본문 |
| `q044` | 94 | `ebs-instance-store.instance-store` | 개념 본문 |
| `q045` | 136 | `efs-fsx.fsx` | 개념 본문 |
| `q046` | 158 | `efs-fsx.fsx` | 개념 본문 |
| `q047` | 173 | `efs-fsx.fsx` | 개념 본문 |
| `q048` | 173 | `efs-fsx.fsx` | 개념 본문 |
| `q049` | 107 | `data-transfer-services.datasync` | 개념 본문 |
| `q050` | 67 | `data-transfer-services.snowball-edge` | 개념 본문 |
| `q051` | 145 | `data-transfer-services.transfer-family` | 개념 본문 |
| `q052` | 97 | `storage-gateway-migration.storage-gateway` | 개념 본문 |
| `q053` | 104 | `storage-gateway-migration.storage-gateway` | 개념 본문 |
| `q054` | 137 | `storage-gateway-migration.storage-gateway` | 개념 본문 |
| `q055` | 126 | `storage-gateway-migration.storage-gateway` | 개념 본문 |
| `q056` | 94 | `storage-gateway-migration.storage-gateway` | 개념 본문 |
| `q057` | 80 | `storage-gateway-migration.storage-gateway` | 개념 본문 |
| `q175` | 191 | `ebs-instance-store.cluster-placement-group` | `exam-gaps.md`의 `cluster-placement-group` |
| `q176` | 164 | `ebs-instance-store.ebs-elastic-volumes` | `exam-gaps.md`의 `ebs-elastic-volumes` |
| `q177` | 182 | `efs-fsx.efs-lifecycle-management` | `exam-gaps.md`의 `efs-lifecycle-management` |
| `q178` | 223 | `efs-fsx.fsx-ontap-multi-az` | `exam-gaps.md`의 `fsx-ontap-multi-az` |

이 중 **4문항**은 근거 개념이 `docs/source/exam-gaps.md`에도 항목으로 있다(위 표의 "근거" 열). 그 개념은 개념 본문과 exam-gaps 항목 **양쪽**을 읽어라 — exam-gaps 항목에는 `[섹션 #문항 pP]` 인용이 붙어 있어 개념 본문이 줄인 사실(수치 한계·전제 조건 등)이 남아 있을 수 있다.

담당 문항의 현재 미달 상태를 다시 재려면:

```bash
node scripts/explanation-audit.mjs ebs-instance-store efs-fsx data-transfer-services storage-gateway-migration
```
...
npm test                                               # 전부 통과
node scripts/explanation-audit.mjs ebs-instance-store efs-fsx data-transfer-services storage-gateway-migration  # exit 0 — 담당 문항 전부 187자 이상
```
...
1. 위 AC 커맨드를 실행한다.
2. `node scripts/explanation-audit.mjs ebs-instance-store efs-fsx data-transfer-services storage-gateway-migration`가 내는 "오답 언급" 열을 눈으로 확인한다.
