# Step 10: rewrite-network-acl-waf — 해설 23개 재작성
...
|---|---|---|---|
| `security-groups-nacl` | 보안 그룹·NACL | 9 | 12 |
| `waf-shield` | WAF·Shield·Firewall Manager | 15 | 11 |

**담당 문항 23개. 현재 해설 평균 125자, 최소 58자다.**

| 문항 | 현재 해설 길이 | 근거 개념 | 근거 |
|---|---|---|---|
| `q129` | 118 | `security-groups-nacl.security-group` | 개념 본문 |
| `q130` | 86 | `security-groups-nacl.nacl` | 개념 본문 |
| `q131` | 62 | `security-groups-nacl.security-group` | 개념 본문 |
| `q132` | 58 | `security-groups-nacl.security-group` | 개념 본문 |
| `q133` | 111 | `security-groups-nacl.security-group` | 개념 본문 |
| `q134` | 103 | `security-groups-nacl.nacl` | 개념 본문 |
| `q135` | 105 | `security-groups-nacl.nacl` | 개념 본문 |
| `q136` | 99 | `security-groups-nacl.security-group` | 개념 본문 |
| `q137` | 92 | `security-groups-nacl.nacl` | 개념 본문 |
| `q146` | 180 | `waf-shield.waf` | 개념 본문 |
| `q147` | 151 | `waf-shield.waf` | 개념 본문 |
| `q148` | 191 | `waf-shield.waf` | 개념 본문 |
| `q149` | 169 | `waf-shield.shield` | 개념 본문 |
| `q152` | 99 | `waf-shield.cloudfront` | 개념 본문 |
| `q153` | 101 | `waf-shield.cloudfront` | 개념 본문 |
| `q154` | 112 | `waf-shield.cloudfront` | 개념 본문 |
| `q224` | 91 | `security-groups-nacl.security-group-referencing` | `exam-gaps.md`의 `security-group-referencing` |
| `q225` | 142 | `security-groups-nacl.nacl-rule-limit` | `exam-gaps.md`의 `nacl-rule-limit` |
| `q226` | 150 | `security-groups-nacl.web-acl-vs-nacl` | `exam-gaps.md`의 `web-acl-vs-nacl` |
| `q231` | 197 | `waf-shield.waf-attach-targets` | `exam-gaps.md`의 `waf-attach-targets` |
| `q232` | 163 | `waf-shield.waf-bot-control` | `exam-gaps.md`의 `waf-bot-control` |
| `q233` | 118 | `waf-shield.waf-rule-types` | `exam-gaps.md`의 `waf-rule-types` |
| `q234` | 182 | `waf-shield.shield-advanced-drt` | `exam-gaps.md`의 `shield-advanced-drt` |
...
```bash
node scripts/explanation-audit.mjs security-groups-nacl waf-shield
```

## 작업

담당 문항 23개의 `explanation`을 **그 개념을 가르치는 글**로 다시 쓴다.
...
- 기존 테스트를 고치거나 깨뜨리지 마라 — **한 곳만 예외다.**
  `src/data/data.test.ts`의 「풀이가 그 약어의 첫 등장 자리에 붙는다」가 담당 문항 `q130`의
  해설을 **문자열 그대로 고정**하고 있다(ADR-015 phase가 남긴 fixture). `q130`을 다시 쓰면
  이 단정이 깨지므로 **그 fixture 문자열만 새 해설로 갱신하라.** 같은 `it` 안의 `q080`
  fixture와 다른 단언은 건드리지 마라 — `q080`은 step 5가 이미 같은 방식으로 갱신했다.
  갱신하기 전에 **테스트가 실제로 주장하는 것이 새 해설에서도 성립하는지 확인하라**:
  약어의 풀네임 괄호가 그 약어의 **첫 등장 자리**에 붙어 있어야 한다. 성립하지 않으면
  fixture를 고칠 것이 아니라 해설을 고쳐야 한다.
- 그 한 곳 말고는 테스트를 고치지 마라. `explanation`만 고치면 통과해야 한다.
  실패하면 위 1·5·6 중 하나를 어긴 것이다.
...
npm test                                               # 전부 통과
node scripts/explanation-audit.mjs security-groups-nacl waf-shield  # exit 0 — 담당 문항 전부 187자 이상
```
...
1. 위 AC 커맨드를 실행한다.
2. `node scripts/explanation-audit.mjs security-groups-nacl waf-shield`가 내는 "오답 언급" 열을 눈으로 확인한다.
