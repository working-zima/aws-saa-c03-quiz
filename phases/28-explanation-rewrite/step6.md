# Step 6: rewrite-serverless-containers — 해설 19개 재작성
...
|---|---|---|---|
| `lambda` | Lambda | 18 | 5 |
| `ecs-eks-fargate` | ECS·EKS·Fargate·Batch·ECR·Elastic Beanstalk | 23 | 5 |
| `api-gateway-step-functions` | API Gateway·Step Functions | 20 | 5 |
| `backup-disaster-recovery` | AWS Backup·재해 복구 전략·Elastic Disaster Recovery | 11 | 2 |
| `kinesis-streaming` | Kinesis Data Streams·Data Firehose·Flink·Video Streams·MSK | 16 | 2 |

**담당 문항 19개. 현재 해설 평균 138자, 최소 83자다.**

| 문항 | 현재 해설 길이 | 근거 개념 | 근거 |
|---|---|---|---|
| `q083` | 120 | `ecs-eks-fargate.ecs` | 개념 본문 |
| `q084` | 133 | `ecs-eks-fargate.ecs` | 개념 본문 |
| `q085` | 83 | `lambda.lambda` | 개념 본문 |
| `q086` | 92 | `lambda.lambda` | 개념 본문 |
| `q087` | 85 | `lambda.lambda` | 개념 본문 |
| `q088` | 90 | `api-gateway-step-functions.step-functions` | 개념 본문 |
| `q089` | 184 | `api-gateway-step-functions.api-gateway` | 개념 본문 |
| `q090` | 197 | `api-gateway-step-functions.api-gateway` | 개념 본문 |
| `q098` | 169 | `backup-disaster-recovery.backup` | 개념 본문 |
| `q128` | 109 | `kinesis-streaming.streaming-services-comparison` | 개념 본문 |
| `q195` | 161 | `ecs-eks-fargate.eks` | `exam-gaps.md`의 `eks` |
| `q196` | 141 | `ecs-eks-fargate.fargate-no-time-limit` | `exam-gaps.md`의 `fargate-no-time-limit` |
| `q197` | 144 | `lambda.lambda-function-url` | `exam-gaps.md`의 `lambda-function-url` |
| `q199` | 157 | `lambda.lambda-vpc-access` | `exam-gaps.md`의 `lambda-vpc-access` |
| `q200` | 206 | `api-gateway-step-functions.api-gateway-jwt-authorizer` | `exam-gaps.md`의 `api-gateway-jwt-authorizer` |
| `q201` | 88 | `ecs-eks-fargate.aws-batch` | `exam-gaps.md`의 `aws-batch` |
| `q202` | 177 | `kinesis-streaming.msk` | `exam-gaps.md`의 `msk` |
| `q206` | 113 | `api-gateway-step-functions.step-functions-features` | `exam-gaps.md`의 `step-functions-features` |
| `q208` | 168 | `backup-disaster-recovery.backup-long-term-retention` | `exam-gaps.md`의 `backup-long-term-retention` |

이 중 **9문항**은 근거 개념이 `docs/source/exam-gaps.md`에도 항목으로 있다(위 표의 "근거" 열). 그 개념은 개념 본문과 exam-gaps 항목 **양쪽**을 읽어라 — exam-gaps 항목에는 `[섹션 #문항 pP]` 인용이 붙어 있어 개념 본문이 줄인 사실(수치 한계·전제 조건 등)이 남아 있을 수 있다.

담당 문항의 현재 미달 상태를 다시 재려면:

```bash
node scripts/explanation-audit.mjs lambda ecs-eks-fargate api-gateway-step-functions backup-disaster-recovery kinesis-streaming
```
...
3번의 재료는 **오답 보기가 가리키는 개념의 본문**에서 가져온다. 담당 주제의 개념 목록이
곧 그 재료다(위 표의 "주제의 전체 개념"). 자기 개념 본문이 짧아도 이웃 개념 본문이 있으므로
쓸 것이 없는 경우는 없다.

**오답이 담당 주제 밖의 개념을 가리키면** 그 개념의 `topics.json` 본문을 쓴다. 주제 경계는
ADR-023이 학습 순서를 위해 그은 선이지 출처의 경계가 아니므로 그 본문도 근거가 된다.
단 그때는 **사양과 수치를 가져오지 마라** — 그 개념이 무엇을 하는 것인지(성격)만 한 줄로 쓰고
지운다. 이유: 오답 하나를 지우는 데 필요한 것은 사양이 아니라 "그것은 무엇인가"이고,
그 주제를 아직 읽지 않은 학습자에게 주제 밖 서비스의 사양은 검증할 수 없는 문장이 된다.
근거는 ADR-027 2번이다.
...
- 기존 테스트를 고치거나 깨뜨리지 마라. 담당 문항의 해설 텍스트를 문자열로 고정하는 단정은
  없으므로, `explanation`만 고치면 통과해야 한다. 실패하면 위 1·5·6 중 하나를 어긴 것이다.
  (`src/data/data.test.ts`의 「풀이가 그 약어의 첫 등장 자리에 붙는다」가 `q080`·`q130`의
  해설만 문자열로 고정하는데, 둘 다 이 step의 담당이 아니다.)
...
npm test                                               # 전부 통과
node scripts/explanation-audit.mjs lambda ecs-eks-fargate api-gateway-step-functions backup-disaster-recovery kinesis-streaming  # exit 0 — 담당 문항 전부 187자 이상
```
...
1. 위 AC 커맨드를 실행한다.
2. `node scripts/explanation-audit.mjs lambda ecs-eks-fargate api-gateway-step-functions backup-disaster-recovery kinesis-streaming`가 내는 "오답 언급" 열을 눈으로 확인한다.
