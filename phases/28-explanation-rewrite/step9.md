# Step 9: rewrite-hybrid-threat-detection — 해설 18개 재작성
...
|---|---|---|---|
| `hybrid-connectivity` | Site-to-Site VPN·Direct Connect·Transit Gateway | 19 | 14 |
| `guardduty-macie-inspector` | GuardDuty·Macie·Inspector·Security Hub | 11 | 4 |

**담당 문항 18개. 현재 해설 평균 138자, 최소 92자다.**

| 문항 | 현재 해설 길이 | 근거 개념 | 근거 |
|---|---|---|---|
| `q109` | 112 | `hybrid-connectivity.site-to-site-vpn` | 개념 본문 |
| `q110` | 119 | `hybrid-connectivity.direct-connect` | 개념 본문 |
| `q111` | 129 | `hybrid-connectivity.vpn-vs-direct-connect` | 개념 본문 |
| `q112` | 117 | `hybrid-connectivity.vpn-vs-direct-connect` | 개념 본문 |
| `q113` | 148 | `hybrid-connectivity.transit-gateway` | 개념 본문 |
| `q114` | 123 | `hybrid-connectivity.vpn-vs-direct-connect` | 개념 본문 |
| `q115` | 128 | `hybrid-connectivity.direct-connect` | 개념 본문 |
| `q116` | 174 | `hybrid-connectivity.transit-gateway` | 개념 본문 |
| `q150` | 106 | `guardduty-macie-inspector.guardduty` | 개념 본문 |
| `q151` | 92 | `guardduty-macie-inspector.macie` | 개념 본문 |
| `q214` | 132 | `hybrid-connectivity.direct-connect-caveats` | `exam-gaps.md`의 `direct-connect-caveats` |
| `q215` | 183 | `hybrid-connectivity.direct-connect-gateway` | `exam-gaps.md`의 `direct-connect-gateway` |
| `q216` | 166 | `hybrid-connectivity.client-vpn` | `exam-gaps.md`의 `client-vpn` |
| `q217` | 126 | `hybrid-connectivity.access-terms` | `exam-gaps.md`의 `access-terms` |
| `q218` | 111 | `hybrid-connectivity.data-locality-cost` | `exam-gaps.md`의 `data-locality-cost` |
| `q219` | 208 | `hybrid-connectivity.onprem-connectivity-heuristic` | `exam-gaps.md`의 `onprem-connectivity-heuristic` |
| `q235` | 123 | `guardduty-macie-inspector.guardduty-db-login` | `exam-gaps.md`의 `guardduty-db-login` |
| `q236` | 178 | `guardduty-macie-inspector.security-service-lineup` | `exam-gaps.md`의 `security-service-lineup` |

이 중 **8문항**은 근거 개념이 `docs/source/exam-gaps.md`에도 항목으로 있다(위 표의 "근거" 열). 그 개념은 개념 본문과 exam-gaps 항목 **양쪽**을 읽어라 — exam-gaps 항목에는 `[섹션 #문항 pP]` 인용이 붙어 있어 개념 본문이 줄인 사실(수치 한계·전제 조건 등)이 남아 있을 수 있다.

담당 문항의 현재 미달 상태를 다시 재려면:

```bash
node scripts/explanation-audit.mjs hybrid-connectivity guardduty-macie-inspector
```

## 작업

담당 문항 18개의 `explanation`을 **그 개념을 가르치는 글**로 다시 쓴다.
...
npm test                                               # 전부 통과
node scripts/explanation-audit.mjs hybrid-connectivity guardduty-macie-inspector  # exit 0 — 담당 문항 전부 187자 이상
```
...
1. 위 AC 커맨드를 실행한다.
2. `node scripts/explanation-audit.mjs hybrid-connectivity guardduty-macie-inspector`가 내는 "오답 언급" 열을 눈으로 확인한다.
