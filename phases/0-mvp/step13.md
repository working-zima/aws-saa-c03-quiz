# Step 13: questions-security-ops
...
- `/docs/source/concepts-raw.md` — 원본. 36~50페이지가 이 step의 범위다.
- `/phases/0-mvp/step7.md` — 앞선 문제 step. 같은 규칙을 따른다.
- 이전 step 산출물: `src/data/topics.json`(주제 22개), `src/data/questions.json`(116문항),
...
|---|---|---|
| `route53` | 6 | q117–q122 |
| `analytics-monitoring` | 6 | q123–q128 |
| `security-groups-nacl` | 9 | q129–q137 |
| `secrets-encryption` | 8 | q138–q145 |
| `threat-protection` | 9 | q146–q154 |
| `identity-access` | 9 | q155–q163 |
| `cost-management` | 6 | q164–q169 |

**합계 53문항 (q117–q169).** 문항 수와 id 범위를 정확히 지켜라.
...
`answerIndex`를 0~3에 고르게 분산하라. 이 step의 53문항 안에서 각 위치가
전체의 20~30% 사이여야 한다.

### 지킬 것

- CRITICAL: 원본에 없는 AWS 지식을 끌어오지 마라 (ADR-006). 정확한 요금, 원본에 없는 서비스,
  최신 기능, 시험 출제 경향 같은 걸 넣지 마라. 근거는 전부 36~50페이지 안에 있어야 한다.
- 같은 개념을 묻는 문제를 문구만 바꿔 중복 출제하지 마라. 기존 116문항과도 겹치면 안 된다.
...
1. 위 AC 커맨드를 실행한다.
2. 데이터 체크리스트를 확인한다:
   - 새 문항이 배열 **끝에** 붙었는가? 기존 116문항이 그대로인가?
...
- 주제 1~15번 문제를 다시 만들지 마라. 이유: step 7과 12에서 이미 만들었다.
