# Step 6: dynamodb-and-elasticache
...
| 주제 | 개념 | 문항 | 범위 |
|---|---|---|---|
| `dynamodb` | 18 | 20 | 개념·문항 전부 |
| `elasticache-purpose-built-db` | 14 | 16 | 개념·문항 전부 |

### `dynamodb`

**개념 18개. 문장형 제목 7개.** 아래 목록에서 `▸`가 문장형이다.

```
   0. dynamodb                                       DynamoDB
   1. dynamodb-single-digit-latency                  DynamoDB의 응답 시간
   2. dynamodb-streams                               DynamoDB Streams
   3. dynamodb-global-tables                         DynamoDB 글로벌 테이블
   4. dynamodb-ttl                                   DynamoDB TTL
   5. dynamodb-global-secondary-index                전역 보조 인덱스(GSI)
   6. dynamodb-capacity-modes                        프로비저닝된 용량과 온디맨드 용량
   7. dynamodb-auto-scaling-target-utilization       DynamoDB 오토 스케일링과 목표 활용률
   8. dynamodb-read-consistency                      최종 일관성 읽기와 강력한 일관성 읽기
▸  9. dynamodb-s3-export-vs-streams                  분석용 적재는 기본 내보내기가 먼저다
▸ 10. dynamodb-incremental-export                    S3 내보내기에는 증분 형태가 있다
▸ 11. dynamodb-export-no-read-capacity               S3 내보내기는 테이블의 읽기 용량을 쓰지 않는다
  12. dynamodb-pitr                                  DynamoDB PITR의 보존 한계
▸ 13. dynamodb-export-requires-pitr                  S3로 내보내려면 PITR이 켜져 있어야 한다
  14. dynamodb-item-size-limit                       DynamoDB 항목 크기 제한
▸ 15. dynamodb-ttl-deletion-delay                    TTL 삭제는 만료 시각에 딱 맞춰 일어나지 않는다
▸ 16. dynamodb-streams-retention-24h                 DynamoDB Streams는 24시간만 보존한다
▸ 17. dynamodb-streams-batch-size                    DynamoDB Streams 소비의 배치 크기를 조정한다
```

**`data.test.ts`가 글자로 고정한 제목·요약** — 고치려면 단언의 문구도 함께 맞춰야 한다.
**단언의 뜻은 바꾸지 마라.**

- `dynamodb` — 단언이 잡고 있는 말: `DynamoDB는`

**문항 20개**

`q069` `q070` `q071` `q186` `q407` `q408` `q409` `q410` `q411` `q412` `q413` `q414` `q415` `q416` `q417` `q418` `q419` `q420` `q421` `q422`
...
- `q186` — 단언이 잡고 있는 말: `DynamoDB에서 사고가 나기 직전 시점으로 데이터를 되돌려야 하는 상황이다. 특정 시점 복구(PIT…`

### `elasticache-purpose-built-db`

**개념 14개. 문장형 제목 3개.** 아래 목록에서 `▸`가 문장형이다.

```
   0. elasticache                                    ElastiCache
   1. documentdb                                     Amazon DocumentDB
   2. neptune                                        Amazon Neptune
   3. neptune-streams                                Neptune Streams
   4. qldb                                           Amazon Quantum Ledger Database(QLDB)
   5. timestream                                     Amazon Timestream
   6. elasticache-redis-vs-memcached                 Redis와 Memcached의 갈림길
   7. dax-dynamodb-only                              DAX는 DynamoDB 전용
   8. elasticache-multi-az-failover                  ElastiCache의 다중 AZ 자동 장애 조치
   9. elasticache-global-datastore                   ElastiCache 글로벌 데이터스토어
  10. documentdb-global-cluster                      DocumentDB 글로벌 클러스터
▸ 11. cache-requires-application-change              캐시를 넣으면 애플리케이션을 고쳐야 한다
▸ 12. elasticache-not-a-durable-store                캐시는 처리 결과를 두는 자리가 아니다
▸ 13. dax-encryption-at-rest                         DAX의 저장 중 암호화는 만들 때만 켠다
```

**제목에 옛 어휘가 든 것** (문장형이 아니어도 본다)

- `elasticache-not-a-durable-store` — 자리 — "캐시는 처리 결과를 두는 자리가 아니다"

**`summary`에 옛 어휘가 든 개념**

- `dax-encryption-at-rest` — 돌아가

**`data.test.ts`가 글자로 고정한 제목·요약** — 고치려면 단언의 문구도 함께 맞춰야 한다.
**단언의 뜻은 바꾸지 마라.**

- `elasticache` — 단언이 잡고 있는 말: `ElastiCache는`
- `cache-requires-application-change` — 단언이 잡고 있는 말: `ElastiCache는`
- `elasticache-not-a-durable-store` — 단언이 잡고 있는 말: `ElastiCache는`

**문항 16개**

`q072` `q073` `q185` `q187` `q423` `q424` `q425` `q426` `q427` `q428` `q429` `q430` `q431` `q432` `q433` `q434`
...
node scripts/content-audit.mjs dynamodb elasticache-purpose-built-db
```
