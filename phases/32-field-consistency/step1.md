# Step 1: s3-encryption-and-access-control-and-ebs
...
| 주제 | 개념 | 문항 | 범위 |
|---|---|---|---|
| `s3-encryption-batch` | 13 | 16 | 개념·문항 전부 |
| `s3-access-control` | 13 | 13 | 개념·문항 전부 |
| `ebs-instance-store` | 15 | 16 | 개념·문항 전부 |

### `s3-encryption-batch`

**개념 13개. 문장형 제목 0개.** 아래 목록에서 `▸`가 문장형이다.

```
   0. sse                                            SSE (Server Side Encryption)
   1. sse-types                                      SSE 종류
   2. client-side-encryption                         클라이언트 측 암호화
   3. batch-operations                               S3 Batch Operations
   4. s3-inventory-report                            S3 인벤토리
   5. s3-object-lambda                               S3 Object Lambda
   6. envelope-encryption                            봉투 암호화 (Envelope Encryption)
   7. sse-kms-audit-trail                            SSE-KMS의 감사 추적과 업로드 강제
   8. batch-copy-vs-replication                      일회성 대량 복사와 지속 복제
   9. s3-batch-operations-lambda-invoke              S3 Batch Operations의 Lambda 호출
  10. sse-kms-cost                                   SSE-KMS의 비용 구조와 S3 Bucket Key
  11. sse-c-no-rotation-or-audit                     SSE-C의 자동 교체와 감사 추적 한계
  12. s3-secure-transport-condition                  버킷 정책의 전송 구간 암호화 강제
```

**`data.test.ts`가 글자로 고정한 제목·요약** — 고치려면 단언의 문구도 함께 맞춰야 한다.
**단언의 뜻은 바꾸지 마라.**

- `s3-inventory-report` — 단언이 잡고 있는 말: `인벤토리 보고서`
- `s3-batch-operations-lambda-invoke` — 단언이 잡고 있는 말: `인벤토리 보고서`

**문항 16개**

`q034` `q035` `q036` `q037` `q038` `q039` `q173` `q174` `q276` `q277` `q278` `q279` `q280` `q281` `q282` `q283`

**`data.test.ts`가 글자로 고정한 프롬프트·보기** — 고치려면 단언의 문구도 함께 맞춰야 한다.
**단언의 뜻은 바꾸지 마라.**

- `q034` — 단언이 잡고 있는 말: `S3에 파일을 그대로 저장하면 보안 침해가 일어났을 때 공격자가 그 내용을 곧바로 확인할 수 있다. 그…`
- `q039` — 단언이 잡고 있는 말: `S3 버킷에 파일이 수백만 개에서 수십억 개까지 들어 있는 상황이다. 이 파일 전부에 복사나 삭제 같은…`
- `q173` — 단언이 잡고 있는 말: `봉투 암호화는 데이터를 데이터 키로 암호화한 다음, 그 데이터 키를 다시 마스터 키로 한 번 더 암호화…`
- `q174` — 단언이 잡고 있는 말: `SSE-KMS로 암호화한 S3 버킷에 객체가 초당 수백만 건씩 올라온다. 객체마다 KMS API를 호출…`
- `q277` — 단언이 잡고 있는 말: `버킷에 객체가 수백만 개 쌓여 있다`

### `s3-access-control`

**개념 13개. 문장형 제목 6개.** 아래 목록에서 `▸`가 문장형이다.

```
▸  0. s3-cross-account-bucket-policy                 계정 간 버킷 접근은 버킷 정책으로 연다
   1. s3-presigned-url                               S3 사전 서명된 URL
   2. s3-access-grants                               S3 액세스 권한(Access Grants)
   3. s3-access-point                                S3 액세스 포인트
   4. s3-multi-region-access-point                   S3 멀티 리전 액세스 포인트
   5. s3-storage-lens                                S3 Storage Lens
▸  6. s3-cors-not-authorization                      CORS는 권한을 주는 장치가 아니다
   7. s3-requester-pays                              요청자 부담 버킷
▸  8. s3-storage-lens-advanced-activity-metrics      고급 활동 메트릭으로 식은 버킷을 찾는다
▸  9. s3-account-level-public-access-block           공개 액세스 차단은 계정 수준에도 건다
▸ 10. block-public-access-allows-explicit-grants     막는 것은 "공개"이지 "허용"이 아니다
  11. s3-bucket-policy-source-vpc-condition          버킷을 특정 VPC에서만 열기
▸ 12. s3-website-endpoint-no-https                   정적 웹사이트 엔드포인트는 HTTPS를 지원하지 않는다
```

**문항 13개**

`q247` `q248` `q249` `q250` `q251` `q252` `q253` `q254` `q255` `q256` `q257` `q258` `q259`
...
**개념 15개. 문장형 제목 4개.** 아래 목록에서 `▸`가 문장형이다.

```
   0. ebs                                            EBS (Elastic Block Store)
   1. ebs-elastic-volumes                            EBS Elastic Volumes
   2. instance-store                                 인스턴스 스토어 (Instance Store)
   3. cluster-placement-group                        클러스터 배치 그룹
   4. elastic-fabric-adapter                         Elastic Fabric Adapter(EFA)
   5. ebs-volume-type-names                          EBS 볼륨 유형의 실제 이름
▸  6. gp3-iops-independent-of-size                   gp3는 IOPS를 용량과 따로 정한다
   7. spread-placement-group                         분산 배치 그룹
   8. io2-block-express-iops-ceiling                 io2 Block Express가 올려 주는 IOPS 상한
▸  9. ebs-encryption-by-default                      EBS 기본 암호화는 계정 속성이다
▸ 10. ebs-encryption-performance                     EBS 암호화는 성능을 깎지 않는다
  11. ebs-recycle-bin                                EBS 스냅샷 휴지통
  12. ebs-snapshot-block-public-access               EBS 스냅샷의 공개 액세스 차단
▸ 13. data-lifecycle-manager                         스냅샷의 생성·보존·삭제를 정책으로 돌린다
  14. ebs-fast-snapshot-restore                      스냅샷으로 만든 볼륨의 첫 접근이 느린 이유
```

**제목에 옛 어휘가 든 것** (문장형이 아니어도 본다)

- `data-lifecycle-manager` — 돌린 — "스냅샷의 생성·보존·삭제를 정책으로 돌린다"

**`data.test.ts`가 글자로 고정한 제목·요약** — 고치려면 단언의 문구도 함께 맞춰야 한다.
**단언의 뜻은 바꾸지 마라.**

- `cluster-placement-group` — 단언이 잡고 있는 말: `클러스터 배치 그룹`

**문항 16개**

`q040` `q041` `q044` `q175` `q176` `q284` `q285` `q286` `q287` `q288` `q289` `q290` `q291` `q292` `q293` `q294`

**`data.test.ts`가 글자로 고정한 프롬프트·보기** — 고치려면 단언의 문구도 함께 맞춰야 한다.
**단언의 뜻은 바꾸지 마라.**

- `q175` — 단언이 잡고 있는 말: `수천 개 노드가 동시에 데이터를 읽고 쓰는 HPC 워크로드를 운영하는 상황이다. 노드 사이의 네트워크 …`, `클러스터 배치 그룹`, `EFS 수명 주기 관리`
- `q176` — 단언이 잡고 있는 말: `EC2에 연결된 EBS 볼륨의 크기를 서비스를 멈추지 않고 늘려야 하는 상황이다. 볼륨을 떼었다 다시 …`, `클러스터 배치 그룹`
- `q287` — 단언이 잡고 있는 말: `클러스터 배치 그룹`
...
node scripts/content-audit.mjs s3-encryption-batch s3-access-control ebs-instance-store
```
