# Step 12: api-gateway-step-functions
...
| 주제 | 개념 | 문항 | 범위 |
|---|---|---|---|
| `api-gateway-step-functions` | 20 | 25 | 개념·문항 전부 |

### `api-gateway-step-functions`

**개념 20개. 문장형 제목 8개.** 아래 목록에서 `▸`가 문장형이다.
...
- `api-gateway-ip-restriction-by-resource-policy` — 자리 — "호출자를 IP로 거르는 자리도 리소스 정책이다"

**`summary`에 옛 어휘가 든 개념**

- `step-functions-express-workflow` — 자리
- `step-functions-map-state` — 돌리

**`data.test.ts`가 글자로 고정한 제목·요약** — 고치려면 단언의 문구도 함께 맞춰야 한다.
**단언의 뜻은 바꾸지 마라.**

- `api-gateway` — 단언이 잡고 있는 말: `API Gateway는`
- `step-functions` — 단언이 잡고 있는 말: `Step Functions는`
- `step-functions-long-running-workflow` — 단언이 잡고 있는 말: `Step Functions는`

**문항 25개**

`q088` `q089` `q090` `q200` `q206` `q529` `q530` `q531` `q532` `q533` `q534` `q535` `q536` `q537` `q538` `q539` `q540` `q541` `q542` `q543` `q544` `q545` `q546` `q547` `q548`
...
- `q088` — 단언이 잡고 있는 말: `이미지 리사이징이나 보고서 발송 같은 처리 여러 개를 정해진 순서로 이어 실행해야 하는 상황이다. 각 …`
- `q200` — 단언이 잡고 있는 말: `JWT로 사용자를 인증하는 퍼블릭 API를 ALB와 직접 통합해야 하는 상황이다. 이 둘을 기본으로 지…`
- `q533` — 단언이 잡고 있는 말: `연결을 오래 유지한 채`
...
node scripts/content-audit.mjs api-gateway-step-functions
```
