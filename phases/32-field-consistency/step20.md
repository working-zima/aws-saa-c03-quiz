# Step 20: waf-shield-and-guardduty-macie-inspector
...
| 주제 | 개념 | 문항 | 범위 |
|---|---|---|---|
| `waf-shield` | 15 | 20 | 개념·문항 전부 |
| `guardduty-macie-inspector` | 11 | 12 | 개념·문항 전부 |

### `waf-shield`

**개념 15개. 문장형 제목 3개.** 아래 목록에서 `▸`가 문장형이다.
...
`q146` `q147` `q148` `q149` `q152` `q153` `q154` `q231` `q232` `q233` `q234` `q668` `q669` `q670` `q671` `q672` `q673` `q674` `q675` `q676`

**`data.test.ts`가 글자로 고정한 프롬프트·보기** — 고치려면 단언의 문구도 함께 맞춰야 한다.
**단언의 뜻은 바꾸지 마라.**

- `q147` — 단언이 잡고 있는 말: `WAF가 방어하는 공격이 속한 계층은 무엇인가?`
- `q153` — 단언이 잡고 있는 말: `가중치 기반 라우팅`
- `q232` — 단언이 잡고 있는 말: `전 세계에서 오는 악성 봇이 대량의 요청을 보내 컴퓨팅 리소스를 낭비하고 있는 상황이다. 요청 속도 같…`
...
- `security-service-lineup` — 갈리 — "헷갈리기 쉬운 보안 서비스 네 가지"

**`summary`에 옛 어휘가 든 개념**

- `security-hub` — 자리
- `security-service-lineup` — 갈리
- `macie-automated-discovery` — 돌리

**`data.test.ts`가 글자로 고정한 제목·요약** — 고치려면 단언의 문구도 함께 맞춰야 한다.
**단언의 뜻은 바꾸지 마라.**

- `guardduty-db-login` — 단언이 잡고 있는 말: `GuardDuty는`
- `inspector-scans-ecr-images` — 단언이 잡고 있는 말: `Inspector는`

**문항 12개**

`q150` `q151` `q235` `q236` `q677` `q678` `q679` `q680` `q681` `q682` `q683` `q684`
...
- `q236` — 단언이 잡고 있는 말: `EC2 인스턴스의 소프트웨어 패치 누락과 공개적으로 알려진 보안 취약점(CVE)을 스캔하는 서비스는 무…`
...
node scripts/content-audit.mjs waf-shield guardduty-macie-inspector
```
