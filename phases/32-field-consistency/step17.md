...
| 주제 | 개념 | 문항 | 범위 |
|---|---|---|---|
| `emr-glue-athena` | 20 | 21 | 개념·문항 전부 |

### `emr-glue-athena`

**개념 20개. 문장형 제목 4개.** 아래 목록에서 `▸`가 문장형이다.
...
- `emr-node-instance-family-choice` — 갈린 — "노드 역할에 따라 인스턴스 제품군이 갈린다"

**`summary`에 옛 어휘가 든 개념**

- `glue-etl-with-per-customer-kms-key` — 띄우
- `log-storage-s3-athena` — 갈린

**문항 21개**

`q123` `q124` `q125` `q127` `q222` `q617` `q618` `q619` `q620` `q621` `q622` `q623` `q624` `q625` `q626` `q627` `q628` `q629` `q630` `q631` `q632`
...
- `q222` — 단언이 잡고 있는 말: `S3에 막 들어온 데이터를 Athena로 곧바로 분석하려는 상황이다. 이 데이터를 훑어 구조를 파악하고…`
...
node scripts/content-audit.mjs emr-glue-athena
```
