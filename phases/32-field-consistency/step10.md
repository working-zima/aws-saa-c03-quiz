...
| 주제 | 개념 | 문항 | 범위 |
|---|---|---|---|
| `lambda` | 18 | 25 | 개념·문항 전부 |

### `lambda`

**개념 18개. 문장형 제목 10개.** 아래 목록에서 `▸`가 문장형이다.
...
- `lambda-reserved-concurrency` — 띄우
- `lambda-kinesis-event-source` — 띄우
- `lambda-layer-size-limit` — 자리

**`data.test.ts`가 글자로 고정한 제목·요약** — 고치려면 단언의 문구도 함께 맞춰야 한다.
**단언의 뜻은 바꾸지 마라.**

- `lambda` — 단언이 잡고 있는 말: `Lambda는 서버 운영을 AWS에 맡기고 개발자가 올린 코드만 실행하는 서비스다.`

**문항 25개**

`q085` `q086` `q087` `q197` `q199` `q489` `q490` `q491` `q492` `q493` `q494` `q495` `q496` `q497` `q498` `q499` `q500` `q501` `q502` `q503` `q504` `q505` `q506` `q507` `q508`
...
- `q197` — 단언이 잡고 있는 말: `관리자가 필요할 때 눌러 보고서를 만드는 정도의 단순한 호출이다. API Gateway를 앞에 두기에는…`
- `q199` — 단언이 잡고 있는 말: `인터넷 게이트웨이`
- `q495` — 단언이 잡고 있는 말: `콜드 스타트는 그대로 남는다`
...
node scripts/content-audit.mjs lambda
```
