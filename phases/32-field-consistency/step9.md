# Step 9: cloudfront-global-accelerator
...
| 주제 | 개념 | 문항 | 범위 |
|---|---|---|---|
| `cloudfront-global-accelerator` | 24 | 27 | 개념·문항 전부 |

### `cloudfront-global-accelerator`

**개념 24개. 문장형 제목 8개.** 아래 목록에서 `▸`가 문장형이다.

```
   0. cloudfront                                     CloudFront
▸  1. cloudfront-alb-origin                          CloudFront의 오리진은 버킷만이 아니다
▸  2. cloudfront-multiple-origins                    배포 하나에 오리진을 여럿 둔다
▸  3. cloudfront-onprem-origin                       오리진이 AWS 밖에 있어도 된다
   4. global-accelerator                             Global Accelerator
   5. global-accelerator-static-ip                   Global Accelerator의 고정 IP
   6. global-accelerator-endpoints                   Global Accelerator가 앞에 붙는 대상
   7. edge-keyword                                   'Edge'라는 키워드
   8. lambda-at-edge                                 Lambda@Edge
   9. cloudfront-functions                           CloudFront Functions
  10. global-accelerator-protocols                   Global Accelerator가 처리하는 것
▸ 11. cloudfront-reduces-data-transfer-cost          엣지 캐시는 지연만이 아니라 비용도 줄인다
  12. global-accelerator-vs-dns-failover             DNS 캐시에 발목 잡히지 않는 장애 조치
  13. cloudfront-signed-url                          CloudFront 서명된 URL
  14. cloudfront-signed-cookie                       CloudFront 서명된 쿠키
  15. cloudfront-geo-restriction                     CloudFront 지리적 제한
  16. cloudfront-field-level-encryption              CloudFront 필드 수준 암호화
▸ 17. lambda-at-edge-origin-selection-by-viewer-location 뷰어 위치로 오리진을 고르는 일은 엣지 함수가 한다
▸ 18. lambda-at-edge-response-compression            엣지에서 응답을 압축해 전송량을 줄인다
  19. cloudfront-price-class                         CloudFront 가격 등급
  20. cloudfront-ttl                                 CloudFront TTL과 캐시 무효화의 관계
  21. cloudfront-s3-upload-with-oac                  CloudFront를 거쳐 S3에 올리기
▸ 22. cloudfront-alb-origin-access-restriction       ALB 오리진은 보안 그룹으로 CloudFront만 들이게 한다
▸ 23. cloudfront-functions-no-external-calls         CloudFront Functions는 다른 서비스를 부르지 못한다
```

**`summary`에 옛 어휘가 든 개념**

- `lambda-at-edge-response-compression` — 축이
- `cloudfront-functions-no-external-calls` — 자리

**`data.test.ts`가 글자로 고정한 제목·요약** — 고치려면 단언의 문구도 함께 맞춰야 한다.
**단언의 뜻은 바꾸지 마라.**

- `cloudfront` — 단언이 잡고 있는 말: `CloudFront는`, `CloudFront는`, `CloudFront는`
- `global-accelerator` — 단언이 잡고 있는 말: `Global Accelerator는`
- `cloudfront-reduces-data-transfer-cost` — 단언이 잡고 있는 말: `CloudFront는`, `CloudFront는`, `CloudFront는`
- `cloudfront-ttl` — 단언이 잡고 있는 말: `TTL이 만료되기 전에는 원본이 바뀌어도 엣지가 옛 파일을 계속 내보낸다.`
- `cloudfront-s3-upload-with-oac` — 단언이 잡고 있는 말: `CloudFront는`, `CloudFront는`, `CloudFront는`

**문항 27개**

`q082` `q192` `q193` `q194` `q198` `q467` `q468` `q469` `q470` `q471` `q472` `q473` `q474` `q475` `q476` `q477` `q478` `q479` `q480` `q481` `q482` `q483` `q484` `q485` `q486` `q487` `q488`
...
- `q192` — 단언이 잡고 있는 말: `실시간 게임 서버처럼 HTTP가 아닌 트래픽을 전 세계 사용자에게 빠르게 전달해야 하는 상황이다. 리전…`
- `q198` — 단언이 잡고 있는 말: `로그인 과정의 지연까지 줄이려고 권한 부여 로직을 사용자와 가장 가까운 곳에서 처리하려는 상황이다. C…`
- `q468` — 단언이 잡고 있는 말: `가중치 기반 라우팅`
...
node scripts/content-audit.mjs cloudfront-global-accelerator
```
