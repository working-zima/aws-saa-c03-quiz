# Step 18: kinesis-and-redshift-opensearch
...
| 주제 | 개념 | 문항 | 범위 |
|---|---|---|---|
| `kinesis-streaming` | 16 | 17 | 개념·문항 전부 |
| `redshift-opensearch-quicksight` | 11 | 11 | 개념·문항 전부 |

### `kinesis-streaming`

**개념 16개. 문장형 제목 5개.** 아래 목록에서 `▸`가 문장형이다.

```
   0. kinesis-data-streams                           Kinesis Data Streams
   1. data-firehose                                  Data Firehose
   2. managed-service-apache-flink                   Managed Service for Apache Flink
   3. kinesis-video-streams                          Amazon Kinesis Video Streams
   4. msk                                            Amazon MSK (Managed Streaming for Apache Kafka)
   5. streaming-services-comparison                  Data Firehose vs Kinesis Data Streams vs Managed Service for Apache Flink
▸  6. flink-kinesis-source-sink                      Flink는 스트림 사이에 끼워 넣는다
▸  7. firehose-lambda-transformation                 Firehose가 적재 전에 Lambda로 데이터를 고친다
▸  8. firehose-format-conversion                     Firehose가 적재하면서 형식을 바꾼다
   9. kinesis-retention-and-fanout                   보존 기간과 향상된 팬아웃
  10. kinesis-client-library                         샤드와 체크포인트를 직접 다루는 소비자
  11. msk-kafka-connect                              MSK가 수집과 변환까지 맡는 방식
  12. kinesis-record-size-limit                      스트림에 넣을 수 있는 레코드 크기
▸ 13. kinesis-partition-key-hot-shard                파티션 키가 쏠리면 핫 샤드가 생긴다
  14. kinesis-capacity-mode                          Kinesis 스트림의 프로비저닝 모드와 온디맨드 모드
▸ 15. firehose-buffering                             Firehose는 모아서 보낸다
```

**`summary`에 옛 어휘가 든 개념**

- `kinesis-video-streams` — 자리

**`data.test.ts`가 글자로 고정한 제목·요약** — 고치려면 단언의 문구도 함께 맞춰야 한다.
**단언의 뜻은 바꾸지 마라.**

- `kinesis-data-streams` — 단언이 잡고 있는 말: `Kinesis Data Streams는`
- `data-firehose` — 단언이 잡고 있는 말: `Data Firehose는`

**문항 17개**

`q128` `q202` `q633` `q634` `q635` `q636` `q637` `q638` `q639` `q640` `q641` `q642` `q643` `q644` `q645` `q646` `q647`
...
```
   ...
   2. opensearch-text-search                         Amazon OpenSearch Service
   3. quicksight                                     Amazon QuickSight
   4. oltp-vs-olap                                   OLTP와 OLAP
   5. athena-vs-redshift-workload                    임시 쿼리와 반복되는 고성능 쿼리
▸  6. redshift-hot-cold-split                        자주 쓰는 데이터만 웨어하우스에 넣는다
   7. quicksight-ml-forecast                         QuickSight에 들어 있는 ML 예측
   8. redshift-concurrency-scaling                   Redshift 동시성 확장
▸  9. redshift-copy-from-s3                          Redshift는 S3에서 COPY로 병렬 적재한다
▸ 10. dynamodb-to-s3-analytics                       운영 테이블의 과거 데이터는 S3로 흘려보낸다
```

**`summary`에 옛 어휘가 든 개념**

- `redshift-spectrum` — 자리

**`data.test.ts`가 글자로 고정한 제목·요약** — 고치려면 단언의 문구도 함께 맞춰야 한다.
**단언의 뜻은 바꾸지 마라.**

- `redshift` — 단언이 잡고 있는 말: `RedShift는`
- `opensearch-text-search` — 단언이 잡고 있는 말: `DynamoDB는`

**문항 11개**

`q260` `q261` `q262` `q263` `q264` `q265` `q266` `q267` `q268` `q269` `q270`
...
node scripts/content-audit.mjs kinesis-streaming redshift-opensearch-quicksight
```
