...
| 주제 | 개념 | 문항 | 범위 |
|---|---|---|---|
| `efs-fsx` | 25 | 32 | 개념·문항 전부 |

### `efs-fsx`

**개념 25개. 문장형 제목 14개.** 아래 목록에서 `▸`가 문장형이다.
...
- `efs-performance-modes` — 축이 — "EFS의 성능 모드는 처리량 모드와 다른 축이다"

**`summary`에 옛 어휘가 든 개념**

- `fsx-lustre-sub-millisecond-latency` — 갈린
- `sql-server-always-on-shared-storage` — 자리
- `efs-ia-file-size-threshold` — 자리

**`data.test.ts`가 글자로 고정한 제목·요약** — 고치려면 단언의 문구도 함께 맞춰야 한다.
**단언의 뜻은 바꾸지 마라.**

- `efs-lifecycle-management` — 단언이 잡고 있는 말: `EFS 수명 주기 관리`

**문항 32개**

`q042` `q043` `q045` `q046` `q047` `q048` `q177` `q178` `q327` `q328` `q329` `q330` `q331` `q332` `q333` `q334` `q335` `q336` `q337` `q338` `q339` `q340` `q341` `q342` `q343` `q344` `q345` `q346` `q347` `q348` `q349` `q350`
...
- `q043` — 단언이 잡고 있는 말: `EFS IA(Infrequent Access)는 접근이 뜸해진 파일을 옮겨 두는 EFS의 스토리지 클래…`
- `q177` — 단언이 잡고 있는 말: `EFS에 오래 방치된 파일의 보관 비용을 줄이면서도 필요할 때는 즉시 읽을 수 있어야 하는 상황이다. …`, `EFS 수명 주기 관리`
...
node scripts/content-audit.mjs efs-fsx
```
