# Step 13: sqs-sns-eventbridge-concepts
...
| 주제 | 개념 | 문항 | 범위 |
|---|---|---|---|
| `sqs-sns-eventbridge` | 33 | — | 개념만 (`name`·`summary`) |

### `sqs-sns-eventbridge`

**개념 33개. 문장형 제목 14개.** 아래 목록에서 `▸`가 문장형이다.
...
- `eventbridge-event-bus-types` — 자리

**`data.test.ts`가 글자로 고정한 제목·요약** — 고치려면 단언의 문구도 함께 맞춰야 한다.
**단언의 뜻은 바꾸지 마라.**

- `eventbridge` — 단언이 잡고 있는 말: `EventBridge는`
- `eventbridge-vs-step-functions` — 단언이 잡고 있는 말: `Step Functions는`, `EventBridge는`
- `sqs-details` — 단언이 잡고 있는 말: `중복과 순서 뒤바뀜이 생길 수 있다`
...
node scripts/content-audit.mjs sqs-sns-eventbridge
```
