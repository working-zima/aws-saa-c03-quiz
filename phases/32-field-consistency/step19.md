# Step 19: cloudwatch-xray-and-secrets-encryption
...
| 주제 | 개념 | 문항 | 범위 |
|---|---|---|---|
| `cloudwatch-xray` | 11 | 11 | 개념·문항 전부 |
| `secrets-encryption` | 20 | 23 | 개념·문항 전부 |

### `cloudwatch-xray`

**개념 11개. 문장형 제목 3개.** 아래 목록에서 `▸`가 문장형이다.

```
   0. cloudwatch                                     CloudWatch
   1. x-ray                                          X-Ray
   2. performance-insight                            Performance Insight
   3. amazon-managed-grafana                         Amazon Managed Grafana
   4. cloudwatch-network-monitor                     CloudWatch Network Monitor
   5. cloudwatch-container-insights                  CloudWatch Container Insights
   6. log-analysis-options                           로그 분석 선택지: OpenSearch와 CloudWatch Logs Insights
   7. performance-insights-rightsizing               Performance Insights와 적정 규모 조정
▸  8. cloudwatch-agent-memory-metric                 메모리 사용률은 기본 지표가 아니다
▸  9. ec2-detailed-monitoring                        상세 모니터링은 1분 간격이다
▸ 10. cloudwatch-alarm-state-change-event            알람 상태 변경은 이벤트로 흘러간다
```

**`summary`에 옛 어휘가 든 개념**

- `amazon-managed-grafana` — 자리
- `performance-insights-rightsizing` — 자리

**`data.test.ts`가 글자로 고정한 제목·요약** — 고치려면 단언의 문구도 함께 맞춰야 한다.
**단언의 뜻은 바꾸지 마라.**

- `cloudwatch` — 단언이 잡고 있는 말: `CloudWatch는`

**문항 11개**

`q126` `q223` `q648` `q649` `q650` `q651` `q652` `q653` `q654` `q655` `q656`
...
- `q650` — 단언이 잡고 있는 말: `Managed Grafana는`
- `q654` — 단언이 잡고 있는 말: `대상 추적 정책`

### `secrets-encryption`

**개념 20개. 문장형 제목 3개.** 아래 목록에서 `▸`가 문장형이다.

```
   0. secrets-manager                                Secrets Manager
   1. parameter-store                                Systems Manager Parameter Store
   2. kms                                            KMS (Key Management Service)
   3. acm                                            ACM (AWS Certificate Manager)
   4. cloudhsm                                       AWS CloudHSM
   5. secrets-manager-vs-parameter-store             Secrets Manager vs Systems Manager Parameter Store
   6. rotation-heuristic                             자동 순환이 나오면 Secrets Manager
   7. kms-key-types-by-management                    KMS 키의 관리 주체: 고객 관리 키·AWS 관리 키·AWS 소유 키
   8. kms-multi-region-key                           KMS 다중 리전 키
   9. kms-imported-key-material                      KMS의 가져온 키 자료
  10. kms-cloudhsm-key-store                         CloudHSM이 뒷받침하는 KMS 키
▸ 11. kms-key-per-tenant                             KMS 키를 테넌트마다 따로 만든다
  12. acm-dns-validation                             ACM의 도메인 검증 방식
  13. secrets-manager-batch-get-secret-value         Secrets Manager의 BatchGetSecretValue API
▸ 14. kms-automatic-key-rotation                     KMS 자동 키 교체는 해마다 돈다
  15. kms-symmetric-vs-asymmetric-rotation           KMS 대칭 키와 비대칭 키의 자동 교체
  16. imported-key-material-rotation                 KMS의 가져온 키 자료를 교체하는 방법
  17. lambda-env-var-kms                             Lambda 환경 변수의 KMS 암호화
▸ 18. acm-cloudfront-region                          CloudFront에 붙일 인증서는 us-east-1에서 발급한다
  19. acm-expiration-event                           ACM 인증서 만료 임박 이벤트
```

**`summary`에 옛 어휘가 든 개념**

- `kms-key-types-by-management` — 갈리

**문항 23개**

`q138` `q139` `q140` `q141` `q142` `q143` `q144` `q145` `q227` `q228` `q229` `q230` `q657` `q658` `q659` `q660` `q661` `q662` `q663` `q664` `q665` `q666` `q667`

**`data.test.ts`가 글자로 고정한 프롬프트·보기** — 고치려면 단언의 문구도 함께 맞춰야 한다.
**단언의 뜻은 바꾸지 마라.**

- `q142` — 단언이 잡고 있는 말: `KMS가 저장하는 대상으로 알맞은 것은 무엇인가?`
- `q144` — 단언이 잡고 있는 말: `Secrets Manager와 Parameter Store의 공통 기능은 무엇인가?`
- `q227` — 단언이 잡고 있는 말: `S3에 올린 정적 사이트를 CloudFront로 서비스하고 있다. 사용자 지정 도메인에 HTTPS를 적…`
...
node scripts/content-audit.mjs cloudwatch-xray secrets-encryption
```
