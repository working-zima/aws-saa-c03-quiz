...
| 주제 | 개념 | 문항 | 범위 |
|---|---|---|---|
| `elastic-load-balancing` | 16 | 20 | 개념·문항 전부 |

### `elastic-load-balancing`

**개념 16개. 문장형 제목 9개.** 아래 목록에서 `▸`가 문장형이다.
...
- `alb-l7-vs-nlb-l4` — 갈리 — "계층으로 갈리는 ALB와 NLB"

**`summary`에 옛 어휘가 든 개념**

- `alb-listener-rule-fixed-response` — 자리
- `gwlb-endpoint-cross-account-inspection` — 자리

**문항 20개**

`q078` `q079` `q080` `q081` `q190` `q191` `q453` `q454` `q455` `q456` `q457` `q458` `q459` `q460` `q461` `q462` `q463` `q464` `q465` `q466`
...
node scripts/content-audit.mjs elastic-load-balancing
```
