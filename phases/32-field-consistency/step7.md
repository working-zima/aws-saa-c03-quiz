...
| 주제 | 개념 | 문항 | 범위 |
|---|---|---|---|
| `ec2-autoscaling` | 18 | 24 | 개념·문항 전부 |

### `ec2-autoscaling`

**개념 18개. 문장형 제목 4개.** 아래 목록에서 `▸`가 문장형이다.
...
- `gpu-instance-family` — 갈린 — "GPU가 필요하면 서비스 자체가 갈린다"

**`summary`에 옛 어휘가 든 개념**

- `ami-and-launch-template` — 띄우
- `predictive-scaling` — 띄우
- `asg-single-instance-self-healing` — 띄운

**`data.test.ts`가 글자로 고정한 제목·요약** — 고치려면 단언의 문구도 함께 맞춰야 한다.
**단언의 뜻은 바꾸지 마라.**

- `scheduled-scaling` — 단언이 잡고 있는 말: `예약된 조정과 대상 추적`

**문항 24개**

`q074` `q075` `q076` `q077` `q188` `q189` `q435` `q436` `q437` `q438` `q439` `q440` `q441` `q442` `q443` `q444` `q445` `q446` `q447` `q448` `q449` `q450` `q451` `q452`
...
- `q075` — 단언이 잡고 있는 말: `평균 CPU 사용률 70% 같은 목표값을 유지하도록 서버 수를 자동으로 늘리거나 줄이는 정책은 무엇인가?`, `대상 추적 정책`
- `q076` — 단언이 잡고 있는 말: `온디맨드 인스턴스`
- `q077` — 단언이 잡고 있는 말: `온디맨드 인스턴스`
- `q188` — 단언이 잡고 있는 말: `트래픽이 급증할 때 새 EC2가 부팅되고 애플리케이션이 시작될 때까지 응답이 지연되는 상황이다. 실행 …`
- `q439` — 단언이 잡고 있는 말: `온디맨드 인스턴스`
- `q440` — 단언이 잡고 있는 말: `온디맨드 인스턴스`
- `q442` — 단언이 잡고 있는 말: `대상 추적 정책`
- `q448` — 단언이 잡고 있는 말: `온디맨드 인스턴스`
- `q451` — 단언이 잡고 있는 말: `클러스터 배치 그룹`
- `q452` — 단언이 잡고 있는 말: `클러스터 배치 그룹`
...
node scripts/content-audit.mjs ec2-autoscaling
```
