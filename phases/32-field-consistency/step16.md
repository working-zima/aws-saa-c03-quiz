# Step 16: hybrid-connectivity-and-route53
...
| 주제 | 개념 | 문항 | 범위 |
|---|---|---|---|
| `hybrid-connectivity` | 19 | 24 | 개념·문항 전부 |
| `route53` | 13 | 16 | 개념·문항 전부 |

### `hybrid-connectivity`

**개념 19개. 문장형 제목 5개.** 아래 목록에서 `▸`가 문장형이다.

```
   0. site-to-site-vpn                               Site-to-Site VPN
   1. client-vpn                                     AWS Client VPN
   2. direct-connect                                 Direct Connect
   3. virtual-private-gateway                        가상 프라이빗 게이트웨이(VGW)
   4. access-terms                                   Customer Gateway와 Bastion Host
   5. transit-gateway                                Transit Gateway
   6. direct-connect-gateway                         Direct Connect Gateway
▸  7. region-attached-edge-options                   Local Zone·Outposts·Wavelength Zone은 리전 연결이 전제다
   8. onprem-connectivity-heuristic                  온프레미스 연결 문제의 출발점
   9. vpn-vs-direct-connect                          Site-to-Site VPN vs Direct Connect
▸ 10. per-vpc-vpn-for-isolation                      VPC끼리 막아야 하면 연결을 VPC마다 따로 건다
  11. transit-gateway-cross-region-peering           리전 간 Transit Gateway 피어링
▸ 12. onprem-access-via-interface-endpoint           온프레미스에서 인터페이스 엔드포인트로 들어간다
  13. data-locality-cost                             데이터 지역성으로 전송 비용 줄이기
▸ 14. outposts-data-residency                        데이터를 옮기지 않고 관리형 서비스를 데이터 쪽으로 옮긴다
  15. direct-connect-caveats                         Direct Connect의 두 가지 함정
  16. direct-connect-resiliency                      Direct Connect 최대 복원력 구성
  17. direct-connect-vif-types                       Direct Connect의 가상 인터페이스(VIF)
▸ 18. centralized-onprem-egress                      아웃바운드 인터넷을 온프레미스로 되돌린다
```

**`summary`에 옛 어휘가 든 개념**

- `outposts-data-residency` — 자리

**`data.test.ts`가 글자로 고정한 제목·요약** — 고치려면 단언의 문구도 함께 맞춰야 한다.
**단언의 뜻은 바꾸지 마라.**

- `site-to-site-vpn` — 단언이 잡고 있는 말: `Site-to-Site VPN은`
- `direct-connect` — 단언이 잡고 있는 말: `Direct Connect는`
- `transit-gateway` — 단언이 잡고 있는 말: `Transit Gateway는`

**문항 24개**

`q109` `q110` `q111` `q112` `q113` `q114` `q115` `q116` `q214` `q215` `q216` `q217` `q218` `q219` `q599` `q600` `q601` `q602` `q603` `q604` `q605` `q606` `q607` `q608`
...
- `q114` — 단언이 잡고 있는 말: `Site-to-Site VPN은 인터넷에 암호화 터널을 구성한다. Direct Connect는 전용선을…`, `Site-to-Site VPN은`, `Direct Connect는`
- `q115` — 단언이 잡고 있는 말: `높은 보안성과 대역폭, 빠른 성능을 함께 제공하는 Direct Connect의 통신 기반은 무엇인가?`
- `q215` — 단언이 잡고 있는 말: `일관되고 낮은 지연 시간과 수백 개 VPC 연결이 동시에 필요한 하이브리드 네트워크를 구성하려는 상황이…`
- `q217` — 단언이 잡고 있는 말: `Site-to-Site VPN은 인터넷에 암호화된 터널을 만들어 온프레미스와 AWS를 연결한다. 이 연…`, `Site-to-Site VPN은`
- `q219` — 단언이 잡고 있는 말: `인터넷 게이트웨이`
- `q608` — 단언이 잡고 있는 말: `인터넷 게이트웨이`

### `route53`

**개념 13개. 문장형 제목 3개.** 아래 목록에서 `▸`가 문장형이다.

```
   0. route53                                        Route53
   1. routing-policies                               Route53 라우팅 정책
   2. resolver                                       Route53 Resolver
   3. private-hosted-zone                            프라이빗 호스팅 영역과 퍼블릭 호스팅 영역
   4. route53-zone-file-import                       DNS 호스팅을 Route 53으로 옮기는 절차
   5. route53-failover-routing                       페일오버 라우팅 정책
▸  6. multi-region-failover-for-region-outage        리전 장애는 가용 영역으로 막지 못한다
▸  7. latency-record-for-non-aws-endpoint            온프레미스 엔드포인트에도 지연 시간 레코드를 만든다
   8. route53-alias-record                           별칭 레코드
▸  9. private-hosted-zone-vpc-only                   프라이빗 호스팅 영역은 VPC에만 연결한다
  10. route53-resolver-forward-rule                  전달 규칙과 VPC 연결
  11. multivalue-answer-details                      다중값 응답 라우팅의 세부 동작
  12. route53-query-logging                          Route 53 쿼리 로깅
```

**`data.test.ts`가 글자로 고정한 제목·요약** — 고치려면 단언의 문구도 함께 맞춰야 한다.
**단언의 뜻은 바꾸지 마라.**

- `route53` — 단언이 잡고 있는 말: `Route 53은`
- `routing-policies` — 단언이 잡고 있는 말: `Route 53은`

**문항 16개**

`q117` `q118` `q119` `q120` `q121` `q122` `q220` `q221` `q609` `q610` `q611` `q612` `q613` `q614` `q615` `q616`

**`data.test.ts`가 글자로 고정한 프롬프트·보기** — 고치려면 단언의 문구도 함께 맞춰야 한다.
**단언의 뜻은 바꾸지 마라.**

- `q119` — 단언이 잡고 있는 말: `가중치 기반 라우팅`
- `q120` — 단언이 잡고 있는 말: `가중치 기반 라우팅`
- `q122` — 단언이 잡고 있는 말: `인터넷 게이트웨이`
- `q221` — 단언이 잡고 있는 말: `다중값 응답 라우팅은 각 레코드에 연동된 헬스 체크로 장애가 난 곳을 빼고 무작위로 응답한다. 이 정책…`
...
node scripts/content-audit.mjs hybrid-connectivity route53
```
