# Step 22: organizations-cloudtrail-and-cost-management
...
| 주제 | 개념 | 문항 | 범위 |
|---|---|---|---|
| `organizations-cloudtrail-config` | 16 | 18 | 개념·문항 전부 |
| `cost-management` | 17 | 19 | 개념·문항 전부 |

### `organizations-cloudtrail-config`

**개념 16개. 문장형 제목 2개.** 아래 목록에서 `▸`가 문장형이다.

```
   0. organizations-scp                              AWS Organizations와 SCP
   1. cloudtrail                                     CloudTrail
   2. cloudtrail-lake                                AWS CloudTrail Lake
   3. aws-config                                     AWS Config
   4. audit-manager                                  AWS Audit Manager
   5. organizational-unit                            조직 단위(OU)
▸  6. organizations-tag-policy                       태그 정책과 SCP는 하는 일이 다르다
   7. organizations-consolidated-billing             Organizations의 통합 청구
   8. cloudtrail-data-events                         관리 이벤트와 데이터 이벤트
   9. config-configuration-recorder                  구성 레코더
  10. scp-attachment-targets                         SCP를 붙일 수 있는 자리
  11. scp-condition-exception                        SCP에서 예외를 두는 방법
  12. cloudtrail-log-file-validation                 CloudTrail 로그 파일 유효성 검사
  13. config-conformance-pack                        AWS Config 준수 팩
▸ 14. config-custom-rule                             관리형 규칙에 없는 값은 사용자 지정 규칙으로 본다
  15. config-rule-remediation                        AWS Config 규칙과 자동 수정
```

**제목에 옛 어휘가 든 것** (문장형이 아니어도 본다)

- `scp-attachment-targets` — 자리 — "SCP를 붙일 수 있는 자리"

**`summary`에 옛 어휘가 든 개념**

- `scp-attachment-targets` — 자리

**문항 18개**

`q162` `q163` `q242` `q709` `q710` `q711` `q712` `q713` `q714` `q715` `q716` `q717` `q718` `q719` `q720` `q721` `q722` `q723`
...
**개념 17개. 문장형 제목 6개.** 아래 목록에서 `▸`가 문장형이다.
...
- `rds-reserved-instance` — 돌린
- `compute-optimizer-ebs-recommendations` — 자리

**`data.test.ts`가 글자로 고정한 제목·요약** — 고치려면 단언의 문구도 함께 맞춰야 한다.
**단언의 뜻은 바꾸지 마라.**

- `cost-allocation-tag-activation` — 단언이 잡고 있는 말: `비용 할당 태그는 결제 콘솔에서 활성화해야 Cost Explorer에 보인다.`

**문항 19개**

`q164` `q165` `q166` `q167` `q168` `q169` `q243` `q244` `q245` `q246` `q724` `q725` `q726` `q727` `q728` `q729` `q730` `q731` `q732`

**`data.test.ts`가 글자로 고정한 프롬프트·보기** — 고치려면 단언의 문구도 함께 맞춰야 한다.
**단언의 뜻은 바꾸지 마라.**

- `q164` — 단언이 잡고 있는 말: `온디맨드 인스턴스`
- `q243` — 단언이 잡고 있는 말: `부서별로 비용을 나눠 보려고 리소스에 사용자 정의 태그를 붙인 상황이다. Cost Explorer에서 …`
...
node scripts/content-audit.mjs organizations-cloudtrail-config cost-management
```
