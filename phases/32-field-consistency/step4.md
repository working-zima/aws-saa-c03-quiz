...
| 주제 | 개념 | 문항 | 범위 |
|---|---|---|---|
| `rds-storage-features` | 21 | 28 | 개념·문항 전부 |

### `rds-storage-features`

**개념 21개. 문장형 제목 8개.** 아래 목록에서 `▸`가 문장형이다.
...
- `rds-encrypt-existing-instance` — 자리 — "이미 만든 인스턴스는 그 자리에서 암호화하지 못한다"

**`summary`에 옛 어휘가 든 개념**

- `rds-blue-green-deployment` — 띄워

**`data.test.ts`가 글자로 고정한 제목·요약** — 고치려면 단언의 문구도 함께 맞춰야 한다.
**단언의 뜻은 바꾸지 마라.**

- `automated-backup-retention` — 단언이 잡고 있는 말: `RDS 자동 백업`, `RDS 자동 백업`

**문항 28개**

`q058` `q059` `q060` `q061` `q062` `q063` `q064` `q065` `q179` `q180` `q181` `q182` `q375` `q376` `q377` `q378` `q379` `q380` `q381` `q382` `q383` `q384` `q385` `q386` `q387` `q388` `q389` `q390`
...
- `q179` — 단언이 잡고 있는 말: `쓰기 작업량이 급격히 치솟는 RDS 워크로드를 운영하는 상황이다. 지연 시간을 보장하려면 IOPS를 직…`
- `q180` — 단언이 잡고 있는 말: `RDS 다중 AZ 배포의 대기 인스턴스로 할 수 없는 작업은 무엇인가?`
- `q181` — 단언이 잡고 있는 말: `규정 준수를 위해 백업을 얼마나 오래 보관할 수 있는지부터 확인하려는 상황이다. RDS 자동 백업의 최…`, `RDS 자동 백업`
- `q379` — 단언이 잡고 있는 말: `다중 AZ DB 인스턴스 배포`
- `q380` — 단언이 잡고 있는 말: `다중 AZ DB 인스턴스 배포`
...
node scripts/content-audit.mjs rds-storage-features
```
