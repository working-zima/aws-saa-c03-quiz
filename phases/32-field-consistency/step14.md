# Step 14: sqs-sns-eventbridge-questions-and-backup-dr
...
| 주제 | 개념 | 문항 | 범위 |
|---|---|---|---|
| `sqs-sns-eventbridge` | — | 40 | 문항만 (`prompt`·`choices`) |
| `backup-disaster-recovery` | 11 | 11 | 개념·문항 전부 |

### `sqs-sns-eventbridge`

**문항 40개**

`q091` `q092` `q093` `q094` `q095` `q096` `q097` `q099` `q203` `q204` `q205` `q207` `q549` `q550` `q551` `q552` `q553` `q554` `q555` `q556` `q557` `q558` `q559` `q560` `q561` `q562` `q563` `q564` `q565` `q566` `q567` `q568` `q569` `q570` `q571` `q572` `q573` `q574` `q575` `q576`
...
- `q555` — 단언이 잡고 있는 말: `24시간 넘게 들고 있지 않는다`

### `backup-disaster-recovery`

**개념 11개. 문장형 제목 4개.** 아래 목록에서 `▸`가 문장형이다.
...
- `elastic-disaster-recovery` — 띄우

**`data.test.ts`가 글자로 고정한 제목·요약** — 고치려면 단언의 문구도 함께 맞춰야 한다.
**단언의 뜻은 바꾸지 마라.**

- `backup` — 단언이 잡고 있는 말: `AWS Backup은`
- `backup-s3-continuous-backup` — 단언이 잡고 있는 말: `AWS Backup은`

**문항 11개**

`q098` `q208` `q577` `q578` `q579` `q580` `q581` `q582` `q583` `q584` `q585`
...
- `q098` — 단언이 잡고 있는 말: `RDS 자동 백업`, `S3 버전 관리`
- `q208` — 단언이 잡고 있는 말: `RDS 자동 백업`
- `q577` — 단언이 잡고 있는 말: `디스크 변경을 지속적으로 복제`
- `q579` — 단언이 잡고 있는 말: `상태 점검을 건 DNS 장애 조치`
...
node scripts/content-audit.mjs sqs-sns-eventbridge backup-disaster-recovery
```
