# Step 15: vpc-networking-and-security-groups
...
| 주제 | 개념 | 문항 | 범위 |
|---|---|---|---|
| `vpc-networking` | 20 | 22 | 개념·문항 전부 |
| `security-groups-nacl` | 9 | 17 | 개념·문항 전부 |

### `vpc-networking`

**개념 20개. 문장형 제목 6개.** 아래 목록에서 `▸`가 문장형이다.
...
- `internet-gateway` — 단언이 잡고 있는 말: `인터넷 게이트웨이`, `인터넷 게이트웨이`
- `egress-only-igw` — 단언이 잡고 있는 말: `인터넷 게이트웨이`
- `privatelink` — 단언이 잡고 있는 말: `PrivateLink는`
- `internet-gateway-is-not-per-az` — 단언이 잡고 있는 말: `인터넷 게이트웨이`, `인터넷 게이트웨이`
- `nat-gateway-elastic-ip` — 단언이 잡고 있는 말: `인터넷 게이트웨이`

**문항 22개**

`q100` `q101` `q102` `q103` `q104` `q105` `q106` `q107` `q108` `q209` `q210` `q211` `q212` `q213` `q586` `q587` `q588` `q589` `q590` `q591` `q592` `q593`
...
- `q100` — 단언이 잡고 있는 말: `인터넷 게이트웨이`
- `q101` — 단언이 잡고 있는 말: `인터넷 게이트웨이`
- `q102` — 단언이 잡고 있는 말: `인터넷 게이트웨이`
- `q103` — 단언이 잡고 있는 말: `NAT 게이트웨이는 외부에서 시작하는 접근은 막고 내부에서 인터넷으로 나가는 통신만 허용하는 장치다. …`
- `q105` — 단언이 잡고 있는 말: `인터넷 게이트웨이`
- `q107` — 단언이 잡고 있는 말: `인터넷 게이트웨이`
- `q209` — 단언이 잡고 있는 말: `인터넷 게이트웨이`
- `q210` — 단언이 잡고 있는 말: `Egress-only 인터넷 게이트웨이는 NAT 게이트웨이처럼 안에서 밖으로 나가는 통신만 담당한다. …`, `인터넷 게이트웨이`
- `q213` — 단언이 잡고 있는 말: `S3는 VPC나 서브넷 안에 만들 수 없는 리전 수준 서비스다. 이런 S3 버킷의 접근을 제어할 때 사…`
- `q587` — 단언이 잡고 있는 말: `인터넷 게이트웨이`
- `q589` — 단언이 잡고 있는 말: `인터넷 게이트웨이`
- `q590` — 단언이 잡고 있는 말: `인터넷 게이트웨이`
- `q591` — 단언이 잡고 있는 말: `인터넷 게이트웨이`
- `q592` — 단언이 잡고 있는 말: `인터넷 게이트웨이`

### `security-groups-nacl`

**개념 9개. 문장형 제목 3개.** 아래 목록에서 `▸`가 문장형이다.
...
`q129` `q130` `q131` `q132` `q133` `q134` `q135` `q136` `q137` `q224` `q225` `q226` `q594` `q595` `q596` `q597` `q598`

**`data.test.ts`가 글자로 고정한 프롬프트·보기** — 고치려면 단언의 문구도 함께 맞춰야 한다.
**단언의 뜻은 바꾸지 마라.**

- `q130` — 단언이 잡고 있는 말: `인터넷 게이트웨이`
- `q131` — 단언이 잡고 있는 말: `보안 그룹은 리소스에 도달하려는 요청을 문 앞에서 검사하는 방화벽이다. 규칙을 하나도 추가하지 않은 초…`
- `q132` — 단언이 잡고 있는 말: `보안 그룹은 리소스에서 밖으로 나가는 트래픽에도 규칙을 적용한다. 규칙을 하나도 추가하지 않은 초기 상…`
- `q133` — 단언이 잡고 있는 말: `보안 그룹은 리소스에 도달하려는 요청을 문 앞에서 검사하는 방화벽이다. 이 보안 그룹의 규칙으로 지정할…`
- `q134` — 단언이 잡고 있는 말: `NACL은 개별 리소스가 아니라 서브넷 경계에서 트래픽을 통제한다. 규칙을 손대지 않은 초기 상태에서 …`
- `q135` — 단언이 잡고 있는 말: `NACL은 서브넷 경계에서 트래픽을 허용하거나 거부하는 기능이다. 이 규칙을 정할 때 대상으로 지정할 …`
- `q224` — 단언이 잡고 있는 말: `인터넷 게이트웨이`
- `q225` — 단언이 잡고 있는 말: `NACL은 규칙 수에 제한이 있어 매장 5만 곳의 IP를 개별 등록할 수 없다. 이 매장들의 IP에서 …`
- `q226` — 단언이 잡고 있는 말: `Web ACL과 네트워크 ACL의 역할을 올바르게 설명한 것은 무엇인가?`
...
node scripts/content-audit.mjs vpc-networking security-groups-nacl
```
