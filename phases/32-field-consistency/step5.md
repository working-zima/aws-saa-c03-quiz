...
- `read-replica-no-schema-change` — 자리

**문항 21개**

`q066` `q067` `q068` `q183` `q184` `q391` `q392` `q393` `q394` `q395` `q396` `q397` `q398` `q399` `q400` `q401` `q402` `q403` `q404` `q405` `q406`

**`data.test.ts`가 글자로 고정한 프롬프트·보기** — 고치려면 단언의 문구도 함께 맞춰야 한다.
**단언의 뜻은 바꾸지 마라.**

- `q066` — 단언이 잡고 있는 말: `Aurora를 가장 잘 설명한 것은 무엇인가?`
- `q183` — 단언이 잡고 있는 말: `플레이어 수가 시시각각 바뀌어 용량을 예측하기 어려운 게임 서버에 관계형 데이터베이스가 필요한 상황이다…`
- `q184` — 단언이 잡고 있는 말: `Aurora에 읽기 전용 복제본을 여러 개 두려는 상황이다. 애플리케이션이 어느 복제본으로 요청을 보낼…`
...
node scripts/content-audit.mjs aurora
```
