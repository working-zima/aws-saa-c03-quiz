# Step 22: waf-shield-and-guardduty-macie-inspector
...
|---|---|---|
| `waf-shield` | 15 | 20 |
| `guardduty-macie-inspector` | 11 | 12 |

### 개념 목록 (배열 순서 — 이름은 바꾸지 않는다)

`waf-shield`

 0. `waf` — WAF (Web Application Firewall)
 1. `shield` — Shield
 2. `cloudfront` — CloudFront
 3. `firewall-manager` — AWS Firewall Manager
 4. `waf-attach-targets` — WAF를 붙일 수 있는 곳
 5. `waf-rule-types` — WAF 규칙의 종류
 6. `waf-managed-rule-groups` — WAF 관리형 규칙 그룹
 7. `waf-rate-based-rule` — WAF 속도 기반 규칙
 8. `waf-bot-control` — AWS WAF Bot Control
 9. `shield-standard-network-layer` — Shield Standard가 다루지 않는 계층
10. `shield-advanced-drt` — Shield Advanced와 DRT
11. `shield-advanced-protection-group` — Shield Advanced 보호 그룹
12. `waf-body-inspection-size-limit` — WAF가 검사하는 요청 본문에는 크기 한도가 있다
13. `waf-web-acl-region-must-match-rest-api` — REST API에 붙일 Web ACL은 같은 리전에 만든다
14. `waf-logging-to-firehose` — WAF 로그는 Firehose를 거쳐 S3로 간다

`guardduty-macie-inspector`

 0. `guardduty` — GuardDuty
 1. `macie` — Macie
 2. `amazon-inspector` — Amazon Inspector
 3. `security-hub` — AWS Security Hub
 4. `security-service-lineup` — 헷갈리기 쉬운 보안 서비스 네 가지
 5. `guardduty-db-login` — GuardDuty의 데이터베이스 로그인 이상 탐지
 6. `macie-automated-discovery` — Macie의 민감 데이터 자동 탐지
 7. `inspector-scans-ecr-images` — Inspector의 ECR 컨테이너 이미지 스캔
 8. `macie-delegated-administrator` — Macie는 위임 관리자 계정에서 조직 전체를 본다
 9. `guardduty-finding-to-eventbridge` — GuardDuty 탐지 결과를 자동 대응으로 잇는다
10. `macie-finding-to-eventbridge` — Macie의 탐지 결과에 알림을 붙인다

### 문항 id

`waf-shield` — `q146` `q147` `q148` `q149` `q152` `q153` `q154` `q231` `q232` `q233` `q234` `q668` `q669` `q670` `q671` `q672` `q673` `q674` `q675` `q676`

`guardduty-macie-inspector` — `q150` `q151` `q235` `q236` `q677` `q678` `q679` `q680` `q681` `q682` `q683` `q684`
...
const T=['waf-shield','guardduty-macie-inspector'];
