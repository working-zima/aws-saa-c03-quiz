...
`efs-fsx` — 개념 25개. **①이 걸린 것은 굵게 표시했다.**

 0. `efs` — EFS (Elastic File System)
 1. `efs-lifecycle-management` — EFS 수명 주기 관리
 2. `fsx` — FSx (File System for Extended use)
 3. `fsx-windows-file-server` — Amazon FSx for Windows File Server
 4. `fsx-for-lustre` — Amazon FSx for Lustre
 5. `fsx-file-gateway` — Amazon FSx File Gateway
 6. `efs-throughput-modes` — EFS의 버스팅 처리량과 프로비저닝된 처리량
 7. `efs-elastic-throughput` — EFS의 Elastic 처리량 모드
 8. `efs-performance-modes` — **EFS의 성능 모드는 처리량 모드와 다른 축이다**
 9. `efs-one-zone` — EFS One Zone
10. `efs-posix-permissions` — **파일 권한 모델이 그대로 따라온다**
11. `fsx-lustre-sub-millisecond-latency` — **1ms 이내라는 조건이 붙으면 Lustre다**
12. `fsx-lustre-persistent-deployment` — **영구 파일 시스템이 내구성과 가용성을 맡는다**
13. `fsx-ontap-multi-az` — FSx for NetApp ONTAP의 다중 AZ 배포
14. `fsx-ontap-multi-protocol-tiering` — **ONTAP은 NFS와 SMB를 함께 받고 계층을 자동으로 나눈다**
15. `fsx-ontap-iscsi-block` — **ONTAP은 iSCSI로 블록 스토리지를 내준다**
16. `fsx-ontap-snapmirror` — FSx for NetApp ONTAP과 SnapMirror
17. `sql-server-always-on-shared-storage` — **SQL Server 클러스터에는 공유 스토리지가 필요하다**
18. `efs-ia-file-size-threshold` — **EFS IA는 작은 파일에는 해당하지 않는다**
19. `efs-lifecycle-transition-to-primary` — **IA로 내려간 파일을 읽으면 다시 올려 보낼 수 있다**
20. `efs-mount-target-per-az` — **마운트 대상은 가용 영역마다 하나씩 만든다**
21. `efs-cross-account-mount` — **계정을 넘어 EFS를 마운트한다**
22. `efs-replication-one-way` — **EFS 복제는 한 방향이다**
23. `fsx-windows-storage-auto-scaling` — **FSx for Windows는 용량을 자동으로 늘린다**
24. `fsx-lustre-s3-data-repository-association` — **데이터 리포지토리 연결로 S3와 잇는다**

### 문항 id

`efs-fsx` — 32개: `q042` `q043` `q045` `q046` `q047` `q048` `q177` `q178` `q327` `q328` `q329` `q330` `q331` `q332` `q333` `q334` `q335` `q336` `q337` `q338` `q339` `q340` `q341` `q342` `q343` `q344` `q345` `q346` `q347` `q348` `q349` `q350`
...
node scripts/content-audit.mjs efs-fsx# ①②④ 0건
```
