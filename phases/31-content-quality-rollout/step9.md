...
`aurora` — 개념 18개. **①이 걸린 것은 굵게 표시했다.**

 0. `aurora` — Aurora
 1. `aurora-serverless-v2` — Aurora Serverless v2
 2. `aurora-reader-endpoint` — Reader Endpoint는 Aurora 전용
 3. `aurora-endpoint-types` — Aurora의 엔드포인트 종류
 4. `aurora-replica-auto-scaling` — **Aurora Auto Scaling은 레플리카를 늘린다**
 5. `babelfish` — Babelfish for Aurora PostgreSQL
 6. `aurora-pgvector` — Aurora PostgreSQL의 pgvector 확장
 7. `aurora-select-into-outfile-s3` — Aurora MySQL에서 S3로 바로 내보내기
 8. `aurora-global-database-dr-targets` — Aurora 글로벌 데이터베이스가 채우는 RPO와 RTO
 9. `aurora-cross-region-read-replica` — 리전 간 Aurora 복제본
10. `aurora-continuous-backup-rpo` — **Aurora의 지속적 증분 백업이 RPO를 좁힌다**
11. `aurora-clone` — **Aurora 클론은 Aurora에만 있다**
12. `aurora-storage-configurations` — Aurora Standard와 Aurora I/O-Optimized
13. `sql-server-license-cost` — **SQL Server를 그대로 옮기면 라이선스 비용이 따라온다**
14. `aurora-zdr-and-activity-streams` — 확장 수단이 아닌 Aurora 기능 둘
15. `aurora-global-database-write-region` — **Aurora Global Database는 액티브-액티브가 아니다**
16. `aurora-serverless-max-acu` — **Aurora Serverless의 용량 상한은 ACU로 정한다**
17. `read-replica-no-schema-change` — **읽기 전용 복제본에서는 스키마를 바꿀 수 없다**

### 문항 id

`aurora` — 21개: `q066` `q067` `q068` `q183` `q184` `q391` `q392` `q393` `q394` `q395` `q396` `q397` `q398` `q399` `q400` `q401` `q402` `q403` `q404` `q405` `q406`
...
node scripts/content-audit.mjs aurora# ①②④ 0건
```
