...
|---|---|---|---|---|---|---|---|---|
| `ec2-autoscaling` | 18 | 24 | 4 | 10 (개념 6/문항 4) | — | — | — | — |
...
`ec2-autoscaling` — 개념 18개. **①이 걸린 것은 굵게 표시했다.**

 0. `ec2` — EC2
 1. `ami-and-launch-template` — AMI와 시작 템플릿
 2. `ec2-image-builder` — EC2 Image Builder
 3. `memory-optimized-instance-family` — 메모리 최적화 인스턴스 제품군
 4. `gpu-instance-family` — **GPU가 필요하면 서비스 자체가 갈린다**
 5. `reserved-instance-types` — 표준 예약 인스턴스와 전환 가능 예약 인스턴스
 6. `spot-workload-fit` — 스팟에 올릴 수 있는 워크로드
 7. `scheduled-scaling` — 예약된 조정과 대상 추적의 갈림길
 8. `target-tracking-vs-simple-scaling` — 대상 추적과 단순 조정의 갈림길
 9. `predictive-scaling` — 예측 스케일링
10. `spot-allocation-strategy` — 스팟 할당 전략
11. `asg-instance-type-override` — Auto Scaling 그룹의 인스턴스 유형 재정의
12. `asg-on-demand-base-capacity` — **스팟 그룹에 온디맨드 기반 용량을 섞는다**
13. `warm-pool` — Auto Scaling 웜 풀
14. `asg-single-instance-self-healing` — **인스턴스가 하나여도 오토 스케일링 그룹에 넣는다**
15. `elb-health-check-drives-asg-replacement` — **상태 검사를 어디서 하느냐가 교체를 가른다**
16. `enhanced-networking` — 향상된 네트워킹
17. `parallelcluster` — AWS ParallelCluster

### 문항 id

`ec2-autoscaling` — 24개: `q074` `q075` `q076` `q077` `q188` `q189` `q435` `q436` `q437` `q438` `q439` `q440` `q441` `q442` `q443` `q444` `q445` `q446` `q447` `q448` `q449` `q450` `q451` `q452`
...
node scripts/content-audit.mjs ec2-autoscaling# ①②④ 0건
```
