# Step 13: elastic-load-balancing
...
|---|---|---|---|---|---|---|---|---|
| `elastic-load-balancing` | 16 | 20 | 9 | 5 (개념 3/문항 2) | — | — | — | — |
...
`elastic-load-balancing` — 개념 16개. **①이 걸린 것은 굵게 표시했다.**

 0. `elb` — ELB
 1. `gateway-load-balancer` — 게이트웨이 로드 밸런서
 2. `alb-l7-vs-nlb-l4` — 계층으로 갈리는 ALB와 NLB
 3. `alb-routing-conditions` — ALB가 볼 수 있는 라우팅 조건
 4. `nlb-tls-listener` — **NLB도 TLS 연결을 받는다**
 5. `nlb-udp-listener` — **UDP를 받는 것은 NLB뿐이다**
 6. `nlb-ip-targets` — **NLB는 VPC 밖의 IP도 대상으로 받는다**
 7. `alb-cookie-stickiness` — **스티키 세션은 ALB의 기능이다**
 8. `internal-load-balancer` — 내부 로드 밸런서
 9. `sticky-session-tradeoff` — 스티키 세션의 부작용
10. `alb-least-outstanding-requests` — ALB의 분산 알고리즘
11. `alb-target-group-independent-scaling` — **경로마다 타깃 그룹과 조정 단위를 나눈다**
12. `alb-listener-rule-fixed-response` — **대상에 보내지 않고 규칙이 직접 답한다**
13. `load-balancer-idle-timeout` — **유휴 타임아웃은 경로 전체를 맞춘다**
14. `end-to-end-encryption-behind-alb` — **종단 간 암호화는 로드 밸런서 뒤까지다**
15. `gwlb-endpoint-cross-account-inspection` — **게이트웨이 로드 밸런서 엔드포인트가 계정 경계를 넘는다**

### 문항 id

`elastic-load-balancing` — 20개: `q078` `q079` `q080` `q081` `q190` `q191` `q453` `q454` `q455` `q456` `q457` `q458` `q459` `q460` `q461` `q462` `q463` `q464` `q465` `q466`
...
node scripts/content-audit.mjs elastic-load-balancing# ①②④ 0건
```
