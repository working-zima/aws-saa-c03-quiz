...
|---|---|---|---|---|---|---|---|---|
| `vpc-networking` | 20 | 22 | 6 | 3 (개념 2/문항 1) | — | — | — | — |
...
`vpc-networking` — 개념 20개. **①이 걸린 것은 굵게 표시했다.**
...
`vpc-networking` — 22개: `q100` `q101` `q102` `q103` `q104` `q105` `q106` `q107` `q108` `q209` `q210` `q211` `q212` `q213` `q586` `q587` `q588` `q589` `q590` `q591` `q592` `q593`

### 이 주제에 미리 알려 둘 것 — `vpc-networking`

**인터페이스 VPC 엔드포인트의 연결 대상은 방금 ADR-031이 판정했다.** `vpc-endpoint`와
`endpoint-pricing`, 그리고 `q104`·`q105`·`q209`의 해설이 이미 맞춰져 있다 —
**다시 열어 고치지 마라.** 인터페이스는 S3에 닿고(유료), DynamoDB는 게이트웨이 전용이다.
...
node scripts/content-audit.mjs vpc-networking# ①②④ 0건
```
