...
|---|---|---|---|---|---|---|---|---|
| `sqs-sns-eventbridge` | 33 | 40 | 14 | 1 (개념 0/문항 1) | — | — | — | — |

③④⑤⑥의 실제 건수는 `node scripts/content-audit.mjs <주제id>`로 직접 재라 —
이 표를 쓴 시점과 달라져 있을 수 있다. **①②도 마찬가지다. 숫자가 아니라 도구가 기준이다.**

**이 주제는 개념 33개·문항 40개로 가장 커서 두 step으로 나눴다.**
이 step(A)은 **개념 전부와 문항 앞 20개**를 맡는다. 뒤 20개는 다음 step(B)이 맡는다.

맡은 문항: `q091` `q092` `q093` `q094` `q095` `q096` `q097` `q099` `q203` `q204` `q205` `q207` `q549` `q550` `q551` `q552` `q553` `q554` `q555` `q556`

### 개념 목록 (배열 순서)

`sqs-sns-eventbridge` — 개념 33개. **①이 걸린 것은 굵게 표시했다.**

 0. `sqs` — SQS (Simple Queue Service)
 1. `sns` — SNS (Simple Notification Service)
 2. `eventbridge` — EventBridge
 3. `eventbridge-scheduler` — EventBridge Scheduler
 4. `amazon-mq` — Amazon MQ
 5. `ses` — Amazon SES (Simple Email Service)
 6. `dead-letter-queue` — 데드레터 큐(DLQ)
 7. `sns-is-not-a-queue` — **SNS는 큐가 아니다**
 8. `sns-sqs-fanout-per-consumer` — **팬아웃은 소비자마다 큐를 둔다**
 9. `eventbridge-vs-step-functions` — 이벤트 라우팅과 워크플로 오케스트레이션
10. `eventbridge-ordering-and-retention` — EventBridge가 보장하지 않는 것
11. `eventbridge-event-pattern-vs-polling` — **이벤트 패턴으로 걸면 폴링이 없다**
12. `eventbridge-event-bus-types` — 기본·사용자 지정·파트너 이벤트 버스
13. `eventbridge-pipes` — EventBridge 파이프
14. `eventbridge-api-destination` — EventBridge API 대상
15. `eventbridge-private-api-target` — 프라이빗 API로 이벤트를 넣는 길
16. `eventbridge-resource-change-rule` — **리소스가 바뀌는 순간을 규칙으로 잡는다**
17. `sqs-details` — SQS의 보존 기간과 표준 대기열의 약점
18. `sns-fifo-topic` — SNS FIFO 토픽
19. `ses-inbound-email-receiving` — **SES는 이메일을 받기도 한다**
20. `sqs-queue-depth-scaling` — 대기열 깊이 기반 오토 스케일링
21. `sqs-batch-and-polling` — SQS의 배치·가시성 타임아웃·롱 폴링
22. `sqs-visibility-timeout-vs-processing-time` — **가시성 타임아웃은 처리 시간보다 길어야 한다**
23. `sqs-message-size-limit` — SQS 메시지의 크기 한계
24. `sqs-fifo-message-group-id` — **FIFO 큐의 순서는 메시지 그룹 단위다**
25. `sqs-fifo-deduplication-id` — **중복 제거는 5분 창 안에서 일어난다**
26. `sqs-content-based-deduplication` — **본문이 같으면 알아서 걸러 낸다**
27. `sns-no-message-body-rewrite` — **SNS는 메시지 본문을 고쳐 보내지 못한다**
28. `sqs-queue-policy` — 큐에 직접 붙이는 리소스 기반 정책
29. `cross-account-sns-to-sqs-queue-policy` — **계정을 넘는 발행은 큐 정책에 토픽 ARN을 적는다**
30. `sqs-encryption-and-consumer-kms-permission` — **큐를 암호화하면 소비자에게 복호화 권한이 필요하다**
31. `sns-encrypted-topic-publish-permissions` — **암호화된 SNS 토픽에 게시하려면 권한이 세 군데다**
32. `sqs-vpc-endpoint-and-queue-policy` — **큐로 가는 길을 사설 경로로 묶는다**
...
node scripts/content-audit.mjs sqs-sns-eventbridge# ①②④ 0건
```
