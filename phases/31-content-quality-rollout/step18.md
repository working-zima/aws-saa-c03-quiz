...
### 문체 기준 넷 — **사용자가 step 0~2를 검수하고 직접 낸 것이다. 이것이 합격선이다**

사용자의 판정은 "전체적으로는 통과에 가깝다. 이전 설명보다 훨씬 자연스럽고, 초보자가 읽을
때도 `개념 정의 → 비교 → 사용 조건 → 다른 선택지가 아닌 이유`의 흐름이 잘 보인다"였고,
아래 넷은 그 위에 얹는 보정이다. **`content-audit.mjs`는 이 넷을 하나도 세지 못한다.**

**1. 은유적 추상어를 반복하지 마라.** `자리`·`축`·`갈린다`·`밀린다` 계열이다. 한두 번은
괜찮지만 반복되면 문장이 인위적으로 느껴진다. **하는 말을 그대로 써라.**

| 쓰지 마라 | 이렇게 써라 |
|---|---|
| 그 자리를 맡는 것이 이 클래스다 | 이럴 때 S3 Intelligent-Tiering을 선택한다 |
| 이 조건에서 갈리는 상대는 | 이때 비교 대상이 되는 것은 |
| 보관 단가에서 밀린다 | 보관 단가가 더 높다 |
| 클래스별 비용에는 축이 둘이다 | 클래스별 비용은 두 가지로 나뉜다 |
| 결정을 돕는 자리에 서지 실행하는 자리에 서지 않는다 | 결정을 돕는 기능이지 실행하는 기능이 아니다 |
| 판단 축이다 / 두 번째 축은 | 판단 기준이다 / 두 번째 기준은 |

이것은 ADR-030 기준 5(`돌린다`·`띄운다`를 쓰지 않는다)와 같은 요구를 **설명문 쪽으로 넓힌
것**이다. `흔든다`처럼 도구가 세지 않는 낱말도 같은 계열이면 고쳐라.

**2. 글의 진행을 설명하는 문장을 줄여라.** `앞의 둘`·`여기서부터는`·`이제 그것들을 가르는
축을 본다`·`그 첫 자리가` 같은 것이다. 개념 사이의 연결에 **실제로 필요한 경우에는 남기되,
없어도 이해되는 문장은 빼라.** 독자가 글의 구조를 계속 의식하게 하지 말고 개념 자체를 바로
이해하게 해라.

| 쓰지 마라 | 이렇게 해라 |
|---|---|
| 여기까지가 클래스 여덟 개 각각이고, 이제 그것들을 가르는 축을 본다. 첫 축은 조회 요청에 바로 응답하는지다 | 스토리지 클래스를 가르는 첫 번째 기준은 조회 요청에 바로 응답하는지다 |
| 앞의 두 클래스가 …를 맡는다면, 이 클래스는 …를 맡는다 | S3 Standard-IA는 읽는 일 자체가 드문 데이터에 사용한다 |
| 여기서부터는 복제다. 앞의 넷이 버킷 하나 안에서 하는 일이라면, 복제는 … | 복제는 같은 객체를 다른 버킷에도 두는 기능이다 |
| 앞에서 본 다중 AZ의 정반대편이 단일 AZ다 | 단일 AZ는 다중 AZ와 반대로 … |
| 이 주제는 A를 먼저 보고 그다음 B를 본다 | (지운다. 필요하면 사실로 바꾼다 — "S3는 이런 스토리지 클래스를 여덟 개 제공한다") |

**⚠️ 이 기준은 ⓑ「도입 문장」 처방과 부딪힌다.** 개념이 왜 거기 있는지를 세우라는 요구를
기계적으로 따르면 주제마다 진행 안내 문장이 쌓이고, **사용자가 바로 그 결과를 보고 이 기준을
냈다.** 연결은 필요한 곳에만 세우고, 세울 때도 `앞의`·`여기서부터`가 아니라 **대상의 이름**을
불러라("앞의 둘이 …라면" → "버전 관리와 객체 잠금이 …라면").

**3. 한 문단에 역할을 하나만 줘라.** 개념 설명·시험 판단 기준·오답 제거 이유를 한 문단에
몰아넣으면 문장이 다시 무거워진다. `무엇인가 → 언제 쓰는가 → 주의할 점/비교`로 나눈다.
**특히 초보자가 처음 보는 개념에서는 첫 1~2문장만 읽어도 핵심 의미가 잡혀야 한다.**
다만 **문단 개수는 바꾸지 마라** — `data.test.ts`가 문단 번호로 본문을 단언하는 자리가 있다.
한 문단 안에서 문장 순서를 고쳐 핵심을 앞으로 끌어내는 것으로 푼다.

**4. AWS 공식 용어를 쉬운 말로 갈아치우지 마라. 먼저 제시하고 풀어 줘라.** 학습자가 시험장과
AWS 문서에서 만날 이름은 공식 용어다. 쉬운 말로 대체해 버리면 그 이름을 배울 자리가 사라진다.
...
**용어 자체가 출처에 있어야 한다는 규칙은 그대로다**(ADR-009·ADR-010). 위 `지터`는
`docs/source/dump-gaps/Q151-Q200.md`가 이미 쓰는 말이라 새로 들여온 것이 아니다.
출처에 없는 이름을 이 기준을 핑계로 만들어 넣지 마라.

**스스로 검사하는 법** — 작업을 마치고 이 두 정규식으로 자기 주제를 훑어라. 걸린 것을 전부
고칠 필요는 없지만(`자리한`=위치하다처럼 자연스러운 쓰임이 있다), **한 주제에 여러 건이
남았다면 그것이 사용자가 지적한 상태다.**

```bash
node -e "…" # 개념·문항 본문에서 /자리|축이|축을|축은|갈린|갈리|밀린|맡는 것/ 과
            #                    /앞의 (둘|셋|넷)|여기서부터|여기까지|이제 그것|앞에서 본/ 을 센다
```
