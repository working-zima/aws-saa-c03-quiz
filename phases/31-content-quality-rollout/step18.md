# Step 18: security-groups-nacl-and-hybrid-connectivity
...
| 주제 | 개념 | 문항 |
|---|---|---|
| `security-groups-nacl` | 9 | 17 |
| `hybrid-connectivity` | 19 | 24 |

### 개념 목록 (배열 순서 — 이름은 바꾸지 않는다)

`security-groups-nacl`

 0. `security-group` — 보안 그룹 (Security Group)
 1. `nacl` — NACL (Network Access Control List, 네트워크 ACL)
 2. `security-group-stateful-vs-nacl-stateless` — 상태 저장과 상태 비저장
 3. `web-acl-vs-nacl` — Web ACL과 네트워크 ACL은 다른 것
 4. `security-group-referencing` — 보안 그룹 참조
 5. `nacl-rule-limit` — NACL의 규칙 수 제한
 6. `nacl-deny-at-source-subnet` — 거부 규칙은 보내는 쪽 서브넷에 건다
 7. `alb-security-group-outbound-and-health-check-port` — 로드 밸런서 보안 그룹은 나가는 쪽도 열어야 한다
 8. `nlb-security-group` — NLB에도 보안 그룹을 붙인다
...
`security-groups-nacl` — `q129` `q130` `q131` `q132` `q133` `q134` `q135` `q136` `q137` `q224` `q225` `q226` `q594` `q595` `q596` `q597` `q598`

`hybrid-connectivity` — `q109` `q110` `q111` `q112` `q113` `q114` `q115` `q116` `q214` `q215` `q216` `q217` `q218` `q219` `q599` `q600` `q601` `q602` `q603` `q604` `q605` `q606` `q607` `q608`
...
const T=['security-groups-nacl','hybrid-connectivity'];
