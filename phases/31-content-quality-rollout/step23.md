# Step 23: iam-permissions-and-identity-federation
...
| 주제 | 개념 | 문항 |
|---|---|---|
| `iam-permissions` | 18 | 23 |
| `identity-federation` | 11 | 13 |

### 개념 목록 (배열 순서 — 이름은 바꾸지 않는다)

`iam-permissions`

 0. `iam` — IAM (Identity And Access Management)
 1. `instance-profile` — IAM 인스턴스 프로파일
 2. `iam-group-policy-attachment` — 권한은 사용자가 아니라 그룹에 붙인다
 3. `iam-roles-anywhere` — IAM Roles Anywhere
 4. `iam-access-analyzer` — IAM Access Analyzer
 5. `network-access-analyzer` — Network Access Analyzer
 6. `least-privilege` — 최소 권한 원칙 (Least Privilege)
 7. `abac` — 속성 기반 액세스 제어(ABAC)
 8. `permissions-boundary` — 권한 경계
 9. `cross-account-iam-role` — 계정 간 IAM 역할과 신뢰 정책
10. `iam-group-users-only` — IAM 그룹에는 EC2를 넣을 수 없다
11. `iam-user-is-account-scoped` — IAM 사용자는 계정 밖으로 나가지 않는다
12. `iam-explicit-deny-precedence` — 명시적 거부가 허용을 이긴다
13. `iam-notaction-deny` — NotAction을 쓴 Deny로 한 서비스만 남긴다
14. `iam-requested-region-condition` — 요청이 향한 리전을 조건으로 건다
15. `access-analyzer-delegated-administrator` — 조직 전체 결과는 위임 관리자 계정에 모인다
16. `root-user-multiple-mfa` — 루트 사용자에게 MFA 장치를 여러 개 붙인다
17. `root-user-cannot-be-disabled` — 루트 사용자는 없앨 수 없다

`identity-federation`

 0. `identity-center` — Identity Center
 1. `sts` — STS (Security Token Service)
 2. `cognito` — Cognito
 3. `aws-directory-service` — AWS Directory Service와 AD Connector
 4. `sts-assume-role` — STS로 임시 자격 증명 받기
 5. `identity-center-external-idp` — IAM Identity Center와 외부 IdP의 연결
 6. `cognito-pools` — Cognito 사용자 풀과 자격 증명 풀
 7. `cognito-social-idp-federation` — Cognito 사용자 풀과 소셜 로그인 연동
 8. `custom-identity-broker-for-non-saml` — SAML을 못 쓰는 디렉터리는 브로커로 잇는다
 9. `identity-center-permission-set` — 권한 세트
10. `saml-federation-role-to-ad-group-mapping` — 사내 디렉터리를 SAML 2.0으로 붙이고 역할을 그룹에 맵핑한다

### 문항 id

`iam-permissions` — `q155` `q156` `q157` `q237` `q238` `q239` `q685` `q686` `q687` `q688` `q689` `q690` `q691` `q692` `q693` `q694` `q695` `q696` `q697` `q698` `q699` `q700` `q701`

`identity-federation` — `q158` `q159` `q160` `q161` `q240` `q241` `q702` `q703` `q704` `q705` `q706` `q707` `q708`
...
const T=['iam-permissions','identity-federation'];
