...
`lambda` — 개념 18개. **①이 걸린 것은 굵게 표시했다.**

 0. `lambda` — Lambda
 1. `lambda-function-url` — Lambda 함수 URL
 2. `lambda-function-url-iam-auth` — 함수 URL의 AWS_IAM 인증 유형
 3. `lambda-vpc-access` — Lambda의 VPC 연결
 4. `lambda-container-image` — **Lambda를 컨테이너 이미지로 배포한다**
 5. `lambda-invocation-types` — 이벤트 호출과 요청-응답 호출
 6. `lambda-memory-cpu-proportional` — **Lambda는 메모리를 올리면 CPU가 함께 올라간다**
 7. `lambda-reserved-concurrency` — **예약된 동시성은 콜드 스타트를 줄이지 않는다**
 8. `lambda-provisioned-concurrency-autoscaling` — **프로비저닝된 동시성은 고정값으로 두지 않는다**
 9. `lambda-concurrency-limit-throttling` — **동시 실행 한도를 넘으면 호출이 거절된다**
10. `lambda-kinesis-event-source` — **Lambda가 스트림 레코드를 직접 처리한다**
11. `lambda-snapstart` — Lambda SnapStart
12. `lambda-memory-ceiling` — Lambda 한 번 실행의 메모리 상한
13. `lambda-layer-size-limit` — Lambda 레이어의 크기 제한
14. `lambda-efs-mount` — **Lambda 함수에 EFS를 마운트한다**
15. `lambda-version-alias-config-freeze` — **환경 변수는 버전과 함께 굳는다**
16. `lambda-execution-role-logs` — **로그를 남기려면 실행 역할에 로그 권한이 있어야 한다**
17. `serverless-runtime-no-os-access` — **관리형 런타임에는 들어갈 운영 체제가 없다**

### 문항 id

`lambda` — 25개: `q085` `q086` `q087` `q197` `q199` `q489` `q490` `q491` `q492` `q493` `q494` `q495` `q496` `q497` `q498` `q499` `q500` `q501` `q502` `q503` `q504` `q505` `q506` `q507` `q508`
...
node scripts/content-audit.mjs lambda# ①②④ 0건
```
