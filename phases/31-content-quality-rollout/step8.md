...
|---|---|---|---|---|---|---|---|---|
| `rds-storage-features` | 21 | 28 | 8 | 8 (개념 3/문항 5) | — | — | — | — |
...
`rds-storage-features` — 개념 21개. **①이 걸린 것은 굵게 표시했다.**

 0. `rds` — RDS
 1. `storage-types` — RDS 스토리지 유형
 2. `features` — RDS 기능
 3. `rds-blue-green-deployment` — RDS 블루/그린 배포
 4. `rds-custom` — RDS Custom
 5. `rds-iam-database-authentication` — RDS의 IAM 데이터베이스 인증
 6. `rds-encryption-scope-and-in-transit` — 저장 중 암호화가 미치는 범위와 전송 중 암호화
 7. `storage-type-names` — RDS 스토리지 유형의 실제 이름
 8. `multi-az-standby-limits` — 다중 AZ 대기 인스턴스로는 할 수 없는 것
 9. `rds-multi-az-db-cluster` — **다중 AZ DB 인스턴스와 다중 AZ DB 클러스터는 다르다**
10. `read-replica-vs-cache` — 캐시가 듣지 않는 조건
11. `connection-issue-heuristic` — 연결(Connection) 문제의 정답 신호
12. `rds-proxy-failover` — **RDS Proxy가 장애 조치에도 관여한다**
13. `rds-snapshot-cross-region-copy` — **스냅샷을 다른 리전으로 복사해 재해에 대비한다**
14. `automated-backup-retention` — RDS 자동 백업의 보존 한계
15. `rds-manual-snapshot-retention` — **수동 스냅샷은 지울 때까지 남는다**
16. `rds-pitr-transaction-log-interval` — **특정 시점 복구의 정밀도는 5분이다**
17. `rds-multi-az-failover-rto` — 다중 AZ 장애 조치에 걸리는 시간
18. `rds-stop-instance-restart` — **RDS 인스턴스는 멈춰 둘 수 있고, 7일 뒤 저절로 켜진다**
19. `rds-encrypt-existing-instance` — **이미 만든 인스턴스는 그 자리에서 암호화하지 못한다**
20. `rds-custom-byol` — **RDS Custom에는 보유 라이선스를 가져다 쓴다**

### 문항 id

`rds-storage-features` — 28개: `q058` `q059` `q060` `q061` `q062` `q063` `q064` `q065` `q179` `q180` `q181` `q182` `q375` `q376` `q377` `q378` `q379` `q380` `q381` `q382` `q383` `q384` `q385` `q386` `q387` `q388` `q389` `q390`
...
node scripts/content-audit.mjs rds-storage-features# ①②④ 0건
```
