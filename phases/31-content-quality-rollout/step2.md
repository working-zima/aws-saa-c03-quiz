# Step 2: s3-versioning-lifecycle
...
|---|---|---|---|---|---|---|---|---|
| `s3-versioning-lifecycle` | 10 | 16 | 1 | 2 (개념 0/문항 2) | — | — | — | — |
...
`s3-versioning-lifecycle` — 개념 10개. **①이 걸린 것은 굵게 표시했다.**
 ...
 7. `s3-replication-cross-account-kms` — **복제는 다른 계정 버킷으로도 간다**
 8. `object-lock-prerequisites` — 객체 잠금의 전제 조건과 MFA 삭제의 한계
 9. `s3-lifecycle-rules-and-size-filter` — 수명 주기 구성은 버킷에 하나, 규칙은 여럿

### 문항 id

`s3-versioning-lifecycle` — 16개: `q025` `q026` `q027` `q028` `q029` `q030` `q031` `q032` `q033` `q171` `q172` `q271` `q272` `q273` `q274` `q275`
...
node scripts/content-audit.mjs s3-versioning-lifecycle# ①②④ 0건
```
