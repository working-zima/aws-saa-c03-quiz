...
|---|---|---|---|---|---|---|---|---|
| `ecs-eks-fargate` | 23 | 25 | 6 | 22 (개념 9/문항 13) | — | — | — | — |
...
`ecs-eks-fargate` — 개념 23개. **①이 걸린 것은 굵게 표시했다.**

 0. `ecs` — ECS
 1. `eks` — Amazon EKS (Elastic Kubernetes Service)
 2. `ecr-image-scan-on-push` — Amazon ECR과 푸시 시 스캔
 3. `aws-batch` — AWS Batch
 4. `elastic-beanstalk` — AWS Elastic Beanstalk
 5. `app2container` — AWS App2Container
 6. `eks-compute-options` — **EKS의 컴퓨팅은 세 갈래이고 관리 책임이 다르다**
 7. `fargate-no-time-limit` — **Fargate에는 실행 시간 제한이 없다**
 8. `fargate-spot` — Fargate Spot
 9. `batch-fargate-compute-environment` — AWS Batch의 컴퓨팅 환경
10. `eks-fargate-pod-isolation` — **Fargate는 파드마다 환경을 격리한다**
11. `eks-cluster-autoscaler` — **파드를 늘리는 것과 노드를 늘리는 것은 다르다**
12. `eks-aws-load-balancer-controller` — AWS Load Balancer Controller
13. `eks-connector` — Amazon EKS Connector
14. `eks-anywhere` — Amazon EKS Anywhere
15. `ecs-task-role` — 태스크 역할과 인스턴스 역할
16. `ecs-task-role-vs-task-execution-role` — **태스크 역할과 태스크 실행 역할은 다른 자리다**
17. `eks-irsa` — 서비스 계정용 IAM 역할(IRSA)
18. `ecs-awsvpc-mode` — ECS의 awsvpc 네트워크 모드
19. `ecs-task-placement-strategy` — ECS 태스크 배치 전략
20. `fargate-per-second-billing` — Fargate의 최소 과금 단위
21. `fargate-efs-mount` — **Fargate 태스크에 EFS를 마운트한다**
22. `eks-secrets-kms-encryption` — EKS 시크릿의 KMS 암호화

### 문항 id

`ecs-eks-fargate` — 25개: `q083` `q084` `q195` `q196` `q201` `q509` `q510` `q511` `q512` `q513` `q514` `q515` `q516` `q517` `q518` `q519` `q520` `q521` `q522` `q523` `q524` `q525` `q526` `q527` `q528`
...
node scripts/content-audit.mjs ecs-eks-fargate# ①②④ 0건
```
