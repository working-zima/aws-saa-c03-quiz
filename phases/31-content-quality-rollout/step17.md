# Step 17: backup-disaster-recovery-and-vpc-networking
...
| 주제 | 개념 | 문항 |
|---|---|---|
| `backup-disaster-recovery` | 11 | 11 |
| `vpc-networking` | 20 | 22 |
...
`backup-disaster-recovery`
 ...
 0. `backup` — AWS Backup
 1. `elastic-disaster-recovery` — AWS Elastic Disaster Recovery(AWS DRS)
 2. `backup-and-restore-dr` — 백업 및 복원
 3. `warm-standby-for-low-rto` — 짧은 RTO는 미리 떠 있는 환경을 요구한다
 4. `backup-long-term-retention` — AWS Backup으로 넘어가야 하는 순간
 5. `backup-ec2-resource-assignment` — EC2를 백업 대상으로 지정하면 볼륨과 구성이 함께 들어간다
 6. `organizations-backup-policy` — Organizations의 백업 정책
 7. `backup-cross-account-copy` — 백업 사본을 다른 계정에 둔다
 8. `backup-s3-continuous-backup` — S3의 연속 백업과 특정 시점 복원
 9. `backup-restore-testing-plan` — 복원 테스트 계획으로 백업을 검증한다
10. `backup-audit-manager` — AWS Backup Audit Manager

`vpc-networking`

 0. `vpc-subnet` — VPC, 서브넷
 1. `internet-gateway` — 인터넷 게이트웨이
 2. `egress-only-igw` — Egress-only 인터넷 게이트웨이
 3. `nat-gateway` — NAT 게이트웨이
 4. `vpc-endpoint` — VPC Endpoint
 5. `privatelink` — PrivateLink
 6. `vpc-peering` — VPC 피어링
 7. `vpc-flow-logs` — VPC 플로우 로그
 8. `comparison` — NAT Gateway vs VPC Endpoint vs PrivateLink vs VPC 피어링
 9. `nat-gateway-traffic-uses-public-endpoints` — NAT를 거친 AWS 서비스 접근은 공용 엔드포인트를 지난다
10. `endpoint-pricing` — 엔드포인트 유형별 비용 차이
11. `privatelink-endpoint-service` — PrivateLink 엔드포인트 서비스
12. `nat-instance` — NAT 인스턴스와 NAT 게이트웨이의 차이
13. `nat-gateway-per-az` — NAT 게이트웨이는 가용 영역마다 둔다
14. `internet-gateway-is-not-per-az` — 인터넷 게이트웨이는 가용 영역마다 두지 않는다
15. `nat-gateway-count-by-environment` — NAT 게이트웨이 수는 환경마다 다르게 잡는다
16. `nat-gateway-elastic-ip` — NAT 게이트웨이에는 엘라스틱 IP를 붙인다
17. `vpc-endpoint-policy` — VPC 엔드포인트 정책
18. `vpc-peering-scaling-limit` — VPC 피어링의 확장 한계
19. `s3-is-regional` — S3는 리전 수준 서비스다

### 문항 id

`backup-disaster-recovery` — `q098` `q208` `q577` `q578` `q579` `q580` `q581` `q582` `q583` `q584` `q585`

`vpc-networking` — `q100` `q101` `q102` `q103` `q104` `q105` `q106` `q107` `q108` `q209` `q210` `q211` `q212` `q213` `q586` `q587` `q588` `q589` `q590` `q591` `q592` `q593`
...
const T=['backup-disaster-recovery','vpc-networking'];
