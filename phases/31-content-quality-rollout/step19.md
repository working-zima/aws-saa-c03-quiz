...
이 step(B)은 **문항 뒤 20개**만 맡는다. 개념과 앞 20개 문항은 앞 step(A)이 이미 끝냈다 —
**A가 세운 용어와 표기를 따라라.** A의 `summary`를 먼저 읽어라.

맡은 문항: `q557` `q558` `q559` `q560` `q561` `q562` `q563` `q564` `q565` `q566` `q567` `q568` `q569` `q570` `q571` `q572` `q573` `q574` `q575` `q576`

### 개념 목록 (배열 순서)

개념은 앞 step(A)이 끝냈다. **읽되 고치지 마라.**

**그래서 아래 「작업 순서」의 2·3번(개념 `name`·본문)과 6번(명사구 단언)은 이 step에
해당하지 않는다.** A가 이미 했다. 이 step은 1·4·5·7번만 한다 — 통독, 문항, 해설,
`questionsSha256`. 아래 AC의 ① 0건은 A의 결과를 확인하는 것이지 이 step이 고칠 것이 아니다.
**①이 남아 있으면 고치지 말고 `summary`에 적어라** — A가 빠뜨린 것이다.
