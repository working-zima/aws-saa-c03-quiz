# Step 19: route53-and-emr-glue-athena
...
`emr-glue-athena`
 ...
 0. `emr` — EMR (Elastic MapReduce)
 1. `emr-node-types` — EMR 클러스터의 세 가지 노드
 2. `spark` — Spark
 3. `glue` — Glue
 4. `glue-crawler` — AWS Glue Crawler
 5. `glue-databrew` — AWS Glue DataBrew
 6. `athena` — Athena
 7. `lake-formation` — AWS Lake Formation
 8. `emr-transient-cluster` — 일시적 클러스터와 장기 실행 클러스터
 9. `emr-managed-scaling` — EMR 관리형 스케일링
10. `glue-etl-with-per-customer-kms-key` — 고객마다 키가 다른 변환 작업은 관리형 ETL로
11. `athena-encrypted-and-pay-per-query` — Athena의 과금 단위와 암호화된 데이터
12. `athena-federated-query` — Athena 페더레이션 쿼리
13. `log-storage-s3-athena` — 가끔 조회하는 로그는 S3에 쌓고 Athena로 읽는다
14. `lake-formation-blueprint-and-athena` — 블루프린트로 모으고 Athena로 권한을 적용한다
15. `lake-formation-lf-tags` — LF 태그로 테이블과 열 권한을 준다
16. `emr-node-instance-family-choice` — 노드 역할에 따라 인스턴스 제품군이 갈린다
17. `emr-runtime-role` — EMR 런타임 역할
18. `emr-security-configuration` — EMR 보안 구성
19. `parquet-columnar-format` — Apache Parquet

### 문항 id

`route53` — `q117` `q118` `q119` `q120` `q121` `q122` `q220` `q221` `q609` `q610` `q611` `q612` `q613` `q614` `q615` `q616`

`emr-glue-athena` — `q123` `q124` `q125` `q127` `q222` `q617` `q618` `q619` `q620` `q621` `q622` `q623` `q624` `q625` `q626` `q627` `q628` `q629` `q630` `q631` `q632`
...
const T=['route53','emr-glue-athena'];
