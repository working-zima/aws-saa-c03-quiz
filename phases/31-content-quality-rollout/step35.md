# Step 35: governance-iac-and-systems-manager-and-ai-ml-services
...
|---|---|---|---|---|---|---|---|---|
| `governance-iac` | 7 | 7 | 0 | 0 (개념 0/문항 0) | — | — | — | — |
| `systems-manager` | 7 | 8 | 0 | 3 (개념 2/문항 1) | — | — | — | — |
| `ai-ml-services` | 6 | 7 | 0 | 3 (개념 1/문항 2) | — | — | — | — |
...
`governance-iac` — 개념 7개. **①이 걸린 것은 굵게 표시했다.**

 0. `cloudformation` — AWS CloudFormation
 1. `service-catalog` — AWS Service Catalog
 2. `control-tower-landing-zone` — Control Tower의 랜딩 존
 3. `resource-access-manager` — AWS Resource Access Manager(AWS RAM)
 4. `workload-discovery` — Workload Discovery on AWS
 5. `control-tower-controls` — Control Tower의 사전 예방적 제어와 탐지 제어
 6. `cloudformation-drift-detection` — CloudFormation 드리프트 감지

`systems-manager` — 개념 7개. **①이 걸린 것은 굵게 표시했다.**

 0. `ssm-run-command` — Systems Manager Run Command
 1. `appconfig` — AWS AppConfig
 2. `ssm-session-manager` — Systems Manager Session Manager
 3. `ec2-instance-connect-endpoint` — EC2 Instance Connect 엔드포인트
 4. `ssm-patch-manager` — Systems Manager Patch Manager
 5. `ssm-managed-instance-core-policy` — 관리형 인스턴스를 만드는 정책
 6. `ssm-inventory` — Systems Manager Inventory

`ai-ml-services` — 개념 6개. **①이 걸린 것은 굵게 표시했다.**

 0. `sagemaker` — Amazon SageMaker AI
 1. `media-ai-service-lineup` — 음성·이미지·번역·문서를 나눠 맡는 AI 서비스 넷
 2. `comprehend` — Amazon Comprehend
 3. `amazon-lex` — Amazon Lex
 4. `sagemaker-autopilot` — SageMaker Autopilot
 5. `rekognition-content-moderation` — Rekognition의 콘텐츠 검토

### 문항 id

`governance-iac` — 7개: `q295` `q296` `q297` `q298` `q299` `q300` `q301`

`systems-manager` — 8개: `q302` `q303` `q304` `q305` `q306` `q307` `q308` `q309`

`ai-ml-services` — 7개: `q310` `q311` `q312` `q313` `q314` `q315` `q316`
...
node scripts/content-audit.mjs governance-iac# ①②④ 0건
node scripts/content-audit.mjs systems-manager# ①②④ 0건
node scripts/content-audit.mjs ai-ml-services# ①②④ 0건
```
