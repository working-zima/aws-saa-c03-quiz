...
|---|---|---|---|---|---|---|---|---|
| `aws-core-services` | 18 | 18 | 1 | 0 (개념 0/문항 0) | — | — | — | — |
...
`aws-core-services` — 개념 18개. **①이 걸린 것은 굵게 표시했다.**
...
17. `blob-offload-to-s3` — **큰 바이너리는 데이터베이스가 아니라 S3에 둔다**

### 문항 id

`aws-core-services` — 18개: `q001` `q002` `q003` `q004` `q005` `q006` `q007` `q008` `q009` `q010` `q011` `q012` `q013` `q014` `q015` `q170` `q317` `q318`

### 이 주제에 미리 알려 둘 것 — `aws-core-services`

**`버킷`·`객체`·`접두사`의 뜻풀이가 `aws-core-services.s3`에 이미 있다**(ADR-028, phase 29 step 0).
이 주제가 그 세 낱말의 원적지다. 지우지 말고, ADR-029대로 다른 주제에서 되풀이될 것을 전제해라.
...
node scripts/content-audit.mjs aws-core-services# ①②④ 0건
```
