...
이 step은 `identity-access`, `cost-management` 주제의 개념 **11개**를 맡는다.

이전 step들이 다른 주제의 개념을 이미 재작성했다 (`rewrite-foundation`, `rewrite-s3`, `rewrite-storage`, `rewrite-database`, `rewrite-compute`, `rewrite-network`, `rewrite-analytics`, `rewrite-security`).
`src/data/topics.json`에서 그 결과를 읽어 문체를 맞춰라. 새로 시작하는 게 아니라 이어 쓰는 작업이다.

## 재작성 대상

### `identity-access` — IAM·Identity Center·STS·Cognito·CloudTrail (★★★, 원본 p48–49)

개념 6개 · 문항 9개(q155~q163)

| 개념 id | name (고정) | 문단 수 | 현재 글자수 | 이 개념을 근거로 하는 문항 |
|---|---|---|---|---|
| `identity-access.iam` | IAM (Identity And Access Management) | 3 | 636 | q155~q157 |
| `identity-access.identity-center` | Identity Center | 2 | 573 | q158~q159 |
| `identity-access.sts` | STS (Security Token Service) | 2 | 133 | q160 |
| `identity-access.cognito` | Cognito | 1 | 110 | q161 |
| `identity-access.cloudtrail` | CloudTrail | 2 | 145 | q162 |
| `identity-access.aws-config` | AWS Config | 1 | 172 | q163 |

### `cost-management` — 절약 플랜·Budgets·Cost Explorer·Billing and Cost Management·Trusted Advisor (★★, 원본 p50–50)

개념 5개 · 문항 6개(q164~q169)

| 개념 id | name (고정) | 문단 수 | 현재 글자수 | 이 개념을 근거로 하는 문항 |
|---|---|---|---|---|
| `cost-management.savings-plan` | 절약 플랜 (Savings Plan) | 2 | 286 | q164~q165 |
| `cost-management.aws-budgets` | AWS Budgets | 2 | 184 | q166 |
| `cost-management.cost-explorer` | Cost Explorer | 2 | 174 | q167 |
| `cost-management.billing-and-cost-management` | Billing and Cost Management | 1 | 103 | q168 |
| `cost-management.trusted-advisor` | Trusted Advisor | 1 | 107 | q169 |
...
### 1단계 — 사실 목록 추출 → `phases/2-rewrite/step8-facts.md`
...
표의 개념 11개를 **전부** 새로 썼는지 스스로 확인하라.

## Acceptance Criteria

```bash
node scripts/check-verbatim.mjs identity-access cost-management   # 전사 구간 0건
...
1. 위 AC 커맨드를 전부 실행한다.
2. 체크리스트를 확인한다:
   - 표의 개념 11개를 전부 재작성했는가? (누락된 것 없이)
   - 표의 문항 15개가 재작성된 본문으로 여전히 풀리는가?
   - 현재 본문에 있던 사실 중 빠진 것이 없는가? (`step8-facts.md`와 대조)
