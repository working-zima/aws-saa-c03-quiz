...
이 step은 `compute-delivery`, `serverless-containers`, `messaging-backup` 주제의 개념 **12개**를 맡는다.

이전 step들이 다른 주제의 개념을 이미 재작성했다 (`rewrite-foundation`, `rewrite-s3`, `rewrite-storage`, `rewrite-database`).
`src/data/topics.json`에서 그 결과를 읽어 문체를 맞춰라. 새로 시작하는 게 아니라 이어 쓰는 작업이다.

## 재작성 대상

### `compute-delivery` — EC2·ELB·Global Accelerator·CloudFront (★★★, 원본 p22–24)

개념 4개 · 문항 9개(q074~q082)

| 개념 id | name (고정) | 문단 수 | 현재 글자수 | 이 개념을 근거로 하는 문항 |
|---|---|---|---|---|
| `compute-delivery.ec2` | EC2 | 5 | 626 | q074~q077 |
| `compute-delivery.elb` | ELB | 4 | 478 | q078~q081 |
| `compute-delivery.cloudfront` | CloudFront | 1 | 101 | q082 |
| `compute-delivery.global-accelerator` | Global Accelerator | 1 | 86 | — |

### `serverless-containers` — ECS·Lambda·Step Functions·API Gateway (★★★, 원본 p25–26)

개념 4개 · 문항 8개(q083~q090)

| 개념 id | name (고정) | 문단 수 | 현재 글자수 | 이 개념을 근거로 하는 문항 |
|---|---|---|---|---|
| `serverless-containers.ecs` | ECS | 3 | 265 | q083~q084 |
| `serverless-containers.lambda` | Lambda | 2 | 493 | q085~q087 |
| `serverless-containers.step-functions` | Step Functions | 2 | 198 | q088 |
| `serverless-containers.api-gateway` | API Gateway | 4 | 309 | q089~q090 |

### `messaging-backup` — SQS·SNS·EventBridge·AWS Backup (★★★, 원본 p27–29)

개념 4개 · 문항 9개(q091~q099)

| 개념 id | name (고정) | 문단 수 | 현재 글자수 | 이 개념을 근거로 하는 문항 |
|---|---|---|---|---|
| `messaging-backup.sqs` | SQS (Simple Queue Service) | 4 | 367 | q091~q094, q099 |
| `messaging-backup.sns` | SNS (Simple Notification Service) | 2 | 276 | q095 |
| `messaging-backup.eventbridge` | EventBridge | 2 | 164 | q096~q097 |
| `messaging-backup.backup` | AWS Backup | 1 | 88 | q098 |
...
### 1단계 — 사실 목록 추출 → `phases/2-rewrite/step4-facts.md`
...
표의 개념 12개를 **전부** 새로 썼는지 스스로 확인하라.

## Acceptance Criteria

```bash
node scripts/check-verbatim.mjs compute-delivery serverless-containers messaging-backup   # 전사 구간 0건
...
1. 위 AC 커맨드를 전부 실행한다.
2. 체크리스트를 확인한다:
   - 표의 개념 12개를 전부 재작성했는가? (누락된 것 없이)
   - 표의 문항 26개가 재작성된 본문으로 여전히 풀리는가?
   - 현재 본문에 있던 사실 중 빠진 것이 없는가? (`step4-facts.md`와 대조)
