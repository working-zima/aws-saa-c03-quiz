...
이 step은 `s3-storage-classes`, `s3-versioning-lifecycle`, `s3-encryption-batch` 주제의 개념 **7개**를 맡는다.

이전 step들이 다른 주제의 개념을 이미 재작성했다 (`rewrite-foundation`).
`src/data/topics.json`에서 그 결과를 읽어 문체를 맞춰라. 새로 시작하는 게 아니라 이어 쓰는 작업이다.

## 재작성 대상

### `s3-storage-classes` — S3 스토리지 클래스 유형 (★★★, 원본 p8–9)

개념 1개 · 문항 9개(q016~q024)

| 개념 id | name (고정) | 문단 수 | 현재 글자수 | 이 개념을 근거로 하는 문항 |
|---|---|---|---|---|
| `s3-storage-classes.storage-classes` | S3 스토리지 클래스 유형 | 6 | 1132 | q016~q024 |

### `s3-versioning-lifecycle` — S3 버전 관리·객체 잠금·수명 주기 정책 (★★★, 원본 p10–12)

개념 3개 · 문항 9개(q025~q033)

| 개념 id | name (고정) | 문단 수 | 현재 글자수 | 이 개념을 근거로 하는 문항 |
|---|---|---|---|---|
| `s3-versioning-lifecycle.versioning` | S3 버전 관리 | 1 | 218 | q025~q026 |
| `s3-versioning-lifecycle.object-lock` | S3 객체 잠금 | 3 | 404 | q027~q030 |
| `s3-versioning-lifecycle.lifecycle-policy` | S3 수명 주기 정책 | 1 | 178 | q031~q033 |

### `s3-encryption-batch` — S3 암호화(SSE)·S3 Batch Operations (★★, 원본 p13–13)

개념 3개 · 문항 6개(q034~q039)

| 개념 id | name (고정) | 문단 수 | 현재 글자수 | 이 개념을 근거로 하는 문항 |
|---|---|---|---|---|
| `s3-encryption-batch.sse` | SSE (Server Side Encryption) | 1 | 217 | q034 |
| `s3-encryption-batch.sse-types` | SSE 종류 | 3 | 473 | q035~q038 |
| `s3-encryption-batch.batch-operations` | S3 Batch Operations | 2 | 190 | q039 |
...
### 1단계 — 사실 목록 추출 → `phases/2-rewrite/step1-facts.md`
...
표의 개념 7개를 **전부** 새로 썼는지 스스로 확인하라.

## Acceptance Criteria

```bash
node scripts/check-verbatim.mjs s3-storage-classes s3-versioning-lifecycle s3-encryption-batch   # 전사 구간 0건
...
1. 위 AC 커맨드를 전부 실행한다.
2. 체크리스트를 확인한다:
   - 표의 개념 7개를 전부 재작성했는가? (누락된 것 없이)
   - 표의 문항 24개가 재작성된 본문으로 여전히 풀리는가?
   - 현재 본문에 있던 사실 중 빠진 것이 없는가? (`step1-facts.md`와 대조)
