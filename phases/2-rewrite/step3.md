...
이 step은 `rds-storage-features`, `aurora-dynamodb-cache` 주제의 개념 **6개**를 맡는다.

이전 step들이 다른 주제의 개념을 이미 재작성했다 (`rewrite-foundation`, `rewrite-s3`, `rewrite-storage`).
`src/data/topics.json`에서 그 결과를 읽어 문체를 맞춰라. 새로 시작하는 게 아니라 이어 쓰는 작업이다.

## 재작성 대상

### `rds-storage-features` — RDS 스토리지 유형과 기능 (★★★, 원본 p19–20)

개념 3개 · 문항 8개(q058~q065)

| 개념 id | name (고정) | 문단 수 | 현재 글자수 | 이 개념을 근거로 하는 문항 |
|---|---|---|---|---|
| `rds-storage-features.rds` | RDS | 1 | 98 | q058 |
| `rds-storage-features.storage-types` | RDS 스토리지 유형 | 2 | 219 | q059 |
| `rds-storage-features.features` | RDS 기능 | 6 | 981 | q060~q065 |

### `aurora-dynamodb-cache` — Aurora·DynamoDB·ElastiCache (★★★, 원본 p21–21)

개념 3개 · 문항 8개(q066~q073)

| 개념 id | name (고정) | 문단 수 | 현재 글자수 | 이 개념을 근거로 하는 문항 |
|---|---|---|---|---|
| `aurora-dynamodb-cache.aurora` | Aurora | 3 | 324 | q066~q068 |
| `aurora-dynamodb-cache.dynamodb` | DynamoDB | 3 | 279 | q069~q071 |
| `aurora-dynamodb-cache.elasticache` | ElastiCache | 2 | 220 | q072~q073 |
...
### 1단계 — 사실 목록 추출 → `phases/2-rewrite/step3-facts.md`
...
표의 개념 6개를 **전부** 새로 썼는지 스스로 확인하라.

## Acceptance Criteria

```bash
node scripts/check-verbatim.mjs rds-storage-features aurora-dynamodb-cache   # 전사 구간 0건
...
1. 위 AC 커맨드를 전부 실행한다.
2. 체크리스트를 확인한다:
   - 표의 개념 6개를 전부 재작성했는가? (누락된 것 없이)
   - 표의 문항 16개가 재작성된 본문으로 여전히 풀리는가?
   - 현재 본문에 있던 사실 중 빠진 것이 없는가? (`step3-facts.md`와 대조)
