...
이 step은 `analytics-monitoring`, `security-groups-nacl` 주제의 개념 **14개**를 맡는다.

이전 step들이 다른 주제의 개념을 이미 재작성했다 (`rewrite-foundation`, `rewrite-s3`, `rewrite-storage`, `rewrite-database`, `rewrite-compute`, `rewrite-network`).
`src/data/topics.json`에서 그 결과를 읽어 문체를 맞춰라. 새로 시작하는 게 아니라 이어 쓰는 작업이다.

## 재작성 대상

### `analytics-monitoring` — EMR·Spark·Redshift·Athena·Kinesis·Glue·X-Ray·CloudWatch (★★, 원본 p38–40)

개념 12개 · 문항 6개(q123~q128)

| 개념 id | name (고정) | 문단 수 | 현재 글자수 | 이 개념을 근거로 하는 문항 |
|---|---|---|---|---|
| `analytics-monitoring.emr` | EMR (Elastic MapReduce) | 1 | 49 | q123 |
| `analytics-monitoring.spark` | Spark | 2 | 120 | q124 |
| `analytics-monitoring.redshift` | RedShift | 1 | 74 | — |
| `analytics-monitoring.athena` | Athena | 2 | 220 | q125 |
| `analytics-monitoring.performance-insight` | Performance Insight | 1 | 67 | — |
| `analytics-monitoring.cloudwatch` | CloudWatch | 1 | 90 | q126 |
| `analytics-monitoring.glue` | Glue | 1 | 137 | q127 |
| `analytics-monitoring.x-ray` | X-Ray | 1 | 155 | — |
| `analytics-monitoring.data-firehose` | Data Firehose | 1 | 91 | — |
| `analytics-monitoring.kinesis-data-streams` | Kinesis Data Streams | 1 | 80 | — |
| `analytics-monitoring.managed-service-apache-flink` | Managed Service for Apache Flink | 1 | 99 | — |
| `analytics-monitoring.streaming-services-comparison` | Data Firehose vs Kinesis Data Streams vs Managed Service for Apache Flink | 2 | 198 | q128 |

### `security-groups-nacl` — 보안 그룹·NACL (★★★, 원본 p41–43)

개념 2개 · 문항 9개(q129~q137)

| 개념 id | name (고정) | 문단 수 | 현재 글자수 | 이 개념을 근거로 하는 문항 |
|---|---|---|---|---|
| `security-groups-nacl.security-group` | 보안 그룹 (Security Group) | 5 | 735 | q129, q131~q133, q136 |
| `security-groups-nacl.nacl` | NACL (Network Access Control List, 네트워크 ACL) | 2 | 251 | q130, q134~q135, q137 |
...
### 1단계 — 사실 목록 추출 → `phases/2-rewrite/step6-facts.md`
...
표의 개념 14개를 **전부** 새로 썼는지 스스로 확인하라.

## Acceptance Criteria

```bash
node scripts/check-verbatim.mjs analytics-monitoring security-groups-nacl   # 전사 구간 0건
...
1. 위 AC 커맨드를 전부 실행한다.
2. 체크리스트를 확인한다:
   - 표의 개념 14개를 전부 재작성했는가? (누락된 것 없이)
   - 표의 문항 15개가 재작성된 본문으로 여전히 풀리는가?
   - 현재 본문에 있던 사실 중 빠진 것이 없는가? (`step6-facts.md`와 대조)
