...
이 step은 `vpc-networking`, `hybrid-connectivity`, `route53` 주제의 개념 **14개**를 맡는다.

이전 step들이 다른 주제의 개념을 이미 재작성했다 (`rewrite-foundation`, `rewrite-s3`, `rewrite-storage`, `rewrite-database`, `rewrite-compute`).
`src/data/topics.json`에서 그 결과를 읽어 문체를 맞춰라. 새로 시작하는 게 아니라 이어 쓰는 작업이다.

## 재작성 대상

### `vpc-networking` — VPC·서브넷·인터넷/NAT 게이트웨이·VPC Endpoint·PrivateLink·피어링 (★★★, 원본 p30–33)

개념 7개 · 문항 9개(q100~q108)

| 개념 id | name (고정) | 문단 수 | 현재 글자수 | 이 개념을 근거로 하는 문항 |
|---|---|---|---|---|
| `vpc-networking.vpc-subnet` | VPC, 서브넷 | 2 | 224 | q100 |
| `vpc-networking.internet-gateway` | 인터넷 게이트웨이 | 1 | 234 | q101 |
| `vpc-networking.nat-gateway` | NAT 게이트웨이 | 3 | 519 | q102~q103 |
| `vpc-networking.vpc-endpoint` | VPC Endpoint | 4 | 333 | q104~q105 |
| `vpc-networking.privatelink` | PrivateLink | 1 | 145 | q106 |
| `vpc-networking.vpc-peering` | VPC 피어링 | 1 | 100 | q107 |
| `vpc-networking.comparison` | NAT Gateway vs VPC Endpoint vs PrivateLink vs VPC 피어링 | 2 | 288 | q108 |

### `hybrid-connectivity` — Site-to-Site VPN·Direct Connect·Transit Gateway (★★★, 원본 p34–35)

개념 4개 · 문항 8개(q109~q116)

| 개념 id | name (고정) | 문단 수 | 현재 글자수 | 이 개념을 근거로 하는 문항 |
|---|---|---|---|---|
| `hybrid-connectivity.site-to-site-vpn` | Site-to-Site VPN | 2 | 177 | q109 |
| `hybrid-connectivity.direct-connect` | Direct Connect | 3 | 297 | q110, q115 |
| `hybrid-connectivity.vpn-vs-direct-connect` | Site-to-Site VPN vs Direct Connect | 2 | 244 | q111~q112, q114 |
| `hybrid-connectivity.transit-gateway` | Transit Gateway | 1 | 104 | q113, q116 |

### `route53` — Route 53 (★★, 원본 p36–37)

개념 3개 · 문항 6개(q117~q122)

| 개념 id | name (고정) | 문단 수 | 현재 글자수 | 이 개념을 근거로 하는 문항 |
|---|---|---|---|---|
| `route53.route53` | Route53 | 1 | 124 | q117 |
| `route53.routing-policies` | Route53 라우팅 정책 | 1 | 412 | q118~q120 |
| `route53.resolver` | Route53 Resolver | 3 | 477 | q121~q122 |
...
### 1단계 — 사실 목록 추출 → `phases/2-rewrite/step5-facts.md`
...
```bash
node scripts/check-verbatim.mjs vpc-networking hybrid-connectivity route53   # 전사 구간 0건
...
2. 체크리스트를 확인한다:
   ...
   - 표의 문항 23개가 재작성된 본문으로 여전히 풀리는가?
   - 현재 본문에 있던 사실 중 빠진 것이 없는가? (`step5-facts.md`와 대조)
