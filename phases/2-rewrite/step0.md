...
이 step은 `aws-core-services`, `region-availability`, `onpremise-migration` 주제의 개념 **15개**를 맡는다.

이 step이 phase 2의 첫 번째다. 여기서 잡은 문체가 이후 8개 step의 기준이 되므로,
1-exam-gaps에서 직접 쓴 개념(각 주제 `concepts` 배열 뒤쪽에 있다)의 문체를 먼저 읽고 맞춰라.

## 재작성 대상

### `aws-core-services` — AWS 핵심 서비스 개요 — EC2·RDS·S3·Route 53·ELB·CloudFront·Lambda (기초, 원본 p1–4)

개념 8개 · 문항 8개(q001~q008)

| 개념 id | name (고정) | 문단 수 | 현재 글자수 | 이 개념을 근거로 하는 문항 |
|---|---|---|---|---|
| `aws-core-services.ec2` | EC2 (Elastic Compute Cloud) | 1 | 56 | q001 |
| `aws-core-services.rds` | RDS (Relational Database Service) | 1 | 74 | q002 |
| `aws-core-services.s3` | S3 (Simple Storage Service) | 2 | 193 | q003 |
| `aws-core-services.route-53` | Route 53 | 2 | 201 | q004 |
| `aws-core-services.dns` | DNS (Domain Name System) | 3 | 462 | q005 |
| `aws-core-services.elb` | ELB (Elastic Load Balancer) | 1 | 124 | q006 |
| `aws-core-services.cloudfront` | CloudFront | 4 | 469 | q007 |
| `aws-core-services.lambda` | Lambda | 1 | 101 | q008 |

### `region-availability` — 리전·가용성·가용 영역·다중 AZ (기초, 원본 p5–6)

개념 5개 · 문항 5개(q009~q013)

| 개념 id | name (고정) | 문단 수 | 현재 글자수 | 이 개념을 근거로 하는 문항 |
|---|---|---|---|---|
| `region-availability.region` | 리전 (Region) | 2 | 262 | q009 |
| `region-availability.availability` | 가용성 (Availability) | 2 | 320 | q010 |
| `region-availability.availability-zone` | 가용 영역 (Availability Zone) | 1 | 180 | q011 |
| `region-availability.multi-az` | 다중 AZ (Multi-AZ) | 1 | 242 | q012 |
| `region-availability.single-az` | 단일 AZ (Single-AZ) | 1 | 145 | q013 |

### `onpremise-migration` — 온프레미스와 마이그레이션 (기초, 원본 p7–7)

개념 2개 · 문항 2개(q014~q015)

| 개념 id | name (고정) | 문단 수 | 현재 글자수 | 이 개념을 근거로 하는 문항 |
|---|---|---|---|---|
| `onpremise-migration.on-premise` | 온프레미스 (On-premise) | 1 | 132 | q014 |
| `onpremise-migration.migration` | 마이그레이션 (Migration) | 1 | 135 | q015 |
...
### 1단계 — 사실 목록 추출 → `phases/2-rewrite/step0-facts.md`
...
표의 개념 15개를 **전부** 새로 썼는지 스스로 확인하라.

## Acceptance Criteria

```bash
node scripts/check-verbatim.mjs aws-core-services region-availability onpremise-migration   # 전사 구간 0건
...
1. 위 AC 커맨드를 전부 실행한다.
2. 체크리스트를 확인한다:
   - 표의 개념 15개를 전부 재작성했는가? (누락된 것 없이)
   - 표의 문항 15개가 재작성된 본문으로 여전히 풀리는가?
   - 현재 본문에 있던 사실 중 빠진 것이 없는가? (`step0-facts.md`와 대조)
