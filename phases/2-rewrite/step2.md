...
이 step은 `block-file-storage`, `data-transfer-services` 주제의 개념 **8개**를 맡는다.

이전 step들이 다른 주제의 개념을 이미 재작성했다 (`rewrite-foundation`, `rewrite-s3`).
`src/data/topics.json`에서 그 결과를 읽어 문체를 맞춰라. 새로 시작하는 게 아니라 이어 쓰는 작업이다.

## 재작성 대상

### `block-file-storage` — EBS·EFS·FSx·인스턴스 스토어 (★★★, 원본 p14–15)

개념 4개 · 문항 9개(q040~q048)

| 개념 id | name (고정) | 문단 수 | 현재 글자수 | 이 개념을 근거로 하는 문항 |
|---|---|---|---|---|
| `block-file-storage.ebs` | EBS (Elastic Block Store) | 2 | 208 | q040~q041 |
| `block-file-storage.efs` | EFS (Elastic File System) | 2 | 447 | q042~q043 |
| `block-file-storage.instance-store` | 인스턴스 스토어 (Instance Store) | 2 | 125 | q044 |
| `block-file-storage.fsx` | FSx (File System for Extended use) | 5 | 373 | q045~q048 |

### `data-transfer-services` — DataSync·Snowball Edge·Transfer Family·Storage Gateway (★★★, 원본 p16–18)

개념 4개 · 문항 9개(q049~q057)

| 개념 id | name (고정) | 문단 수 | 현재 글자수 | 이 개념을 근거로 하는 문항 |
|---|---|---|---|---|
| `data-transfer-services.datasync` | DataSync | 2 | 144 | q049 |
| `data-transfer-services.snowball-edge` | Snowball Edge | 3 | 394 | q050 |
| `data-transfer-services.transfer-family` | Transfer Family | 2 | 204 | q051 |
| `data-transfer-services.storage-gateway` | Storage Gateway | 5 | 667 | q052~q057 |
...
### 1단계 — 사실 목록 추출 → `phases/2-rewrite/step2-facts.md`
...
표의 개념 8개를 **전부** 새로 썼는지 스스로 확인하라.

## Acceptance Criteria

```bash
node scripts/check-verbatim.mjs block-file-storage data-transfer-services   # 전사 구간 0건
...
1. 위 AC 커맨드를 전부 실행한다.
2. 체크리스트를 확인한다:
   - 표의 개념 8개를 전부 재작성했는가? (누락된 것 없이)
   - 표의 문항 18개가 재작성된 본문으로 여전히 풀리는가?
   - 현재 본문에 있던 사실 중 빠진 것이 없는가? (`step2-facts.md`와 대조)
