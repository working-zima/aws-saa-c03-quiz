...
```
sticky bottom-0 -mx-5 border-t border-border bg-page px-5 py-3 sm:-mx-8 sm:px-8
```

- `fixed`가 아니라 **`sticky`**를 쓴다. 이유: 문서 흐름 안에 남으므로 스크롤 최하단에서는
  제자리로 돌아간다. 마지막 본문 줄을 영구히 가리지 않고, 본문에 하단 여백 보정을 넣을 필요도 없다.
- `-mx-5 sm:-mx-8`은 `main`의 좌우 패딩을 상쇄해 바 배경이 화면 폭을 채우게 한다.
  같은 값의 `px-*`를 다시 넣어 내용 정렬은 본문과 맞춘다.
