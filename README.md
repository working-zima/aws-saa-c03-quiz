...
step을 실제로 수행하는 에이전트를 고른다. 기본값은 `claude`다.

```bash
python3 scripts/execute.py {task-name} --agent codex
python3 scripts/execute.py {task-name} --agent codex --max-steps 1   # 한 step만 돌리고 멈춤
```

`--max-steps N`은 N개 step을 돌고 멈춘다. step마다 사람이 리뷰하고 넘어갈 때 쓴다.
중단된 phase는 `completed`로 표시되지 않으므로, 그대로 다시 실행하면 이어서 진행된다.

### claude와 codex의 안전 모델이 다르다

| | claude | codex |
|---|---|---|
| 승인 | `--dangerously-skip-permissions` | `approval_policy=never` |
| 파일 쓰기 | 제한 없음 | `workspace-write` — 작업 디렉토리와 `/tmp` 밖은 차단 |
| 커밋 주체 | 에이전트가 커밋 | **하네스**(`_commit_step`)가 커밋 |

codex는 샌드박스가 `.git` 쓰기까지 막기 때문에 에이전트가 커밋할 수 없다.
그래서 codex 모드에서는 프리앰블이 "커밋하지 마라"로 바뀌고, 커밋은 `execute.py`가 대신 한다.
`npm` 전역 캐시(`~/.npm`)만 예외로 열어 둔다 — 안 열면 `npm install`이 매번 우회 경로를 찾느라 시간을 버린다.

### ⚠ codex 훅은 기본적으로 꺼져 있다

`.codex/hooks.json`은 **hook trust가 없으면 조용히 무시된다.** 경고도 에러도 없다.
그래서 `_codex_cmd`가 `--dangerously-bypass-hook-trust`를 붙인다. **이 플래그를 빼면
가드레일이 하나도 걸리지 않은 채로 돌아간다.** 훅이 실제로 붙었는지는 실행 로그의
`hook: PreToolUse` / `hook: Stop` 줄로 확인할 수 있다.
