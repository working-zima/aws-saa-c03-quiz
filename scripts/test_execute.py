...
class TestCodexAgent:
    def test_codex_command_shape(self, executor):
        """codex 모드의 필수 플래그. 하나라도 빠지면 가드레일이나 샌드박스가 깨진다."""
        ...
        # 훅 트러스트를 우회하지 않으면 .codex/hooks.json 이 조용히 무시된다.
        assert "--dangerously-bypass-hook-trust" in cmd
        assert "workspace-write" in cmd
        assert "approval_policy=never" in cmd
        assert any("network_access=true" in a for a in cmd)
        assert any("writable_roots" in a and ".npm" in a for a in cmd)
        assert "PREAMBLE" in cmd[-1]

    def test_codex_preamble_forbids_commit(self, executor):
        """codex 는 샌드박스 때문에 .git 을 못 쓴다. 커밋을 시키면 실패만 쌓인다."""
        executor._agent = "codex"
        result = executor._build_preamble("", "")
        assert "커밋하지 마라" in result
        assert "feat(mvp):" not in result

    def test_claude_preamble_still_commits(self, executor):
        result = executor._build_preamble("", "")
        assert "모든 변경사항을 커밋하라" in result
        assert "feat(mvp):" in result

    def test_unknown_agent_is_rejected_by_cli(self):
        r = subprocess.run(
            [sys.executable, str(Path(__file__).parent / "execute.py"), "0-mvp", "--agent", "gpt"],
